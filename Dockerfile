# Hostess7 embed — minimal sovereign brain container (war profile)
...
ENV PYTHONUNBUFFERED=1 \
    ...
    HOSTESS7_WAR_PROFILE=${WAR_PROFILE}
...
RUN pip install --no-cache-dir -r requirements.txt \
    ...
    && chmod +x Hostess7.sh docker/entrypoint.sh

EXPOSE 8080

HEALTHCHECK --interval=30s --timeout=10s --start-period=90s --retries=3 \
    CMD python -m hostess7.cohesion iq || python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8080/health', timeout=3)"
