#!/usr/bin/env bash
# Hostess7 → GitHub via secure git (pinned SSH, MITM-resistant, port-443 tunnel fallback)
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
# shellcheck source=hostess7-version.sh
source "$(dirname "${BASH_SOURCE[0]}")/hostess7-version.sh"
VERSION="${HOSTESS7_VERSION}"
TAG="v${VERSION}"
REMOTE="${HOSTESS7_GITHUB_REMOTE:-git@github.com:ZacharyGeurts/Hostess7.git}"
BRANCH="${HOSTESS7_GITHUB_BRANCH:-main}"
PUSH_TAG="${HOSTESS7_PUSH_TAG:-0}"

log() { printf '[hostess7-secure-git] %s\n' "$*"; }

export NEXUS_INSTALL_ROOT="${NEXUS_INSTALL_ROOT:-$(cd "$ROOT/.." && pwd)}"
export NEXUS_STATE_DIR="${NEXUS_STATE_DIR:-${NEXUS_INSTALL_ROOT}/.nexus-state}"
export SG_ROOT="${SG_ROOT:-$(cd "$NEXUS_INSTALL_ROOT/.." && pwd)}"
STAGE="${HOSTESS7_SECURE_PUBLISH_DIR:-${NEXUS_INSTALL_ROOT}/dist/hostess7-secure-publish}"
SECURE="${ROOT}/scripts/hostess7_secure_git.py"
PY=python3

log "sync version"
"$PY" "$ROOT/scripts/hostess7-sync-version.py"

log "pages-build (surfaces + brain + API) before stage"
"$PY" "$ROOT/scripts/hostess7_pages_surfaces_build.py"
"$PY" "$ROOT/scripts/hostess7_pages_brain_build.py" --full
"$PY" "$ROOT/scripts/hostess7_pages_api_export.py"

log "H7 optimise — data/ doctrine JSON + Pages PNG (browser JSON stays plain)"
"$PY" "$ROOT/scripts/hostess7-h7-optimise.py" --apply --profile=publish

log "verify pinned GitHub hosts"
"$PY" "$SECURE" verify
ROUTE="$("$PY" "$SECURE" route | "$PY" -c 'import json,sys; print(json.load(sys.stdin).get("route","none"))')"
log "route: ${ROUTE} (direct=22, tunnel=443)"

log "stage clean tree → ${STAGE}"
rm -rf "$STAGE"
mkdir -p "$STAGE"
rsync -a --delete \
  --exclude='.git' \
  --exclude='cache' \
  --exclude='zac' \
  --exclude='__pycache__' \
  --exclude='*.pyc' \
  "${ROOT}/" "${STAGE}/"

git -C "$STAGE" init -b "$BRANCH"
git -C "$STAGE" config user.email "${GIT_USER_EMAIL:-gzac5314@users.noreply.github.com}"
git -C "$STAGE" config user.name "${GIT_USER_NAME:-ZacharyGeurts}"
git -C "$STAGE" add -A
git -C "$STAGE" commit -m "Hostess7 ${VERSION} — main project source (review publish, no release tag)" || true

if [[ "$PUSH_TAG" == "1" ]]; then
  log "push ${BRANCH} + ${TAG} via pinned SSH"
  "$PY" "$SECURE" push "$STAGE" --branch "$BRANCH" --remote "$REMOTE" --tag "$TAG" --force
else
  log "push ${BRANCH} only (HOSTESS7_PUSH_TAG=0 — no GitHub release tag)"
  "$PY" "$SECURE" push "$STAGE" --branch "$BRANCH" --remote "$REMOTE" --force
fi

log "pages gh-pages"
HOSTESS7_VERSION="$VERSION" bash "$ROOT/scripts/publish-hostess7-pages.sh"

log "done → https://github.com/ZacharyGeurts/Hostess7"