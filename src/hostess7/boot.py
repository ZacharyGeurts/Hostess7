...
from hostess7.paths import brain_state_dir, hostess7_root, scripts_available, scripts_dir
...
def main() -> int:
    ...
    if not scripts_available():
        from hostess7.cohesion import benchmark_iq

        doc = benchmark_iq()
        print(f"hostess7-boot: scripts unavailable — cohesion score {doc.get('score')}", file=sys.stderr)
        try:
            snapshot("boot-packaged")
        except OSError:
            pass
