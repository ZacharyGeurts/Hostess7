...
(function () {
  ...
  function applySeal(doc, gpu) {
    const space = {
      ...
      posture: doc.posture || {},
      webgpu_bridge: gpu,
      world_url: doc.world_url || location.origin + "/Hostess7/queen/browser.html",
  ...
  function openWebbrowserTab(url, opts) {
    const target = url || "/Hostess7/queen/queen-chips-cores.html";
    setDockTab("browser");
  ...
  function wireWorld() {
    ...
    else if (dock === "terminal") {
      // embed works reliably inside browser shell / tabs / desktop windows
      const embed = "/Hostess7/queen/queen-gnu-terminal-embed.html";
      if (typeof navigate === "function") navigate(embed);
      else loadFrame(embed);
    }
