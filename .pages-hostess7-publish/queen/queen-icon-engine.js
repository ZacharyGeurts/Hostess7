...
(function (global) {
  ...
  const WORLD_ICONS = "/Hostess7/queen/assets/icons/";
  ...
  function iconUrl(batteryId, size) {
    ...
    if (hit?.icon_url) return hit.icon_url;
    const sz = size || 32;
    let bid = String(batteryId);
    if (bid.startsWith("shell-") || bid.includes("shell-")) {
      return `${WORLD_ICONS}shell/${bid.replace(/^shell-/, "")}-${sz}.png`;
    }
    return `${WORLD_ICONS}${bid.replace(/^(file-|queen-prog-)/, (m) => (m === "file-" ? "file-" : "prog-"))}-${sz}.png`;
  }
