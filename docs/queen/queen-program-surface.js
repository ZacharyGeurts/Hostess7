...
(function (global) {
  ...
  function qUrl(p) {
    ...
    const b = pn.includes('/Hostess7/') ? '/Hostess7/queen/' : (pn.includes('/Hostess7/queen/') ? '/Hostess7/queen/' : '');
  ...
  async function launchProgram(item, opts) {
    ...
    if (mode === "queen_window") {
      ...
      // Use Queen browser for pages to avoid host Firefox theme; full featured Queen chrome
    ...
    // Prefer Queen browser chrome for external pages (not host Firefox)
