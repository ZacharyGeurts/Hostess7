...
(function () {
  ...
  function iconHtml(icon, entry, size) {
    const sz = size || (state.viewMode === "grid" ? 48 : 20);
    if (entry && global.QueenIconEngine?.fileIconHtml) {
      return global.QueenIconEngine.fileIconHtml(entry, sz);
    }
    if (icon && typeof icon === "object" && icon.type === "battery" && global.QueenIconEngine?.fileIconHtml) {
      return global.QueenIconEngine.fileIconHtml({ kind: icon.id === "folder" ? "dir" : "file", file_type: { type_id: icon.id } }, sz);
    }
  ...
  function renderPlaces() {
    const treeEl = $("qf-tree");
    if (!treeEl) return;
    // Insert or update a places header above tree (Nemo style)
    let places = $("qf-places");
    if (!places) {
      places = document.createElement("div");
      places.id = "qf-places";
      places.className = "qf-places";
      places.innerHTML = `<div class="qf-places-label">Places</div>`;
      treeEl.parentNode?.insertBefore(places, treeEl);
    }
    const items = [
      { id: "computer", label: "Computer", icon: "🖥️", action: () => openPath((state.roots && state.roots[0] && state.roots[0].path) || "/") },
      { id: "home", label: "Home", icon: "🏠", action: () => {
          const h = (state.roots || []).find(r => /home|user|sg|kilroy/i.test(r.path || "")) || state.roots?.[0];
          openPath(h ? h.path : (state.path || "/"));
        } },
      { id: "trash", label: "Trash", icon: "🗑️", action: () => openPath("trash:") },
      { id: "files", label: "Files", icon: "📂", action: () => openPath(state.roots?.[0]?.path || "/") },
    ];
    places.innerHTML = `<div class="qf-places-label">Places</div>` + items.map(it => {
      const active = (it.id === "trash" && state.path === "trash:") || (state.path && state.path === it.label) ? " active" : "";
      return `<button type="button" class="qf-place${active}" data-place="${it.id}"><span class="ico">${it.icon}</span>${it.label}</button>`;
    }).join("");
    places.querySelectorAll(".qf-place").forEach(btn => {
      const id = btn.dataset.place;
      const it = items.find(x => x.id === id);
      if (it) btn.addEventListener("click", it.action);
    });
  }
    ...
    if (isGrid) {
      box.innerHTML = sorted
        .map((e) => {
          ...
          const hide = q && !match;
          const ic = iconHtml(resolveIcon(e), e, 48);
          const nm = esc(e.name);
          const sz = alwaysMeta(e) || launchMeta(e) || "";
          return `<div class="qf-grid-item${state.selected === e.path ? " selected" : ""}${match ? " match" : ""}${hide ? " hidden-by-search" : ""}" data-path="${esc(e.path)}" data-kind="${esc(e.kind)}">
            <div class="icon">${ic}</div>
            <div class="label">${nm}</div>
            <div class="meta">${sz}</div>
          </div>`;
        }).join("");
      box.querySelectorAll(".qf-grid-item").forEach((el) => {
        const path = el.dataset.path;
        el.addEventListener("click", () => {
          state.selected = path;
          box.querySelectorAll(".qf-grid-item").forEach((r) => r.classList.toggle("selected", r.dataset.path === state.selected));
          showPreview(path);
        });
        el.addEventListener("dblclick", () => {
          const entry = findEntry(path);
          runDefaultAction(entry || { path, kind: el.dataset.kind });
        });
        el.addEventListener("contextmenu", (ev) => {
          ev.preventDefault();
          const entry = findEntry(path);
          if (entry) showCtx(ev.clientX, ev.clientY, entry);
        });
      });
      return;
    }
  ...
  async function openPath(path, opts = {}) {
    if (path === "trash:" || path === "trash") {
      // virtual trash view (Nemo-like); backend may enhance later
      $("qf-status-left").textContent = "Trash · (items moved here are recoverable via stabilize)";
      const box = $("qf-rows");
      if (box) box.innerHTML = '<div class="qf-empty">Trash is empty or managed by field-drive (use Queen tools to restore)</div>';
      renderPlaces();
      return;
    }
    if (!opts.skipNavPush) {
  ...
  function bindChrome() {
    ...
    const viewBtn = $("qf-view-toggle");
    if (viewBtn) {
      // restore
      if (localStorage.getItem("qf-view") === "grid") {
        state.viewMode = "grid";
      ...
      viewBtn.addEventListener("click", () => {
        ...
        const isG = state.viewMode === "grid";
        viewBtn.textContent = isG ? "List" : "Grid";
        viewBtn.classList.toggle("active", isG);
        try { localStorage.setItem("qf-view", state.viewMode); } catch(_) {}
        renderRows(state.entries);
      });
    }
  ...
  async function init() {
    ...
    try {
      ...
      if (qp.get("view") === "trash") { await openPath("trash:"); return; }
    } catch(_) {}
