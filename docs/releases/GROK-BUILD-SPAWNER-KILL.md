# GrokBuild SpawnerKill

**Date:** 2026-07-04  
**Product:** GrokBuild SpawnerKill (`field-grok-spawner-kill`)  
...
Always-on **instakill service** for rogue **Grok agent harness spawners** — the `dump_bash_state` shells, `GROK_AGENT=1` wrappers, `systemd-inhibit` sleepers, duplicate `grok agent serve` lanes, and subagent/Task-tool leaks that pile up after agent turns.
...
See `data/field-grok-spawner-patterns.json` — 14 spawner signatures, `keep: 1` for active harness under live `grok` parent.
