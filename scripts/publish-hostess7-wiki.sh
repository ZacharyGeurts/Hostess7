...
WIKI_REMOTE="${WIKI_REMOTE:-https://github.com/ZacharyGeurts/Hostess7.wiki.git}"
...
PUSH_URL="${WIKI_REMOTE}"
if [[ -n "$TOKEN" ]]; then
  PUSH_URL="https://ZacharyGeurts:${TOKEN}@github.com/ZacharyGeurts/Hostess7.wiki.git"
fi
git -C "$WIKI_REPO" remote set-url origin "$PUSH_URL" 2>/dev/null || git -C "$WIKI_REPO" remote add origin "$PUSH_URL"
