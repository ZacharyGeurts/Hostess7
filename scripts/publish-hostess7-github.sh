...
REPO_SLUG="ZacharyGeurts/Hostess7"
PUSH=0
DRY=0
...
RSYNC_EXCLUDES=(
  ...
  --exclude='Grok16/vendor/**'
  --exclude='GrokLab/deploy/qemu-vms/**/disk.qcow2'
  --exclude='GrokLab/deploy/qemu-vms/**'
  --exclude='_archive'
  --exclude='_archive/**'
...
stage_tree() {
  ...
  [[ -f "${ROOT}/Hostess7/RELEASE-${HOSTESS7_VERSION}.md" ]] && \
    cp "${ROOT}/Hostess7/RELEASE-${HOSTESS7_VERSION}.md" "${STAGE}/RELEASE.md"
  printf '%s\n' "$HOSTESS7_VERSION" > "${STAGE}/VERSION"
...
publish_release() {
  local notes="${ROOT}/Hostess7/RELEASE-${HOSTESS7_VERSION}.md"
  [[ -f "$notes" ]] || notes="${STAGE}/RELEASE.md"
  if [[ "$DRY" -eq 1 ]]; then
    log "dry-run: would gh release ${TAG} on ${REPO_SLUG}"
    return 0
  fi
  if gh release view "$TAG" --repo "$REPO_SLUG" >/dev/null 2>&1; then
    gh release edit "$TAG" --repo "$REPO_SLUG" --title "Hostess7 ${HOSTESS7_VERSION}" --notes-file "$notes"
  else
    gh release create "$TAG" \
      --repo "$REPO_SLUG" \
      --target main \
      --title "Hostess7 ${HOSTESS7_VERSION}" \
      --notes-file "$notes"
  fi
...
if [[ -f "${ROOT}/scripts/seal-built-executables.sh" ]]; then
  log "seal release executables in stage (biology/training/cache exempt)"
  NEXUS_INSTALL_ROOT="$STAGE" NEXUS_STATE_DIR="${STAGE}/.nexus-state" \
    NEXUS_SEAL_EXECUTABLES_IMMUTABLE=0 \
