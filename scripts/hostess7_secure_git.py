...
def git_run(cwd: Path, args: list[str], *, remote: str | None = None) -> dict[str, Any]:
    ...
    if remote:
        ...
        has_origin = _run_git(cwd, ["remote", "get-url", "origin"], ssh_cmd=ssh_cmd, env=env)
        remote_args = (
            ["remote", "add", "origin", chk["remote"]]
            if has_origin.returncode != 0
            else ["remote", "set-url", "origin", chk["remote"]]
        )
        _run_git(cwd, remote_args, ssh_cmd=ssh_cmd, env=env)
