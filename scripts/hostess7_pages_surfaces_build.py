...
# C2 quint minus Start — exactly four desktop icons on GitHub Pages (not full pinned grid).
...
def _patch_desktop_doc(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    for pool in (programs, doc.get("field_apps") or [], doc.get("startbar", {}).get("quick") or []):
        for app in pool:
            if isinstance(app, dict) and app.get("id"):
                by_id[str(app["id"])] = app
    if qb:
        by_id["queen-browser"] = qb
    pages_icons: list[dict[str, Any]] = []
    for iid in PAGES_DESKTOP_ICON_IDS:
        src = by_id.get(iid)
        if not src:
            continue
        row = json.loads(json.dumps(src))
        row["pinned"] = True
        row["launcher_visible"] = True
        row["desktop"] = True
        for field in ("exec", "url", "launch_url"):
            if field in row and row[field]:
                row[field] = _pages_url(str(row[field]))
        pages_icons.append(row)
    doc["desktop_icons"] = pages_icons
    doc["program_count"] = len(pages_icons)
...
def _export_apis(desktop: dict[str, Any]) -> list[str]:
    ...
    shell_settings = {
        ...
        "settings": {
            ...
            "ammo_ui_boost_note": f"Hostess 7 {H7_VERSION} · 4 desktop icons · {PAGES_DESKTOP_THEME}",
        },
