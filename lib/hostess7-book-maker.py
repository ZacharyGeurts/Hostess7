...
def main() -> int:
    ...
    parser.add_argument("--book-kind", default="authored")
    parser.add_argument("--dewey-label", default="")
    ...
    if cmd == "pack":
        ...
        if args.body_file:
            ...
            if not bf.is_file():
                bf = INSTALL / args.body_file
            body = bf.read_text(encoding="utf-8") if bf.is_file() else ""
        if not body:
            body = "Draft body — replace via book maker panel."
        if not title:
