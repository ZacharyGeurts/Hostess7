...
#!/bin/bash
# Self-defense — verify signed protective shell before loading modules.
# Training, brain expansion, and Hostess 7 biology paths are never in this manifest.
...
nexus_manifest_emit_path() {
  local path="$1"
  [[ -n "$path" && -f "$path" ]] || return 0
  declare -f nexus_path_is_biology_mutable >/dev/null 2>&1 && nexus_path_is_biology_mutable "$path" && return 0
  printf '%s\n' "$path"
}
...
nexus_sign_manifest() {
  ...
  while IFS= read -r path; do
    [[ -f "$path" ]] || continue
    hash="$(sha256sum "$path" | /usr/bin/awk '{print $1}')"
    printf '%s  %s\n' "$hash" "$path" >>"$out"
...
nexus_verify_integrity() {
  ...
  while read -r hash path; do
    ...
    declare -f nexus_path_is_biology_mutable >/dev/null 2>&1 && nexus_path_is_biology_mutable "$resolved" && continue
    [[ -f "$resolved" ]] || {
      nexus_log "ALERT" "self-defense" "INTEGRITY_MISSING path=${path}"
      fail=1
      continue
    }
    current="$(sha256sum "$resolved" | /usr/bin/awk '{print $1}')"
