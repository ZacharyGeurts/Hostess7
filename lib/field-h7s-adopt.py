...
def can_adopt_speedup(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if h7s.is_h7s_blob(blob):
            return {"ok": False, "reason": "already_h7s", "path": str(path), "already_disguised": True}
        h7_fmt = INSTALL / "lib" / "field-h7-format.py"
        if h7_fmt.is_file():
            spec = importlib.util.spec_from_file_location("field_h7_adopt_guard", h7_fmt)
            if spec and spec.loader:
                h7mod = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(h7mod)
                if hasattr(h7mod, "classify_hostess7_blob"):
                    cls = h7mod.classify_hostess7_blob(blob)
                    if cls.get("is_container"):
                        fmt = str(cls.get("format") or "hostess7")
                        return {
                            "ok": False,
                            "reason": f"already_{fmt.replace('/', '_')}",
                            "path": str(path),
                            "true_format": fmt,
                            "already_disguised": True,
                            "statement": "Hostess7 container — never double-wrap; unwrap first if cross-pack needed.",
                        }
    except OSError as exc:
