...
Learning objectives
 ...
 Identify integration via data_bus[64] , hardwareFabric , and jsonl.
 ...
 Do not treat telemetry as a spatial grid. data_bus is a dashboard vector, not a second fabric. Category errors here produce dashboards that lie about dimensionality.

 Phi as potential — scalar posture

 Phi carries wave and gate potential — electrical metaphor on the fabric. Discrete Laplacian steps in CANVAS.comp treat Phi as a scalar field evolving with WaveSpeed and propalacticScale forcing. Implemented at binding 8. Electrical metaphor — not a claim your GPU measures volts on the PCIe slot.
 ...
 NEXUS turns sockets into sentences: process path, port habit, TX vs RX, corroboration before permanent action. Defense begins when flows become readable positions in operator space. Implemented in NEXUS. Not AMOURANTHRTX Vulkan.
 ...
 Fabric averages → hardwareFabric (Chapter 10)

 Die telemetry → data_bus slots HUD reads (Chapter 8)

 Packet rows → jsonl panel archives (Chapter 5)
 ...
 Figure 2.1 — Phi, Thermo, Flow: three channels, one fabric. Packet field and die are parallel scales, not substitutes.

 Category errors — learn these early

 Do not treat shader visual as instrumentation. Do not treat JSON verdict as joules. Do not treat guest RAM metaphor as cloud perimeter. 3D state ≠ 3D hype.
...
curl -sk https://127.0.0.1:9477/threat-panel.json | head -c 2000
 ...
 Implemented. Sub-micron SEM marketing is procedural pixel detail — not lab imaging.
 ...
 Navigate Options::Canvas::SwipeList as curriculum, not product reset.

 Run energy.comp lab drills and label Classic versus X86Fields.

 Map guest FAT and VGA subfields inside Field Die addressing.
 ...
 Figure 2.1 — Phi, Thermo, Flow on fabric; die bytes and packet sentences beside — three scales, one literacy.
 ...
 Category error to avoid: Declaring Field Die absent because you swiped to energy . Die path returns on next default launch; infrastructure persisted.

 Teaching pattern from Chapter 7: week one afternoon on Classic thermo, week two operations on die default. Veterans live on die; newcomers need visible heatmaps — both are honest if labeled.
...
grep -E 'THERMO|entropy|Boundary' run.log | tail -20
 ...
 Plain English: Die is a dollhouse with rooms — kitchen RAM, parlor VGA, attic FAT — not a single undifferentiated blob.
 ...
 Panel poll: loopback :9477, short dwell, browser or curl path — usually USER_OK after habit learns.

 DNS truth query: port 53 to 127.0.0.1 — corroboration for navigation (Chapter 20).
 ...
 Implemented in NEXUS gatekeeper + jsonl schema habits. Not written into fabric texels by default.
 ...
 Exercise 2.B — paper integration (30 min)
 ...
 vkCmdDispatch → jsonl append (NO)

 Layer pump → data_bus slots (YES — host ritual)

 ThermoAccountant → data_bus[24] (YES — mirror)

 Mouse inject → probe position in FieldSocket (YES — offense)
 ...
 Which swipe uses energy.comp and what lab knob pair proves coupling?

 How is RetroRTX categorized versus default x86?

 What lives at VGA 0xB8000 versus FAT layer pump?

 Define connection intent with three example flows.
