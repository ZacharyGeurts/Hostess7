...
(define (script-fu-spinning-globe inImage
  ...
  (let* (
        (theImage (if (= inCopy TRUE)
                      (car (ammoos-image-duplicate inImage))
                      inImage))
        (theLayer (vector-ref (car (ammoos-image-get-selected-layers theImage)) 0))
        (n 0)
        (ang (* (/ 360 inFrames)
  ...
  (ammoos-layer-add-alpha theLayer)

  (while (> inFrames n)
    (set! n (+ n 1))
    (set! theFrame (car (ammoos-layer-copy theLayer)))
    (ammoos-image-insert-layer theImage theFrame 0 0)
    (ammoos-item-set-name theFrame
                         (string-append "Anim Frame: "
                                        (number->string (- inFrames n) 10)
                                        " (replace)"))
    ;(ammoos-message "Now call map-object")
  ...
  (ammoos-image-autocrop theImage theFrame)

  (if (= inIndex 0)
      ()
      (ammoos-image-convert-indexed theImage CONVERT-DITHER-FS CONVERT-PALETTE-GENERATE inIndex
                                  FALSE FALSE ""))

  (if (= inCopy TRUE)
    (begin
      (ammoos-image-clean-all theImage)
