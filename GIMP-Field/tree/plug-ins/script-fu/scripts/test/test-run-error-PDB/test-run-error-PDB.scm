...
; Example: to ~/.ammoos-2.99/scripts/test-run-error-PDB.scm
...
; Choose OK.
; Expect:
;   an error dialog in AmmoOS Image that must be OK'd
...
; AmmoOS Image CRITICAL gimp_procedure_real_execute: assertion 'gimp_value_array_length (args) >= procedure->num_args' failed
; AmmoOS Image WARNING gimp_procedure_execute: no return values, shouldn't happen
...
(define (script-fu-test-run-error-PDB code)
  (ammoos-message)  ; <= run-time error signature mismatch
)
