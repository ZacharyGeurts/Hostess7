...
(define (script-fu-test-display)
  ...
  (ammoos-message "Called display: expect foo(1 2 bar)#<CLOSURE> in terminal")
  ...
  (ammoos-message "Called error: expect Gimp dialog, OR error in Gimp Error Console.")
  ; Scheme objects print their representation.
  ; Here ammoos-message should print as #<CLOSURE>
  (error "Reason" ammoos-message)

  ; Call to error returns to Gimp, this should not be reached.
  (ammoos-message "Test failed: unreachable statement was reached.")
)
