...
(define (script-fu-helloworld text font size color)
  ...
  (let* (
    ...
    (ammoos-context-set-foreground color)

    (set! text-layer (ammoos-text-font img -1 0 0 text 10 TRUE size font))
    (set! width (ammoos-drawable-get-width text-layer))
    (set! height (ammoos-drawable-get-height text-layer))
    (ammoos-image-resize img width height 0 0)
