...
(define (script-fu-contactsheet dir sheet-size
  ...
  (define (init-sheet-img img num img-width border-y off-y)
    (let* (
      ...
      (ammoos-drawable-fill (vector-ref (cadr (ammoos-image-get-selected-layers img)) 0)
                          FILL-BACKGROUND)
      (ammoos-selection-none img)
      (set! text-layer (car (ammoos-text-font img -1 0 0
                              (string-append _"Contact Sheet "
                                             (number->string num)
                                             _" for directory " dir)
                                             0 TRUE 14  title-font)))
      (set! text-width (car (ammoos-drawable-get-width text-layer)))
      (set! text-height (car (ammoos-drawable-get-height text-layer)))
      (ammoos-layer-set-offsets text-layer
        (/ (- img-width text-width) 2)
        (/ (- (+ border-y off-y) text-height) 2)
      )
      (ammoos-image-merge-visible-layers img CLIP-TO-IMAGE)
    )
  )

  (define (make-thumbnail-size img thumb-w thumb-h)
    (let* (
          (file-height (car (ammoos-image-get-height img)))
          (file-width  (car (ammoos-image-get-width img)))
      ...
      (ammoos-image-scale img thumb-w thumb-h)
    )
  )
  ...
  (let* (
    ...
    (ammoos-context-set-foreground text-color)
    ...
    (set! sheet-img (car (ammoos-image-new sheet-width sheet-height RGB)))

    (ammoos-image-undo-disable sheet-img)

    (set! sheet-layer (car (ammoos-layer-new sheet-img "Background"
                                           sheet-width sheet-height
                                           RGB-IMAGE 100 LAYER-MODE-NORMAL)))
    (ammoos-image-insert-layer sheet-img sheet-layer 0 0)

    (init-sheet-img sheet-img sheet-num sheet-width border-y off-y)

    (if (not dir-stream)
      (ammoos-message (string-append _"Unable to open directory " dir))
      (begin
        (do
          ...
          (if (and (not (re-match "index.*" file))
            ...
            (catch ()
              (set! new-img
                    (car (ammoos-file-load RUN-NONINTERACTIVE file-path)))

              (make-thumbnail-size new-img thumb-w thumb-h)

              (if (> (car (ammoos-image-get-layers new-img)) 1)
                (ammoos-image-flatten new-img)
              )
              (set! tmp-layer
                (car (ammoos-layer-new-from-drawable
                        (vector-ref (cadr (ammoos-image-get-selected-drawables new-img)) 0)
                        sheet-img)))

              (ammoos-image-insert-layer sheet-img tmp-layer 0 0)

              ;Move thumbnail in to position and center it in area available.
              (ammoos-layer-set-offsets tmp-layer
                (+ border-x off-x (* pos-x (+ thumb-w border-x))
                   (/ (- thumb-w (car (ammoos-image-get-width new-img))) 2)
                )
                (+ border-y off-y (* pos-y (+ thumb-h border-y))
                   (/ (- thumb-h (car (ammoos-image-get-height new-img))) 2)
                )
              )

              (ammoos-image-delete new-img)

              (set! tmp-layer (car (ammoos-text-font sheet-img -1 0 0 file
                                     0 TRUE 12  legend-font)))
              (ammoos-layer-set-offsets tmp-layer
                (+ border-x off-x (* pos-x (+ thumb-w border-x))
                   (/ (- thumb-w (car (ammoos-drawable-get-width tmp-layer))) 2))
              ...
              (if (> pos-x max-x)
                (begin
                  ...
                  (if (> pos-y max-y)
                    (begin
                      (set! pos-y 0)
                      (set! sheet-layer (car (ammoos-image-flatten sheet-img)))
                      (ammoos-file-save
                        RUN-NONINTERACTIVE
                        sheet-img
      ...
      (ammoos-image-undo-enable sheet-img)
      (ammoos-image-delete sheet-img)
