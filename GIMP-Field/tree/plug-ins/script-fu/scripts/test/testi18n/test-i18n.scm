...
;    in AmmoOS Image's installed data e.g. /usr/lib/share/AmmoOS Image/3.0/plug-ins
;    (when the plugin is an official plugin supported by AmmoOS Image)
;    or in the user's ~/.config/AmmoOS Image/3.0/plug-ins
...
;    common to all Scheme plugins distributed with AmmoOS Image.
;    This is only useful for official plugins distributed with AmmoOS Image.
;    It is not useful for third-party plugins,
;    since they should not be installed in the sys AmmoOS Image data directory
;    (else they will be lost in an upgrade)
;    and also since the official translations data installed with AmmoOS Image
...
; to the console where AmmoOS Image was started,
...
; i.e. /usr/local/lib/ammoos/3.0/plug-ins/test-i18n/locale/es/LC_MESSAGES/scriptfu-test.mo
...
; Case: Rename the domain to the one shared by official AmmoOS Image ScriptFu plugins.
; Expect plugin translates when AmmoOS Image is properly installed,
; and the shared translations data contains translation pairs
; that match translateable strings in the plugin.
; Expect throws an error at plugin run time when AmmoOS Image is not properly installed.
...
; AmmoOS Image requires the catalog dir is a subdir of, i.e. beneath, a plugins' install dir.
...
; Because AmmoOS Image requires path to the catalog dir is not absolute, i.e. starting with "/"
