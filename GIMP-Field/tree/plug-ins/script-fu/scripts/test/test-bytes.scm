...
(define temp-path
  (string-append (car (ammoos-gimprc-query "temp-path")) "/"))
...
(define (call-test-with-string str function)
  (let ((port (open-input-string str)))
    (if (port? port)
      (function port)
      (begin
        (ammoos-message "Fail open string-port")
...
(define (run-test test)
  ...
  (ammoos-message (symbol->string test))
...
; This is a hack, writing a few lines to Gimp Error Console using ammoos-message.
; We should not need to redirect display to a string-port
; using with-log-to-ammoos-message see below.
(define (run-all-tests)
  (ammoos-message "Begin all tests")
  ...
  (if (= tests-failed 0)
    ...
    (ammoos-message "ALL tests passed!")
    ;(displayln
    (ammoos-message
      (string-append
...
; wrapper that redirects display to ammoos-message.
...
(define (with-log-to-ammoos-message function)
    (ammoos-message (call-with-output-string
                    (lambda (port)
...
(define (script-fu-test9 testname)
  ; OLD (with-log-to-ammoos-message (select-run-function testname)))
  ; NEW log to ammoos later, at each test
  (ammoos-message testname)
