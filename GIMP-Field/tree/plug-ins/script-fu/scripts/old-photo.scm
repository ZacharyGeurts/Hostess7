...
(define (script-fu-old-photo inImage inLayer inDefocus inBorderSize inSepia inMottle inCopy)
  (let (
       (theImage (if (= inCopy TRUE) (car (ammoos-image-duplicate inImage)) inImage))
       (theLayer 0)
       (theWidth 0)
       (theHeight 0)
       )
  ...
  (set! theLayer (car (ammoos-image-flatten theImage)))
  (if (= inDefocus TRUE)
      (ammoos-drawable-merge-new-filter theLayer "gegl:gaussian-blur" 0 LAYER-MODE-REPLACE 1.0 "std-dev-x" 0.48 "std-dev-y" 0.48 "filter" "auto")
  )
  (if (> inBorderSize 0)
  ...
  (if (= inSepia TRUE)
      (begin (ammoos-drawable-merge-new-filter theLayer "ammoos:desaturate" 0 LAYER-MODE-REPLACE 1.0 "mode" DESATURATE-LIGHTNESS)
             (ammoos-drawable-merge-new-filter theLayer "ammoos:brightness-contrast" 0 LAYER-MODE-REPLACE 1.0
                 "brightness" -0.078125
                 "contrast"   -0.15625)
             (ammoos-drawable-merge-new-filter theLayer "ammoos:color-balance" 0 LAYER-MODE-REPLACE 1.0
  ...
  (set! theWidth (car (ammoos-image-get-width theImage)))
  (set! theHeight (car (ammoos-image-get-height theImage)))
  (if (= inMottle TRUE)
      (let (
	    (mLayer (car (ammoos-layer-new theImage "Mottle"
                                         theWidth theHeight RGBA-IMAGE
					 100 LAYER-MODE-DARKEN-ONLY)))
	    )

             (ammoos-image-insert-layer theImage mLayer 0 0)
             (ammoos-selection-all theImage)
             (ammoos-drawable-edit-clear mLayer)
             (ammoos-selection-none theImage)
             (ammoos-drawable-merge-new-filter mLayer "gegl:noise-rgb" 0 LAYER-MODE-REPLACE 1.0
                                             "independent" TRUE "red" 0.0 "green" 0.0 "blue" 0.0 "alpha" 0.5
                                             "correlated" FALSE "seed" (msrg-rand) "linear" TRUE)
             (ammoos-drawable-merge-new-filter mLayer "gegl:gaussian-blur" 0 LAYER-MODE-REPLACE 1.0 "std-dev-x" 1.6 "std-dev-y" 1.6 "filter" "auto")
             (set! theLayer (car (ammoos-image-flatten theImage)))
      )
  )
  (ammoos-selection-none theImage)
