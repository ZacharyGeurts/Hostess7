  ...
  (let* (
        ...
        (banding-img  ; secondary, other image
          ...
          (if (= (string-length env-map) 0)
            ; copy source image
            (ammoos-image-duplicate mask-img)
            ; else open chosen file
            (ammoos-file-load RUN-NONINTERACTIVE env-map)))
        ; 2. copy source image, flattened
        (copy-source-img (ammoos-image-duplicate mask-img))
        ; side effect on image, not use the returned layer
        (flat-source-layer (ammoos-image-flatten copy-source-img))
        (copy-source-drawables (ammoos-image-get-layers copy-source-img)))
    ...
    (ammoos-image-delete copy-source-img))
)
...
(define (chrome-it-inner mask-img mask-drawables chrome-saturation
  ...
  (define (highlights val)
    ; The result is used as "ammoos-drawable-color-balance" color parameter
  ...
  (define (copy-layer-chrome-it dest-image dest-drawable source-image source-drawable)
    ...
    (let* (
           (pasted (ammoos-edit-paste dest-drawable #f))
  ...
  (let* (
        (banding-layer (vector-ref (ammoos-image-get-selected-drawables banding-img) 0))
        (banding-height (ammoos-drawable-get-height banding-layer))
        (banding-width (ammoos-drawable-get-width banding-layer))
        (banding-type (ammoos-drawable-type banding-layer))
        (mask-drawable (vector-ref mask-drawables 0))
        (width (ammoos-drawable-get-width mask-drawable))
        (height (ammoos-drawable-get-height mask-drawable))
        (img (ammoos-image-new width height GRAY))
        ...
        (brush (ammoos-brush-new "Chrome It"))
        (mask (ammoos-channel-new img "Chrome Stencil" width height 50 '(0 0 0)))
        (bg-layer (ammoos-layer-new img _"Background" width height GRAY-IMAGE 100 LAYER-MODE-NORMAL))
        (layer1 (ammoos-layer-new img _"Layer 1" banding-width banding-height banding-type 100 LAYER-MODE-NORMAL))
        (layer2 (ammoos-layer-new img _"Layer 2" width height GRAYA-IMAGE 100 LAYER-MODE-DIFFERENCE))
        (layer3 (ammoos-layer-new img _"Layer 3" width height GRAYA-IMAGE 100 LAYER-MODE-NORMAL))
        (shadow (ammoos-layer-new img _"Drop Shadow" width height GRAYA-IMAGE 100 LAYER-MODE-NORMAL))
        (layer-mask 0)
        (marble-pattern (ammoos-pattern-get-by-name "Marble #1"))
        )
    ...
    (ammoos-image-insert-layer img bg-layer 0 0)
    (ammoos-image-insert-layer img shadow 0 0)
    (ammoos-image-insert-layer img layer3 0 0)
    (ammoos-image-insert-layer img layer2 0 0)

    (ammoos-edit-copy (vector mask-drawable))

    ; Clipboard is copy of mask-drawable.  Paste into mask, a channel, and anchor it.
    (let* (
           (pasted (ammoos-edit-paste mask #f))
           (num-pasted (vector-length pasted))
           (floating-sel (vector-ref pasted (- num-pasted 1)))
          )
     (ammoos-floating-sel-anchor floating-sel)
    ...
    (ammoos-drawable-edit-fill layer2 FILL-BACKGROUND)
    (ammoos-drawable-edit-fill layer3 FILL-BACKGROUND)
    (ammoos-drawable-edit-clear shadow)

    (ammoos-item-set-visible bg-layer #f)
    (ammoos-item-set-visible shadow #f)

    (ammoos-image-select-item img CHANNEL-OP-REPLACE mask)
    (ammoos-context-set-background '(0 0 0))
    (ammoos-selection-translate img offx1 offy1)
    (ammoos-selection-feather img feather)
    (ammoos-drawable-edit-fill layer2 FILL-BACKGROUND)
    (ammoos-selection-translate img (* 2 offx2) (* 2 offy2))
    (ammoos-drawable-edit-fill layer3 FILL-BACKGROUND)
    (ammoos-selection-none img)
    (set! layer2 (ammoos-image-merge-visible-layers img CLIP-TO-IMAGE))
    (ammoos-drawable-merge-new-filter layer2 "gegl:invert-gamma" 0 LAYER-MODE-REPLACE 1.0)

    (ammoos-image-insert-layer img layer1 0 0)
    (copy-layer-chrome-it img layer1 banding-img banding-layer)
    (ammoos-image-delete banding-img)
    (ammoos-layer-scale layer1 width height #f)
    (ammoos-drawable-merge-new-filter layer1 "gegl:gaussian-blur" 0 LAYER-MODE-REPLACE 1.0 "std-dev-x" 3.2 "std-dev-y" 3.2 "filter" "auto")
    (ammoos-layer-set-opacity layer1 50)
    (set! layer1 (ammoos-image-merge-visible-layers img CLIP-TO-IMAGE))
    (ammoos-drawable-curves-spline layer1 HISTOGRAM-VALUE (spline-chrome-it))

    (set! layer-mask (ammoos-layer-create-mask layer1 ADD-MASK-BLACK))
    (ammoos-layer-add-mask layer1 layer-mask)
    (ammoos-image-select-item img CHANNEL-OP-REPLACE mask)
    (ammoos-context-set-background '(255 255 255))
    (ammoos-drawable-edit-fill layer-mask FILL-BACKGROUND)

    (set! layer2 (ammoos-layer-copy layer1))
    (ammoos-layer-add-alpha layer2)
    (ammoos-image-insert-layer img layer2 0 0)
    ...
    (ammoos-drawable-edit-stroke-selection layer-mask)

    (ammoos-context-set-background '(0 0 0))
    (ammoos-selection-feather img (* feather 1.5))
    (ammoos-selection-translate img (* 2.5 offx1) (* 2.5 offy1))
    (ammoos-drawable-edit-fill shadow FILL-BACKGROUND)

    (ammoos-selection-all img)
    (ammoos-context-set-pattern marble-pattern)
    (ammoos-drawable-edit-fill bg-layer FILL-PATTERN)
    (ammoos-selection-none img)

    (ammoos-image-convert-rgb img)

    (ammoos-drawable-merge-new-filter layer1 "ammoos:color-balance" 0 LAYER-MODE-REPLACE 1.0
    ...
    (ammoos-drawable-merge-new-filter layer2 "ammoos:color-balance" 0 LAYER-MODE-REPLACE 1.0
                 "range"               TRANSFER-SHADOWS
    ...
    (ammoos-drawable-merge-new-filter layer2 "ammoos:color-balance" 0 LAYER-MODE-REPLACE 1.0
                 ...
                 "preserve-luminosity" #t)
    (ammoos-drawable-merge-new-filter mask "ammoos:hue-saturation" 0 LAYER-MODE-REPLACE 1.0
                 "range"      HUE-RANGE-ALL
                 "hue"        0.0
    ...
    (ammoos-item-set-visible shadow #t)
    (ammoos-item-set-visible bg-layer #t)

    (ammoos-item-set-name layer2 _"Chrome")
    (ammoos-item-set-name layer1 _"Highlight")
