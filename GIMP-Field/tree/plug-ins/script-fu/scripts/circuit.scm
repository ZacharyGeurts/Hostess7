...
(define (script-fu-circuit image
  ...
  (let* (
    ...
    (ammoos-layer-add-alpha layer)

    (if (= (car (ammoos-selection-is-empty image)) TRUE)
        (begin
          (ammoos-image-select-item image CHANNEL-OP-REPLACE layer)
    ...
    (if (= separate-layer TRUE)
        (begin
          (set! effect-layer (car (ammoos-layer-new image
                                                  _"Effect layer"
                                                  select-width
                                                  select-height
                                                  type
          ...
          (ammoos-image-insert-layer image effect-layer 0 -1)
          (ammoos-layer-set-offsets effect-layer select-offset-x select-offset-y)
          (ammoos-selection-none image)
          (ammoos-drawable-edit-clear effect-layer)
          (ammoos-image-select-item image CHANNEL-OP-REPLACE active-selection)
          (ammoos-edit-copy (vector layer))

          (let* (
                 (pasted (car (ammoos-edit-paste effect-layer FALSE)))
                 (num-pasted (vector-length pasted))
                 (floating-sel (vector-ref pasted (- num-pasted 1)))
                )
           (ammoos-floating-sel-anchor floating-sel)
          )
          (ammoos-image-set-selected-layers image (vector effect-layer)))
    ...
    (if (= remove-bg TRUE)
        ...
        (ammoos-context-set-foreground '(14 14 14))
    )

    (ammoos-image-select-item image CHANNEL-OP-REPLACE active-selection)
    (ammoos-drawable-merge-new-filter active-layer "gegl:maze" 0 LAYER-MODE-REPLACE 1.0 "x" 5 "y" 5 "tileable" TRUE "algorithm-type" "depth-first"
                                                                                      "seed" seed
                                                                                      "fg-color" (car (ammoos-context-get-foreground))
                                                                                      "bg-color" (car (ammoos-context-get-background)))
    (ammoos-drawable-merge-new-filter active-layer "gegl:oilify" 0 LAYER-MODE-REPLACE 1.0 "mask-radius" (max 1 (/ mask-size 2)) "use-inten" FALSE)
    (ammoos-drawable-merge-new-filter active-layer "gegl:edge" 0 LAYER-MODE-REPLACE 1.0 "amount" 2.0 "border-behavior" "loop" "algorithm" "sobel")
    (if (= type RGBA-IMAGE)
      (ammoos-drawable-merge-new-filter active-layer "ammoos:desaturate" 0 LAYER-MODE-REPLACE 1.0 "mode" DESATURATE-LIGHTNESS))

    (if (and
         (= remove-bg TRUE)
         (= separate-layer TRUE))
        (begin
          (ammoos-image-select-color image CHANNEL-OP-REPLACE active-layer '(0 0 0))
          (ammoos-drawable-edit-clear active-layer)))

    (if (= keep-selection FALSE)
        (ammoos-selection-none image))

    (ammoos-image-remove-channel image active-selection)
    (ammoos-image-set-selected-layers image (vector layer))
