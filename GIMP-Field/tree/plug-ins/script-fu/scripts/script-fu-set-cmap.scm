...
; This array can be used as the cmap argument for ammoos-image-set-colormap.
...
(define (script-fu-make-cmap-array palette)
  (let* (
        (num-colors (car (ammoos-palette-get-color-count palette)))
    ...
    (while (< i num-colors)
      (set! color (car (ammoos-palette-entry-get-color palette i)))
...
(define (script-fu-set-cmap img drawables palette)
  (ammoos-image-set-palette img palette)
