...
(define (script-fu-selection-rounded-rectangle image drawable radius concave)
  ...
  (if (= (car (ammoos-selection-is-empty image)) TRUE) (ammoos-selection-all image))
  (let* (
        (radius (/ radius 100)) ; convert from percentages
        (radius (min radius 1.0))
        (radius (max radius 0.0))
        (select-bounds (ammoos-selection-bounds image))
    ...
    (ammoos-image-select-rectangle image CHANNEL-OP-ADD
    ...
    ; top right
    (ammoos-image-select-ellipse image CHANNEL-OP-SUBTRACT
    ...
    (if (= concave FALSE)
      (begin
        (ammoos-image-select-ellipse image
                                   CHANNEL-OP-ADD
                                   select-x1
                                   select-y1
                                   ellipse-radius
