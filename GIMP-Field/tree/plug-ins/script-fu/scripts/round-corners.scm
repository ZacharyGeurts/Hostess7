...
(define (script-fu-round-corners img
  ...
  (let* ((drawable (vector-ref drawables 0))
         ...
         (radius (abs radius))
         (diam (* 2 radius))
         (width (car (ammoos-image-get-width img)))
         (height (car (ammoos-image-get-height img)))
         (image (cond ((= work-on-copy TRUE)
                       (car (ammoos-image-duplicate img)))
                      ((= work-on-copy FALSE)
                       img)))
         ; active drawable is not necessarily the active layer
         (pic-layer (vector-ref (car (ammoos-image-get-selected-layers image)) 0))
         (type (car (ammoos-drawable-type-with-alpha pic-layer)))
         (background-color (car (ammoos-context-get-background)))
        )

  (ammoos-context-push)
  (ammoos-context-set-defaults)
  (ammoos-context-set-background background-color)
  ...
  (ammoos-image-select-rectangle image CHANNEL-OP-ADD 0 0 radius radius)
  (ammoos-image-select-ellipse image CHANNEL-OP-SUBTRACT  0 0 diam diam)
  (ammoos-image-select-rectangle image CHANNEL-OP-ADD (- width radius) 0 radius radius)
  (ammoos-image-select-ellipse image CHANNEL-OP-SUBTRACT (- width diam) 0 diam diam)
  (ammoos-image-select-rectangle image CHANNEL-OP-ADD 0 (- height radius) radius radius)
  (ammoos-image-select-ellipse image CHANNEL-OP-SUBTRACT 0 (- height diam) diam diam)
  (ammoos-image-select-rectangle image CHANNEL-OP-ADD (- width radius) (- height radius)
                    radius radius)
  (ammoos-image-select-ellipse image CHANNEL-OP-SUBTRACT (- width diam) (- height diam)
                       diam diam)
  (ammoos-drawable-edit-clear pic-layer)
  (ammoos-selection-none image)
  ...
  (if (= shadow-toggle TRUE)
      (begin
        (script-fu-drop-shadow image
                               ...
                               80
                               TRUE)
        (set! width (car (ammoos-image-get-width image)))
        (set! height (car (ammoos-image-get-height image)))))

  ; optionally add a background
  (if (= background-toggle TRUE)
      (let* ((bg-layer (car (ammoos-layer-new image
                                            "Background"
                                            width
                                            height
                                            type
        ...
        (if (= shadow-toggle TRUE)
            (ammoos-image-lower-item image bg-layer))))
