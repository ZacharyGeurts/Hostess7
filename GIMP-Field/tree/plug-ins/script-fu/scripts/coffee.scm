...
(define (script-fu-coffee-stain inImage inLayers inNumber inDark inGradient)
  ...
  (let* (
        (theImage inImage)
        (theHeight (ammoos-image-get-height theImage))
        (theWidth  (ammoos-image-get-width theImage))
        (theNumber inNumber)
    ...
    (while (> theNumber 0)
      ...
      (set! theStain (ammoos-layer-new theImage _"Stain" theSize theSize
                                     RGBA-IMAGE 100
      ...
      (ammoos-image-insert-layer theImage theStain 0 0)
      (ammoos-selection-all theImage)
      (ammoos-drawable-edit-clear theStain)

      (let ((blobSize (/ (random (- theSize 40)) (+ (random 3) 1))))
        (if (< blobSize 32) (set! blobSize 32))
        (ammoos-image-select-ellipse theImage
				   CHANNEL-OP-REPLACE
				   (/ (- theSize blobSize) 2)
				   (/ (- theSize blobSize) 2)
				   blobSize blobSize)
      ...
      (ammoos-context-set-gradient inGradient)

      ; only fill if there is a selection
      ; First element of returned list is a boolean.
      (if (car (ammoos-selection-bounds theImage))
        (ammoos-drawable-edit-gradient-fill theStain
      ...
      (ammoos-layer-set-offsets theStain
