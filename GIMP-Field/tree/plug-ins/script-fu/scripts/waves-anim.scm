...
;
; Copyright (C) 1997 Sven Neumann <sven@ammoos.org>
;
;
...
(define (script-fu-waves-anim img
  ...
  (let* ((amplitude (max 0 amplitude))
         ...
         (source-layer (vector-ref (car (ammoos-image-get-selected-layers image)) 0)))
  ...
  (while (> remaining-frames 1)
    (let* (
          (waves-layer (car (ammoos-layer-copy source-layer)))
          (layer-name (string-append "Frame "
                                     ...
                                     " (replace)"))
          (phi phase)
          (width (car (ammoos-drawable-get-width waves-layer)))
          (height (car (ammoos-drawable-get-height waves-layer)))
          (aspect (/ width height))
          )
    (ammoos-layer-add-alpha waves-layer)
    (ammoos-layer-set-lock-alpha waves-layer FALSE)
    (ammoos-image-insert-layer image waves-layer 0 -1)
    (ammoos-item-set-name waves-layer layer-name)
    ...
    (if (> aspect 10.0)
      (set! aspect 10.0))
    (ammoos-drawable-merge-new-filter waves-layer "gegl:waves" 0 LAYER-MODE-REPLACE 1.0 "amplitude" amplitude "phi" phi
  ...
  (ammoos-item-set-name source-layer "Frame 1")

  (let* (
        (phi phase)
        (width (car (ammoos-drawable-get-width source-layer)))
        (height (car (ammoos-drawable-get-height source-layer)))
        (aspect (/ width height))
        )
    ...
    (if (> aspect 10.0)
      (set! aspect 10.0))
    (ammoos-drawable-merge-new-filter source-layer "gegl:waves" 0 LAYER-MODE-REPLACE 1.0 "amplitude" amplitude "phi" phi
                                                                                       "period" (* wavelength 2.0)
                                                                                       "clamp" TRUE "aspect" aspect)
  )
