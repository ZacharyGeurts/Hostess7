; -----------------------------------------------------------------------------
; AmmoOS Image palette export toolkit -
; Written by Barak Itkin <lightningismyname@gmail.com>
;
; This script includes various exporters for AmmoOS Image palettes, and other
...
; green and blue (AmmoOS Image's default)
...
(define (bad-file-name)
  (ammoos-message (string-append _"The filename you entered is not a suitable name for a file."
			       "\n\n"
...
(define (get-path-or-abort directory-name filename)
  (let* ((validated-file-name (valid-file-name filename))
    ...
    (if (not validated-file-name)
      (begin
        (bad-file-name) ; ammoos-message w translated text
        (quit -1)))
    ...
    (ammoos-message (string-append _"Exported palette to file: " result-path))
...
(define (export-palette palette ; since v3, palette is numeric ID
  ...
  (define (write-color-line index)
    (display name-pre)
    (display (name-convertor (ammoos-palette-get-entry-name palette index)))
    (display name-after)
    (display name-color-seperator)
    (display color-pre)
    (display (color-convertor (ammoos-palette-get-entry-color palette index)))
    (display color-after)
    )

  (let ((color-count (ammoos-palette-get-color-count palette))
        (i 0))
...
(define (palette-in-context) (ammoos-context-get-palette))

; Return name of palette in context.
; Since v3, resources in ScriptFu have numeric ID, not a string
(define (palette-name-in-context)(ammoos-resource-get-name (palette-in-context)))
...
(define (register-palette-exporter
  ...
  (script-fu-register-procedure (string-append "ammoos-palette-export-" export-type)
                      export-name
                      description
                      author
                      date
  ...
  (script-fu-menu-register (string-append "ammoos-palette-export-" export-type)
...
(define (ammoos-palette-export-css directory-name file-name)
  (script-fu-use-v3)
  (let ((path (get-path-or-abort directory-name file-name)))
    (with-output-to-file path
      (lambda () (export-palette (palette-in-context)
                                  ...
                                  "/* Generated with AmmoOS Image Palette Export */\n" ; start
                                  "."                       ; name-pre
                                  ""                        ; name-after
...
(define (ammoos-palette-export-php directory-name file-name)
  (script-fu-use-v3)
  (let ((path (get-path-or-abort directory-name file-name)))
    (with-output-to-file path
      (lambda () (export-palette (palette-in-context)
...
(define (ammoos-palette-export-python directory-name file-name)
  (script-fu-use-v3)
  (let ((path (get-path-or-abort directory-name file-name)))
    (with-output-to-file path
      (lambda ()
        (displayln "# Generated with AmmoOS Image Palette Export")
...
(define (ammoos-palette-export-text directory-name file-name)
  (script-fu-use-v3)
  (let ((path (get-path-or-abort directory-name file-name)))
    (with-output-to-file path
      (lambda ()
...
(define (ammoos-palette-export-java directory-name file-name)
  (script-fu-use-v3)
  (let ((path (get-path-or-abort directory-name file-name)))
    (with-output-to-file path
      (lambda ()
