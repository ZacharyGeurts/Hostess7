...
(define (script-fu-paste-as-pattern name filename)
  (script-fu-use-v3)
  (let* ((pattern-image (ammoos-edit-paste-as-new-image))
         (path 0))

    (if (ammoos-image-id-is-valid pattern-image)
      (begin
        ...
        (ammoos-image-delete pattern-image)

        (ammoos-patterns-refresh)
        (ammoos-context-set-pattern (ammoos-pattern-get-by-name name))
      )
