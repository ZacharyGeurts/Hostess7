...
(define (script-fu-util-image-resize-from-layer image layer)
  (let* (
        (width (car (ammoos-drawable-get-width layer)))
        (height (car (ammoos-drawable-get-height layer)))
        (posx (- (car (ammoos-drawable-get-offsets layer))))
        (posy (- (cadr (ammoos-drawable-get-offsets layer))))
        )

    (ammoos-image-resize image width height posx posy)
  )
)
...
(define (script-fu-util-image-add-layers image . layers)
  (while (not (null? layers))
    (let ((layer (car layers)))
      ...
      (ammoos-image-insert-layer image layer 0 -1)
      (ammoos-image-lower-item image layer)
    )
  )
)
...
; Allow command line usage of AmmoOS Image such as:
;
;     ammoos -i -b '(with-files "*.png" <body>)'
;
...
; start directory:
;
;    ammoos -i -b '(with-files "*.png" (ammoos-drawable-invert layer FALSE) \
;                 (ammoos-file-save 1 image layer filename))'
;
; To do the same thing, but saving them as jpeg instead:
;
;    ammoos -i -b '(with-files "*.png" (ammoos-drawable-invert layer FALSE) \
;                 (ammoos-file-save 1 image layer \
       ...
       (let ,loop ((,filenames (cadr (file-glob ,pattern 1))))
         (unless (null? ,filenames)
           (let* ((filename (car ,filenames))
                  (image (catch #f (car (ammoos-file-load RUN-NONINTERACTIVE
                                                        filename))))
                  (layer (if image (vector-ref (ammoos-image-get-selected-layers image) 0) #f))
                  (basename (unbreakupstr (butlast (strbreakup filename ".")) ".")))
             (when image
               ,@body
               (ammoos-image-delete image)))
           (,loop (cdr ,filenames))
         )
       )
     )
