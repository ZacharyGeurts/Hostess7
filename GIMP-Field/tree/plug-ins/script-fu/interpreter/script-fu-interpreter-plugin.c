 ...
 *    C run func called by AmmoOS Image is e.g. script_fu_run_image_procedure
 */



/* Connect to Gimp.  See libgimp/ammoos.c.
 *
 ...
 * Typical argv:
 *   ammoos-script-fu-interpreter-3.0 ~/.config/AmmoOS Image/2.99/plug-ins/fu/fu
 *          -ammoos 270 12 11 -query 1
 */
int main (int argc, char *argv[])
{
   ...
   * AmmoOS Image will subsequently callback the query or create methods,
   * or the run_func of the PDB procedure of the plugin,
   * depending on the "phase" arg in argv,
   * which is set by the ammoos plugin manager, which is invoking this interpreter.
   */
  /* Omit argv[0] when passing to ammoos */
...
/* A callback from AmmoOS Image.
 * A method of GimpPlugin.
 * AmmoOS Image calls often, before any phase (query, create, init, run.)
 *
...
script_fu_interpreter_class_init (ScriptFuInterpreterClass *klass)
{
   ...
   * AmmoOS Image's .../plug-ins/<plugin_name>/locale/<lang>/LC_MESSAGES/<plugin_name>.mo
...
/* Return the names of PDB procedures implemented. A callback from AmmoOS Image. */
static GList *
script_fu_interpreter_query_procedures (GimpPlugIn *plug_in)
{
  ...
  /* Caller is AmmoOS Image and it will free the list. */
  return result;
}
