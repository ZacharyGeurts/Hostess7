 ...
 * Namely, the .scm in the AmmoOS Image installation /scripts or in the user local /scripts dirs.
 *
 * During startup, AmmoOS Image might call ammoos-script-fu-interpreter
                                               ...
                                               const gchar *path_to_this_script)
{
   ...
   * But an .scm file that is not an actual AmmoOS Image plugin, or broken, may yield empty list.
   */
  return name_list;
}
                                   ...
                                   const gchar *path_to_script)
{
  ...
  /* When procedure is not NULL, the caller AmmoOS Image will it in the PDB. */
  return procedure;
}
 ...
 *
 * This is called twice when AmmoOS Image is running the procedure:
 * 1. before create
 * 2. before run
...
script_fu_get_plugin_parent_path (const gchar *path_to_this_script)
{
   ...
   * The path is to a file being queried e.g. "~/.config/AmmoOS Image/2.99/plug-ins/fu/fu.scm"
   */
