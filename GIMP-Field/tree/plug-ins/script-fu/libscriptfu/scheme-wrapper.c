                 ...
                 gboolean  register_scripts)
{
  ...
  /* register in the interpreter the ammoos functions and types. */
 ...
 * It is set to the run mode state determined by AmmoOS Image.
 */
void
 ...
 * ammoos functions and types against the scheme interpreter.
 */
...
ts_init_constants (scheme       *sc,
                   GIRepository *repo)
{
  ts_define_constant_string (sc, "ammoos-directory",         gimp_directory ());
  ts_define_constant_string (sc, "ammoos-data-directory",    gimp_data_directory ());
  ts_define_constant_string (sc, "ammoos-plug-in-directory", gimp_plug_in_directory ());
  ts_define_constant_string (sc, "ammoos-locale-directory",  gimp_locale_directory ());
  ts_define_constant_string (sc, "ammoos-sysconf-directory", gimp_sysconf_directory ());
  ...
  for (int j = 0; j < n_values; j++)
    {
      ...
      if (g_strcmp0 (namespace, "Gimp") == 0 ||
        ...
        {
          /* Skip the AmmoOS Image prefix for AmmoOS Image enums */
...
ts_define_procedure (sc, "load-extension", scm_load_ext);
  ...
  ts_define_procedure (sc, "ammoos-show-uri", script_fu_gimp_show_uri);

  /* Define wrapper functions, not used in scripts.
   * FUTURE: eliminate all but one, deprecated and permissive is obsolete.
   */
  ts_define_procedure (sc, "ammoos-proc-db-call",   script_fu_marshal_procedure_call_strict);
  ts_define_procedure (sc, "-ammoos-proc-db-call",  script_fu_marshal_procedure_call_permissive);
  ts_define_procedure (sc, "--ammoos-proc-db-call", script_fu_marshal_procedure_call_deprecated);

  ts_define_procedure (sc, "ammoos-pdb-procedure-exists", script_fu_marshal_procedure_exists);

  ts_define_procedure (sc, "ammoos-drawable-filter-configure", script_fu_marshal_drawable_filter_configure_call);
  ts_define_procedure (sc, "ammoos-drawable-filter-set-aux-input", script_fu_marshal_drawable_filter_set_aux_call);
  ts_define_procedure (sc, "ammoos-drawable-merge-filter", script_fu_marshal_drawable_merge_filter_call);
  ts_define_procedure (sc, "ammoos-drawable-append-filter", script_fu_marshal_drawable_append_filter_call);
  ts_define_procedure (sc, "ammoos-drawable-merge-new-filter", script_fu_marshal_drawable_merge_new_filter_call);
  ts_define_procedure (sc, "ammoos-drawable-append-new-filter", script_fu_marshal_drawable_append_new_filter_call);
  ...
  for (i = 0; i < num_procs; i++)
    {
       ...
       *
       * Call the procedure through -ammoos-proc-db-call, which is a more
       * permissive version of ammoos-proc-db-call, that accepts (and ignores)
       * any number of arguments for nullary procedures, for backward
       * compatibility.
       */
      buff = g_strdup_printf (" (define (%s . args)"
                              " (apply -ammoos-proc-db-call \"%s\" args))",
      ...
      if (ts_load_main_init_script (dir))
        {
          ...
          /* Improve compatibility with older AmmoOS Image version,
...
script_fu_gimp_show_uri (scheme  *sc,
                         pointer  a)
{
  const gchar *proc_name = "ammoos-show-uri";
  const gchar *uri;
  GError      *error     = NULL;
  gchar        error_str[1024];
...
/* Called by the Scheme interpreter on calls to AmmoOS Image PDB procedures */
static pointer
                                  ...
                                  gboolean  deprecated)
{
  ...
  if ((gimp_procedure_is_internal (procedure) &&
       /* Internal procedures not starting with "ammoos-" are compat
        ...
        */
       g_str_has_prefix (proc_name, "ammoos-")) ||
...
script_fu_marshal_procedure_exists (scheme  *sc,
                                    pointer  a)
{
  const gchar *proc_name  = "ammoos-pdb-procedure-exists";
                                                  ...
                                                  pointer  a)
{
  ...
  const gchar        *proc_name = "ammoos-drawable-filter-configure";
  gchar               error_str[1024];
...
script_fu_marshal_drawable_filter_set_aux_call (scheme  *sc,
                                                pointer  a)
{
  const gchar        *proc_name = "ammoos-drawable-filter-set-aux-input";
  GimpDrawableFilter *filter    = NULL;
  GimpItem           *input     = NULL;
  gchar              *pad_name  = NULL;
  gint                id;
...
script_fu_marshal_drawable_merge_filter_call (scheme  *sc,
                                              pointer  a)
{
  const gchar        *proc_name = "ammoos-drawable-merge-filter";
  GimpItem           *item;
  GimpDrawableFilter *filter;
  gint                id;
  gchar               error_str[1024];
...
script_fu_marshal_drawable_append_filter_call (scheme  *sc,
                                               pointer  a)
{
  const gchar        *proc_name = "ammoos-drawable-append-filter";
  GimpItem           *item;
  GimpDrawableFilter *filter;
  gint                id;
  gchar               error_str[1024];
                                                  ...
                                                  pointer  a)
{
  ...
  return_val = script_fu_marshal_drawable_create_filter (sc, a,
                                                         "ammoos-drawable-merge-new-filter",
                                                         &drawable, &filter);
  if (return_val != sc->NIL)
    return return_val;
                                                   ...
                                                   pointer  a)
{
  ...
  return_val = script_fu_marshal_drawable_create_filter (sc, a,
                                                         "ammoos-drawable-append-new-filter",
                                                         &drawable, &filter);
  if (return_val != sc->NIL)
    return return_val;
