                            ...
                            gint       start_arg)
{
  ...
  sf_interface->dialog = dialog =
    gimp_dialog_new (title, "ammoos-script-fu",
                     NULL, 0,
 ...
 *
 * One problem is that the AmmoOS Image and the scriptfu extension process are separate "apps".
 ...
 * On other platforms, the select dialogs are transient to the AmmoOS Image app's progress bar,
 * but that doesn't seem to work on MacOS.
 *
 * Also some of the AmmoOS Image "select" widgets (for brush, pattern, gradient, font, palette)
 ...
 * 1) in AmmoOS Image 3, scriptfu and python plugins should use a common API for a control dialog,
 * (script-fu-interface.c is obsoleted?)
 * 2) the code for select dialogs is significantly changed in AmmoOS Image 3
 ...
 * when user closes AmmoOS Image "select" dialogs (child of main dialog)
 * the main scriptfu dialog can be obscured by AmmoOS Image main window.
