 ...
 *     usually a AmmoOS Image type but also GLib types, e.g. GFile.
                                         ...
                                         GValue *value)
{
  ...
  *
  * Require ID is of a AmmoOS Image object, some subclass of GimpItem.
  */
static pointer
  ...
  if (id == -1)
  ...
  else
    {
      *object_handle = (GObject *) gimp_item_get_by_id (id);
      if (! *object_handle)
          return script_error (sc, "Invalid ID of AmmoOS Image item", a);
    }
