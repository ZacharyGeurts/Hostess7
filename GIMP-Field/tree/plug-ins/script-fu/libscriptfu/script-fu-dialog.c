 ...
 *
 * The dialog is non-modal with respect to the AmmoOS Image app GUI, which remains responsive.
 *
 * When called from plugin extension-script-fu, the dialog is modal on the extension:
 * although AmmoOS Image app continues responsive, a user choosing a menu item
 ...
 * For ammoos-script-fu-interpreter, each plugin is a separate process
               ...
               gboolean              should_skip_runmode)
{
  ...
  /* The user chose a plugin from ammoos app, now ensure this process is active.
   * The ammoos app was active, but now the plugin should be active.
   ...
   * The user can still raise other ammoos windows, hiding the dialog.
   */
