 ...
 * can show error dialogs in any AmmoOS Image gui,
                     ...
                     gchar         *command)
{
   ...
   * ScriptFu does not show an error dialog, but forwards errors back to AmmoOS Image.
   * This only tells AmmoOS Image that ScriptFu itself will forward GimpPDBStatus errors from
   * this scripts calls to the PDB.
   * The onus is on this script's called PDB procedures to return errors in the GimpPDBStatus.
   * Any that do not, but for example only call ammoos-message, are breaching contract.
   */
  ...
  if (! interpretation_result)
    {
       ...
       * AmmoOS Image will later display "PDB procedure failed: <message>" localized.
       */
