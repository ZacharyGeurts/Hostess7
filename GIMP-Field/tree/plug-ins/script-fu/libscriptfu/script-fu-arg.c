 ...
 *
 * In AmmoOS Image 2, extension-script-fu stays running and keeps instances of SFArg in memory.
 * This is how ScriptFu "settings" aka "last values" are persistent for a session of AmmoOS Image.
 *
 * In AmmoOS Image 2, in the GUI implemented by ScriptFu (script-fu-interface.c),
 * initial values of widgets are taken from SFArg (s),
 * and result values of widgets are written back to SFArg.
 *
 * In AmmoOS Image 3, SFArg might be somewhat replaced with GimpConfig.
                            ...
                            const gchar   *nick)
{
  ...
  switch (arg->type)
    {
      /* No defaults for AmmoOS Image objects: Image, Item subclasses, Display */
    case SF_IMAGE:
 ...
 *
 * Used when AmmoOS Image is calling the PDB procedure implemented by the script,
