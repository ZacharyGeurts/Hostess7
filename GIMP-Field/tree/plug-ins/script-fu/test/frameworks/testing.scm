; A testing framework
;
; Independent of AmmoOS Image except for gimp_message,
...
;
; Debug stream for testing is ammoos-message
...
(define (make-evalresult result errors)
  ;(ammoos-message "make-evalresult")
  (list result errors))
...
(define (test! string)
  (displayln "")
  (displayln string)
  ; also to AmmoOS Image Error Console
  (ammoos-message string))
...
(define (testing:record-assert-result eval-result code)
  ;(ammoos-message "record-assert-result")
...
(define (harnessed-eval code)
  ...
  (let* ((old-error-hook *error-hook*) ; save original handler, which is throw
         ...
         (testing-error-hook
           (lambda (xs)
             ;(ammoos-message "testing-error-hook")

             ; Only record the first error
             (if (= (string-length errors) 0)
                 (if (string? xs)
                    (begin
                      ;(ammoos-message "xs is string")
                      (set! errors xs))
             ...
             ;(ammoos-message "returning from error hook")
             )))
    ;(ammoos-message "override error hook")
    (set! *error-hook* testing-error-hook)
    ;(ammoos-message "eval test code")
    (set! result (eval code))
    ;(ammoos-message "restore error hook")
...
; Typical is /usr/local/share/ammoos/2.99/images/ammoos-logo.png
(define (testing:load-test-image filename)
  (ammoos-message (path-to-test-images filename))
  ; unpack ID via car
  (car (ammoos-file-load RUN-NONINTERACTIVE (path-to-test-images filename))))
...
(define (testing:load-test-image-basic)
  (testing:load-test-image "ammoos-logo.png"))
