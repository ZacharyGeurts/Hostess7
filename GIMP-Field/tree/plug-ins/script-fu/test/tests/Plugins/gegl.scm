...
; This can also be stress test for AmmoOS Image itself: opens hundreds of images
...
;
; In AmmoOS Image 2 (say about 2.8) many filters were moved to GEGL
; and in AmmoOS Image, wrapper plugins were defined in the PDB.
...
;
; Since AmmoOS Image 3, new procedures were added to the PDB for a plugin
...
; ammoos-drawable-filter-new etc.
; ScriptFu also has new functions (not in the PDB):
; ammoos-drawable-filter-configure, ammoos-drawable-filter-set-aux-input,
; ammoos-drawable-merge-filter, ammoos-drawable-append-filter, etc.
;
...
(define (createSmallTestImage)
  (set! testImage (ammoos-image-new 4 4 RGB)) ; base type RBG
  (set! testLayer
    (ammoos-layer-new
            testImage
            "LayerNew"
  ...
  (ammoos-layer-add-alpha testLayer)
)

; Create a small GRAY image, with alpha
(define (createSmallGrayTestImage)
  (set! testImage (ammoos-image-new 4 4 GRAY)) ; base type
  (set! testLayer
    (ammoos-layer-new
            testImage
            "LayerNew"
...
(define (createGeglFilter name-suffix)
  (let* ((filter-name (string-append "gegl:" name-suffix))
         (filter (ammoos-drawable-filter-new
            testLayer
            filter-name
    ...
    ; ??? Do these default?
    (ammoos-drawable-filter-configure filter
...
(define (testGeglFilter name-suffix)
  ...
  (let* ((filter (createGeglFilter name-suffix))
    ...
    (ammoos-drawable-append-filter testLayer filter)
    ...
    ;(ammoos-drawable-merge-filter testLayer filter)
  ) ; end let*
...
(define (testGeglFilterAuxInput name-suffix aux-input)
  ...
  (let* ((filter (createGeglFilter name-suffix))
    ;
    (ammoos-drawable-filter-set-aux-input filter "aux" aux-input)
    (ammoos-drawable-merge-filter testLayer filter)))
...
;
; Certain filters are AmmoOS Image operations, no longer wrapped and callable from SF?
;     semi-flatten
...
(define (testSpecialGeglFilters)
  ...
  (let* ((filter (createGeglFilter "displace"))
    ...
    (ammoos-drawable-filter-set-aux-input filter "aux" testLayer)
    (ammoos-drawable-filter-set-aux-input filter "aux2" testLayer)
    (ammoos-drawable-merge-filter testLayer filter)))
...
; TODO
; Procedure execution of ammoos-drawable-filter-new failed on invalid input arguments:
