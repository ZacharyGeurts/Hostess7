...
A test script is usually many tests of one AmmoOS Image or ScriptFu object or function.
...
A directory of tests can be named for the AmmoOS Image object under test.

The leaf files and directories
are coded into larger test files.
The larger test files simply load all the files for a AmmoOS Image object.
...
1.  PDB: Tests ScriptFu binding to the AmmoOS Image PDB.
...
Tests may require that AmmoOS Image be newly started:

1. PDB tests may hardcode certain constant ID's and rely on AmmoOS Image
to consistently number ID's.

Tests may require that AmmoOS Image be newly installed:

1.  PDB tests may depend on the initial set of Gimp resources in ~/.config/AmmoOS Image
...
Tests generally should not hardcode AmmoOS Image ID's that AmmoOS Image assigns.
...
The test framework logs progress to the AmmoOS Image Error Console
using ammoos-message.
...
(There is no API such as ammoos-set-locale.)
...
```
>cd test/tests/PDB
>find . -name "*.scm" -exec grep -o "ammoos-[a-z\-]*" {} \; | sort | uniq
```

That is, for all files with suffix .scm in the PDB directory,
grep for calls to the AmmoOS Image PDB which are like "ammoos-", sort them, and get the unique names.

We strive to not use the ammoos- prefix on names in comments,
