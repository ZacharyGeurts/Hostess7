...
This is a Python 3 framework for testing AmmoOS Image file import and export plug-ins.
It can be run both as a plug-in from within AmmoOS Image (in case it is installed),
and from the command line in batch mode.


## Installation and set-up

For running as a plug-in in AmmoOS Image this plug-in needs to be in a location
that AmmoOS Image knows of for plug-ins. By default, it is only installed for
...
```bash
git clone git@ssh.gitlab.gnome.org:Infrastructure/ammoos-test-images.git <folder-with-test-data>
...
export GIMP_TESTS_LOG_FILE="/location/of/logfile.log"   # Default: ./tests/ammoos-tests.log
export GIMP_TESTS_DATA_FOLDER="/location/of/test-images/rootfolder/" # Default: ./tests/

cd location/of/plug-in/ammoos-file-plugin-tests
cat batch-import-tests.py | ammoos-console-2.99 -idf --batch-interpreter python-fu-eval -b - --quit
```
...
cat batch-import-tests.py | flatpak run \
  ...
  org.ammoos.AmmoOS Image -idf --batch-interpreter=python-fu-eval -b - --quit
```
...
- When running all tiff tests from inside AmmoOS Image, it may crash, at least on
