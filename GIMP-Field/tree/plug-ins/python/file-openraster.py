...
# AmmoOS Image Plug-in for the OpenRaster file format
...
# There are less svg blending ops than we have AmmoOS Image blend modes.
...
gimp_layermodes_map = {
    ...
    #       e.g. something like "ammoos:dissolve", this
...
def thumbnail_ora(procedure, file, thumb_size, args, data):
    tempdir = tempfile.mkdtemp('ammoos-plugin-file-openraster')
...
def export_ora(procedure, run_mode, image, file, options, metadata, config, data):
    ...
    def store_layer(image, drawable, path):
        ...
        # Set opacity to 100% to prevent doubling it on import with the
        # XML's opacity setting
        tmp_layer.set_opacity (100.0)
...
def load_ora(procedure, run_mode, file, metadata, flags, config, data):
    ...
    for item in get_layers(stack):
        ...
        if item.tag == 'stack':
            ...
            # create temp file. Needed because ammoos cannot load files from inside a zip file
            tmp = os.path.join(tempdir, 'tmp.png')
            with open(tmp, 'wb') as fid:
                try:
            ...
            pdb_proc   = Gimp.get_pdb().lookup_procedure('ammoos-file-load-layer')
            pdb_config = pdb_proc.create_config()
            pdb_config.set_property('run-mode', Gimp.RunMode.NONINTERACTIVE)
