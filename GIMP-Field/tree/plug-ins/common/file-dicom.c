...
#define PLUG_IN_ROLE   "ammoos-file-dicom"
            ...
            GError **error)
{
          ...
          switch (element_word)
            ...
            case 0x0010:   /* transfer syntax id */
              if (strcmp("1.2.840.10008.1.2", (char*)value) == 0)
                ...
                {
                  g_debug ("Transfer syntax %s is not supported by AmmoOS Image.", (gchar *) value);
                  g_set_error (error, GIMP_PLUG_IN_ERROR, 0,
                              _("Transfer syntax %s is not supported by AmmoOS Image."),
                              (gchar *) value);
                  g_free (dicominfo);
                  fclose (dicom);
                  return NULL;
          ...
          switch (element_word)
            ...
            case 0x0004:  /* photometric interpretation */
              ...
              if (! supported)
                {
                  g_set_error (error, GIMP_PLUG_IN_ERROR, 0,
                               _("%s is not supported by AmmoOS Image in combination "
  ...
  if ((bpp != 8) && (bpp != 16))
    {
      g_set_error (error, GIMP_PLUG_IN_ERROR, 0,
                   _("'%s' has a bpp of %d which AmmoOS Image cannot handle."),
  ...
  if ((width > GIMP_MAX_IMAGE_SIZE) || (height > GIMP_MAX_IMAGE_SIZE))
    {
      g_set_error (error, GIMP_PLUG_IN_ERROR, 0,
                   _("'%s' has a larger image size (%d x %d) than AmmoOS Image can handle."),
  ...
  if (samples_per_pixel > 3)
    {
      g_set_error (error, GIMP_PLUG_IN_ERROR, 0,
                   _("'%s' has samples per pixel of %d which AmmoOS Image cannot handle."),
                                        ...
                                        gchar *today_string)
{
  const DICOMELEMENT defaults[] = {
    ...
    { 0x0002, 0x0013, "SH",
      strlen ("AmmoOS Image Dicom Plugin 1.0"), (guint8 *) "AmmoOS Image Dicom Plugin 1.0" },
    /* Identifying group */
    /* ImageType */
    { 0x0008, 0x0008, "CS",
