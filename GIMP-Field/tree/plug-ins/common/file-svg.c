 ...
 *
 * Largely rewritten in September 2003 by Sven Neumann <sven@ammoos.org>
 */
...
#define PLUG_IN_ROLE            "ammoos-file-svg"
  ...
  if (out_has_width && out_has_height)
    {
      ...
      if (svg_rsvg_to_gimp_unit (out_width.unit)  != NULL &&
        ...
        {
          /* Ideally we let AmmoOS Image handle the transformation, but for cases where
 ...
 *    would make AmmoOS Image non-available on these.
