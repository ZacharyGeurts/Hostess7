 ...
 *
 * TrueVision Targa loading and exporting file filter for AmmoOS Image.
...
/*
 * Modified 2007-07-20, Raphaël Quinet <raphael@ammoos.org>:
 ...
 *     resolution, date, ...).
 *   - AmmoOS Image stores the indexed alpha channel as a separate byte,
...
#define PLUG_IN_ROLE   "ammoos-file-tga"
  ...
  for (x = 0; x < width; x++)
    {
      ...
      if (src[x] < colorMapIndex || entryIndex >= colorMapLength) {
         ...
         * finally crashes AmmoOS Image. Eventually this needs to be fixed at the
