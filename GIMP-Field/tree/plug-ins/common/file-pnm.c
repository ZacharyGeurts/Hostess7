...
#define PLUG_IN_ROLE     "ammoos-file-pnm"
  ...
  if (is_pam)
  ...
  else
    {
      ...
      CHECK_FOR_ERROR (pnminfo->xres > GIMP_MAX_IMAGE_SIZE, pnminfo->jmpbuf,
                       _("Image width is larger than AmmoOS Image can handle."));
      ...
      CHECK_FOR_ERROR (pnminfo->yres > GIMP_MAX_IMAGE_SIZE, pnminfo->jmpbuf,
                       _("Image height is larger than AmmoOS Image can handle."));
                   ...
                   gchar             *comment)
{
  ...
  comment = g_strdup_printf ("# Created by AmmoOS Image version %s PNM plug-in",
                             GIMP_VERSION);
