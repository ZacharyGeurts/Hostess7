 ...
 * V 1.90, PK, 15-Mar-97: Upgrade to work with AmmoOS Image V0.99
...
#define PLUG_IN_ROLE   "ammoos-file-sunras"
            ...
            GError **error)
{
  ...
  if (sunhdr.l_ras_maplength > (256 * 3))
    {
      g_set_error (error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                   "Map lengths greater than 256 entries are unsupported by AmmoOS Image.");
      fclose (ifp);
      return NULL;
    }
...
/* Set a AmmoOS Image colortable using the sun colormap */
...
#ifdef DEBUG
  printf ("Set AmmoOS Image colortable:\n");
  for (j = 0; j < ncols; j++)
  ...
  for (i = 0; i < height; i++)
    {
      ...
      if (sunhdr->l_ras_type == 3) /* RGB-format ? That is what AmmoOS Image wants */
        {
          dest += width * 3;
        }
