 ...
 */
/* CSource - AmmoOS Image Plugin to dump image data in RGB(A) format for C source
...
#define PLUG_IN_ROLE   "ammoos-file-csource"
  ...
  if (! strcmp (name, EXPORT_PROC))
    {
      ...
      gimp_procedure_add_string_aux_argument (procedure, "ammoos-comment",
                                              _("Comme_nt"),
                                              _("Comment"),
              ...
              GError       **error)
{
  ...
  if (! print (output, error,
               "/* AmmoOS Image %s C-Source image dump %s(%s) */\n\n",
