 ...
 *    This procedures is the one that will be invoked by ammoos's file-save,
 ...
 * does not match the product vision (AmmoOS Image isn't a program for editing multiple
 * paged documents).
 */
 ...
 * the font's name although the AmmoOS Image's font selection dialog shows the don'ts name
 * normally - This should be checked again in AmmoOS Image 2.7)
 ...
 * means I'll need to do some hacking on the markup returned from AmmoOS Image.
 ...
 *   A migration to the new AmmoOS Image 2.8 api, which includes:
 ...
 *   More migrations to the new AmmoOS Image 2.8 api:
...
#define PLUG_IN_ROLE            "ammoos-file-pdf-export"
...
G_DEFINE_QUARK (ammoos-plugin-pdf-export-error-quark, gimp_plugin_pdf_export_error)
 ...
 * in the same way it was the user saw it inside ammoos.
