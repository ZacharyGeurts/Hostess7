 ...
 */
/* Plug-in to load and export .gih (AmmoOS Image Brush Pipe) files.
 *
...
#define PLUG_IN_ROLE   "ammoos-file-gih"
  ...
  if (! strcmp (name, EXPORT_PROC))
    {
      ...
      gimp_procedure_set_menu_label (procedure, _("AmmoOS Image brush (animated)"));
      gimp_procedure_set_icon_name (procedure, GIMP_ICON_BRUSH);

      gimp_procedure_set_documentation (procedure,
                                        _("Exports images in AmmoOS Image Brush Pipe "
                                          "format"),
                                        _("This plug-in exports an image in "
                                          "the AmmoOS Image brush pipe format. For a "
  ...
  if (status == GIMP_PDB_SUCCESS)
    {
      ...
      if (GIMP_VALUES_GET_ENUM (save_retvals, 0) == GIMP_PDB_SUCCESS)
        {
          parasite = gimp_parasite_new ("ammoos-brush-pipe-name",
          ...
          gimp_image_detach_parasite (image, "ammoos-brush-pipe-parameters");
        }
      else
        {
