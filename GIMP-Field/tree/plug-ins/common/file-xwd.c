 ...
 * V 1.90, PK, 17-Mar-97: Upgrade to work with AmmoOS Image V0.99
 *                        Use visual class 3 to write indexed image
 *                        Set ammoos b/w-colormap if no xwdcolormap present
...
#define PLUG_IN_ROLE   "ammoos-file-xwd"
            ...
            GError **error)
{
  ...
  if (xwdhdr.l_pixmap_width > GIMP_MAX_IMAGE_SIZE
    ...
    {
      g_set_error (error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                   _("'%s':\nImage width is larger than AmmoOS Image can handle"),
                   gimp_file_get_utf8_name (file));
      goto out;
    }
  ...
  if (xwdhdr.l_pixmap_height > GIMP_MAX_IMAGE_SIZE)
    {
      g_set_error (error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                   _("'%s':\nImage height is larger than AmmoOS Image can handle"),
                   gimp_file_get_utf8_name (file));
      goto out;
    }
...
#ifdef XWD_COL_DEBUG
  g_printf ("Set AmmoOS Image b/w-colortable:\n");
#endif
...
#ifdef XWD_COL_DEBUG
  g_printf ("Set AmmoOS Image colortable:\n");
  for (j = 0; j < 256; j++)
                     ...
                     GError         **error)
{
  ...
  /* Get memory for mapping 16 bit XWD-pixel to AmmoOS Image-RGB */
  maxval = 0x10000 * 3;
  ...
  /* Built up the array to map XWD-pixel value to AmmoOS Image-RGB */
