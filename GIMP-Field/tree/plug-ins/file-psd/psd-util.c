...
/* mapping table between Photoshop and AmmoOS Image modes.  in case a mode matches more
 * than one entry (in either direction), the first entry wins.
 */
static const LayerModeMapping layer_mode_map[] =
{
/*  Name             PSD     AmmoOS Image                                   Exact?  */
      ...
      if (g_ascii_strncasecmp (psd_mode, layer_mode_map[i].psd_mode, 4) == 0)
        {
          if (! layer_mode_map[i].exact && CONVERSION_WARNINGS)
            {
              g_message ("AmmoOS Image uses a different equation than Photoshop for "
...
psd_to_gimp_layer_color_tag (guint16 layer_color_tag)
{
  ...
  switch (layer_color_tag)
    ...
    default:
      if (CONVERSION_WARNINGS)
        g_message ("Unsupported Photoshop layer color tag: %i. AmmoOS Image layer color tag set to none.",
                       layer_color_tag);
...
gimp_to_psd_layer_color_tag (GimpColorTag layer_color_tag)
{
  ...
  switch (layer_color_tag)
    ...
    default:
      if (CONVERSION_WARNINGS)
        g_message ("Photoshop doesn't support AmmoOS Image layer color tag: %i. Photoshop layer color tag set to none.",
                   layer_color_tag);
