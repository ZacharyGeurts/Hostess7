 ...
 * This AmmoOS Image plug-in is designed to export Adobe Photoshop(tm) files (.PSD)
 *
 * Monigotes
 *
 ...
 *
 *  2005-2-11 Jay Cox <jaycox@ammoos.org>
...
#define WRITE_RETURN(type, output, val, why, cleanup) G_STMT_START { \
  write_##type (output, val, why, error);                            \
  if (*error)                                                        \
    {                                                                \
      cleanup;                                                       \
      return FALSE;                                                  \
...
#define WRITE_GOTO(type, output, val, why) G_STMT_START { \
  write_##type (output, val, why, error);                 \
  if (*error)                                             \
    goto clean;                                           \
  } G_STMT_END

#define XFWRITE_RETURN(output, buf, len, why, cleanup) G_STMT_START { \
  xfwrite (output, buf, len, why, error);                             \
  if (*error)                                                         \
    {                                                                 \
      cleanup;                                                        \
      return FALSE;                                                   \
...
#define XFWRITE_GOTO(output, buf, len, why) G_STMT_START { \
  xfwrite (output, buf, len, why, error);                  \
  if (*error)                                              \
    goto clean;                                            \
  } G_STMT_END
...
write_string (GOutputStream  *output,
              ...
              GError        **error)
{
  WRITE_RETURN (gchar, output, strlen (val), why, ;);
  XFWRITE_RETURN (output, val, strlen (val), why, ;);
                    ...
                    GError        **error)
{
  ...
  if (len !=  0)
    {
      WRITE_RETURN (gchar, output, len, why, ;);
      XFWRITE_RETURN (output, val, len, why, ;);
    }
  else
    {
      WRITE_RETURN (gchar, output, 0, why, ;);
    }
         ...
         GError        **error)
{
  ...
  if (! g_output_stream_write_all (output, buf, len, &bytes_written, NULL, error))
    g_prefix_error (error, "%s: Error while writing '%s': ", G_STRFUNC, why);
}
             ...
             GError        **error)
{
  ...
  if (g_seekable_seek (G_SEEKABLE (output), pos + 1, G_SEEK_SET, NULL, error))
    g_prefix_error (error, "%s: Error while writing '%s': ", G_STRFUNC, why);
}
              ...
              GError        **error)
{
  ...
  if (! g_output_stream_write_all (output, &b, 2, &bytes_written, NULL, error))
    g_prefix_error (error, "%s: Error while writing '%s': ", G_STRFUNC, why);
}
              ...
              GError        **error)
{
  ...
  if (! g_output_stream_write_all (output, &b, 4, &bytes_written, NULL, error))
    g_prefix_error (error, "%s: Error while writing '%s': ", G_STRFUNC, why);
}
              ...
              GError        **error)
{
  ...
  if (! g_output_stream_write_all (output, &b, 8, &bytes_written, NULL, error))
    g_prefix_error (error, "%s: Error while writing '%s': ", G_STRFUNC, why);
}
      ...
      if (luniName)
        {
          ...
          XFWRITE_RETURN (output, "8BIMluni", 8, "luni xdb signature", ;);
          WRITE_RETURN (gint32, output, xdBlockSize, "luni xdb size", ;);
          WRITE_RETURN (gint32, output, len, "luni xdb pascal string", ;);

          for (count = 0; count < len; count++)
            WRITE_RETURN (gint16, output, luniName[count], "luni xdb pascal string", ;);

          /* Pad to an even number of chars */
          if (len % 2)
            WRITE_RETURN (gint16, output, 0x0000, "luni xdb pascal string padding", ;);
        }
    }
...
gimpBaseTypeToPsdMode (GimpImageBaseType gimpBaseType)
{
  switch (gimpBaseType)
    ...
    case GIMP_INDEXED:
      return 2;                                         /* Indexed */
    default:
      g_message (_("Error: Can't convert AmmoOS Image base imagetype to PSD mode"));
      IFDBG(1) g_debug ("PSD Export: gimpBaseType value is %d, "
                        "can't convert to PSD mode", gimpBaseType);
      g_return_val_if_reached (3);
             ...
             GError             **error)
{
  ...
  XFWRITE_RETURN (output, "8BPS", 4, "signature", ;);
  WRITE_RETURN (gint16, output, options->psb ? 2 : 1, "version", ;);
  WRITE_RETURN (gint32, output, 0, "reserved 1", ;);      /* 6 for the 'reserved' field + 4 bytes for a long */
  WRITE_RETURN (gint16, output, 0, "reserved 1", ;);      /* and 2 bytes for a short */
  WRITE_RETURN (gint16, output, nChannels, "channels", ;);
  WRITE_RETURN (gint32, output, PSDImageData.image_height, "rows", ;);
  WRITE_RETURN (gint32, output, PSDImageData.image_width, "columns", ;);
  WRITE_RETURN (gint16, output, 8 * get_bpc (image), "depth", ;);
  if (options->cmyk)
    WRITE_RETURN (gint16, output, PSD_CMYK, "mode", ;);
  else if (options->duotone)
    WRITE_RETURN (gint16, output, PSD_DUOTONE, "mode", ;);
  else
    WRITE_RETURN (gint16, output, gimpBaseTypeToPsdMode (PSDImageData.baseType), "mode", ;);
  ...
  if (export_duotone && parasite)
    {
      ...
      WRITE_RETURN (gint32, output, parasite_size, "color data length",
                    gimp_parasite_free (parasite));
      XFWRITE_RETURN (output, parasite_data, parasite_size, "colormap",
                      gimp_parasite_free (parasite));
      ...
      if (nColors == 0)
        {
          ...
          WRITE_RETURN (gint32, output, 0, "color data length",
                        g_free (cmap));
        }
      else if (nColors != 256)
        {
          ...
          WRITE_RETURN (gint32, output, 768, "color data length",
                        g_free (cmap));
          ...
          XFWRITE_RETURN (output, cmap_modified, 768, "colormap",  /* Write readjusted colormap */
                          g_free (cmap); g_free (cmap_modified));

          g_free (cmap_modified);
        }
      else         /* nColors equals 256 */
        {
          WRITE_RETURN (gint32, output, 768, "color data length",   /* For this type, length is always 768 */
                        g_free (cmap));
          reshuffle_cmap_write (cmap);
          XFWRITE_RETURN (output, cmap, 768, "colormap",  /* Write readjusted colormap */
                          g_free (cmap));
        }
      g_free (cmap);
      break;
                ...
                GError             **error)
{
  ...
  /* Only relevant resources in AmmoOS Image are: 0x03EE, 0x03F0 & 0x0400 */
      ...
      if (PSDImageData.nChannels > 0 || write_transparency_resource)
        {
          ...
          WRITE_RETURN (gint32, output, 0, "0x03EE resource size", ;);
          ...
          if (! g_seekable_seek (G_SEEKABLE (output), name_sec, G_SEEK_SET, NULL, error))
            return FALSE;
          ...
          if (! g_seekable_seek (G_SEEKABLE (output), eof_pos, G_SEEK_SET, NULL, error))
            return FALSE;
      ...
      if (PSDImageData.nChannels > 0 || write_transparency_resource)
        {
           ...
           */
          WRITE_RETURN (gint32, output, 1, "0x0435 version", ;);
          ...
          if (write_transparency_resource)
            {
              ...
              WRITE_RETURN (gint16, output, (gint16) DOUBLE_TO_INT16 (0.0), "channel color g", ;);
              WRITE_RETURN (gint16, output, (gint16) DOUBLE_TO_INT16 (0.0), "channel color b", ;);
              WRITE_RETURN (gint16, output, 0,                     "channel color padding", ;);
              WRITE_RETURN (gint16, output, 100,                   "channel opacity", ;);
              WRITE_RETURN (gchar, output, 1,                      "channel mode", ;);
            }
          ...
          for (iter = PSDImageData.lChannels; iter; iter = g_list_next (iter))
            {
              ...
              WRITE_RETURN (gint16, output, DOUBLE_TO_INT16 (rgb[0]), "channel color r",
                            g_object_unref (color));
              WRITE_RETURN (gint16, output, DOUBLE_TO_INT16 (rgb[1]), "channel color g",
                            g_object_unref (color));
              WRITE_RETURN (gint16, output, DOUBLE_TO_INT16 (rgb[2]), "channel color b",
                            g_object_unref (color));
              WRITE_RETURN (gint16, output, 0,                        "channel color padding",
                            g_object_unref (color));
              WRITE_RETURN (gint16, output, ROUND (opacity),          "channel opacity",
                            g_object_unref (color));
              WRITE_RETURN (gchar, output, 1,                         "channel mode",
                            g_object_unref (color));
  ...
  if (gimp_image_find_next_guide (image, 0))
    {
      ...
      XFWRITE_RETURN (output, "8BIM", 4, "imageresources signature", ;);
      WRITE_RETURN (gint16, output, 0x0408, "0x0408 Id (Guides)", ;); /* 1032 */
      /* write_pascalstring (output, Name, "Id name"); */
      WRITE_RETURN (gint16, output, 0, "Id name", ;); /* Set to null string (two zeros) */
      WRITE_RETURN (gint32, output, 16 + 5 * n_guides, "0x0408 resource size", ;);
      /* Save grid and guide header */
      WRITE_RETURN (gint32, output,   1, "grid/guide header version", ;);
      WRITE_RETURN (gint32, output, 576, "grid custom spacing horizontal", ;);/* dpi*32/4??*/
      WRITE_RETURN (gint32, output, 576, "grid custom spacing vertical", ;);  /* dpi*32/4??*/
      WRITE_RETURN (gint32, output, n_guides, "number of guides", ;);
      ...
      while ((guide_id = gimp_image_find_next_guide (image, guide_id)))
        {
          ...
          WRITE_RETURN (gint32, output, position, "Position of guide", ;);
          WRITE_RETURN (gchar, output, orientation, "Orientation of guide", ;);
          n_guides--;
        }
      if ((g_seekable_tell (G_SEEKABLE (output)) & 1))
        WRITE_RETURN (gchar, output, 0, "pad byte", ;);
  ...
  /* --------------- Write paths ------------------- */
  if (! save_paths (output, image, error))
    return FALSE;
  ...
  if (options->clipping_path)
    {
      ...
      if (! save_clipping_path (output, image,
  ...
  /* --------------- Write resolution data ------------------- */
  {
    ...
    XFWRITE_RETURN (output, "8BIM", 4, "imageresources signature (for resolution)", ;);
    WRITE_RETURN (gint16, output, 0x03ed, "0x03ed Id (resolution)", ;); /* 1005 */
    WRITE_RETURN (gint16, output, 0, "Id name", ;); /* Set to null string (two zeros) */
    WRITE_RETURN (gint32, output, 16, "0x0400 resource size", ;);
    WRITE_RETURN (gint32, output, xres_fix, "hRes (16.16 fixed point)", ;);
    WRITE_RETURN (gint16, output, psd_unit, "hRes unit", ;);
    WRITE_RETURN (gint16, output, psd_unit, "width unit", ;);
    WRITE_RETURN (gint32, output, yres_fix, "vRes (16.16 fixed point)", ;);
    WRITE_RETURN (gint16, output, psd_unit, "vRes unit", ;);
    WRITE_RETURN (gint16, output, psd_unit, "height unit", ;);
  }
  ...
  if (SelLayers)
    {
              ...
              if (SelLayers->data == ((PSD_Layer *) iter->data)->layer)
                {
                  ...
                  WRITE_RETURN (gint16, output, 0x0400, "0x0400 Id", /* 1024 */
                                g_list_free (SelLayers));
                  /* write_pascalstring (output, Name, "Id name"); */
                  WRITE_RETURN (gint16, output, 0, "Id name", /* Set to null string (two zeros) */
                                g_list_free (SelLayers));
                  WRITE_RETURN (gint32, output, sizeof (gint16), "0x0400 resource size",
                                g_list_free (SelLayers));

                  /* Layer State Information uses the layer index. */
                  WRITE_RETURN (gint16, output, PSDImageData.nLayers - i - 1, "active layer",
                                g_list_free (SelLayers));
      ...
      XFWRITE_RETURN (output, "8BIM", 4, "imageresources signature",
                      g_list_free (SelLayers));
      WRITE_RETURN (gint16, output, 0x042D, "0x042D Id", /* 1069 */
                    g_list_free (SelLayers));
      WRITE_RETURN (gint16, output, 0, "Id name", /* Set to null string (two zeros) */
                    g_list_free (SelLayers));
      WRITE_RETURN (gint32, output,
                    sizeof (gint16) + sizeof (gint32) * g_list_length (SelLayers),
                    "0x0400 resource size",
                    g_list_free (SelLayers));
      WRITE_RETURN (gint16, output, g_list_length (SelLayers), "2 bytes count",
                    g_list_free (SelLayers));
      for (iter = SelLayers; iter; iter = iter->next)
        WRITE_RETURN (gint32, output,
                      GPOINTER_TO_INT (gimp_item_get_tattoo (iter->data)),
                      "4 bytes layer ID",
                      g_list_free (SelLayers));
    }
  g_list_free (SelLayers);
    ...
    if (profile)
      {
        ...
        WRITE_RETURN (gint16, output, 0x040f, "0x040f Id",
                      g_object_unref (profile));
        WRITE_RETURN (gint16, output, 0, "Id name", /* Set to null string (two zeros) */
                      g_object_unref (profile));
        WRITE_RETURN (gint32, output, icc_length, "0x040f resource size",
                      g_object_unref (profile));
        XFWRITE_RETURN (output, icc_data, icc_length, "ICC profile",
                        g_object_unref (profile));
                    ...
                    GError        **error)
{
  ...
  xfwrite (output, data->str, data->len, "clipping path resources data", error);
  g_string_free (data, TRUE);
                     ...
                     GError             **error)
{
  ...
  if (! options->psb)
    WRITE_RETURN (gint32, output, 0, "layers & mask information length", ;);
  else
    WRITE_RETURN (gint64, output, 0, "layers & mask information length", ;);
  ...
  /* AmmoOS Image layers must be written in reverse order */
  ...
  for (iter = g_list_last (PSDImageData.lLayers), i = PSDImageData.nLayers;
       ...
       iter = g_list_previous (iter), i--)
    {
      ...
      WRITE_RETURN (gint32, output, offset_y,               "Layer top",    ;);
      WRITE_RETURN (gint32, output, offset_x,               "Layer left",   ;);
      WRITE_RETURN (gint32, output, offset_y + layerHeight, "Layer bottom", ;);
      WRITE_RETURN (gint32, output, offset_x + layerWidth,  "Layer right",  ;);
       ...
       * Can be removed once CMYK channels are accessible in AmmoOS Image
       */
      if (options->cmyk)
        {
          nChannelsLayer =
      ...
      WRITE_RETURN (gint16, output, nChannelsLayer, "Number channels in the layer", ;);
      ...
      for (j = 0; j < nChannelsLayer; j++)
        {
          ...
          WRITE_RETURN (gint16, output, idChannel, "Channel ID", ;);
      ...
      WRITE_RETURN (gchar, output, flags, "Flags", ;);

      /* Padding byte to make the length even */
      WRITE_RETURN (gchar, output, 0, "Filler", ;);

      ExtraDataPos = g_seekable_tell (G_SEEKABLE (output)); /* Position of Extra Data size */
      WRITE_RETURN (gint32, output, 0, "Extra data size", ;);
      ...
      if (hasMask)
        {
          ...
          WRITE_RETURN (gint32, output, 20,                        "Layer mask size", ;);
          WRITE_RETURN (gint32, output, maskOffset_y,              "Layer mask top", ;);
          WRITE_RETURN (gint32, output, maskOffset_x,              "Layer mask left", ;);
          WRITE_RETURN (gint32, output, maskOffset_y + maskHeight, "Layer mask bottom", ;);
          WRITE_RETURN (gint32, output, maskOffset_x + maskWidth,  "Layer mask right", ;);
          WRITE_RETURN (gchar,  output, 0,                         "Layer mask default color", ;);
          flags = (0                    |  /* position relative to layer */
                   (apply ? 0 : 1) << 1 |  /* layer mask disabled        */
                   0 << 2);                /* invert layer mask          */
          WRITE_RETURN (gchar,  output, flags,                     "Layer mask flags", ;);
          WRITE_RETURN (gint16, output, 0,                         "Layer mask Padding", ;);
        }
      else
        {
          /* NOTE Writing empty Layer mask / adjustment layer data */
          WRITE_RETURN (gint32, output, 0, "Layer mask size", ;);
          IFDBG(1) g_debug ("\t\tLayer mask size: %d", 0);
        }

      /* NOTE Writing empty Layer blending ranges data */
      WRITE_RETURN (gint32, output, 0, "Layer blending size", ;);
      ...
      /* Unicode layer name */
      if (! write_datablock_luni (output, layerName, "luni extra data block", error))
        return FALSE;

      g_free (layerName);

      /* Layer ID */
      XFWRITE_RETURN (output, "8BIMlyid", 8, "lyid signature", ;);
      WRITE_RETURN (gint32, output, 4, "lyid size", ;);
      WRITE_RETURN (gint32, output, gimp_item_get_tattoo (GIMP_ITEM (psd_layer->layer)), "Layer ID", ;);

      /* Layer color tag */
      XFWRITE_RETURN (output, "8BIMlclr", 8, "sheet color signature", ;);
      WRITE_RETURN (gint32, output, 8, "sheet color size", ;);
      WRITE_RETURN (gint16, output,
                    gimp_to_psd_layer_color_tag (gimp_item_get_color_tag (GIMP_ITEM (psd_layer->layer))),
                    "sheet color code", ;);
      WRITE_RETURN (gint16, output, 0, "sheet color unused value", ;);
      WRITE_RETURN (gint16, output, 0, "sheet color unused value", ;);
      WRITE_RETURN (gint16, output, 0, "sheet color unused value", ;);
      ...
      if (psd_layer->type != PSD_LAYER_TYPE_LAYER)
        {
          ...
          if (type < 3 || depth <= 5)
            {
              XFWRITE_RETURN (output, "8BIMlsct", 8, "section divider", ;);
            }
          else
            {
               ...
               */
              XFWRITE_RETURN (output, "8BIMlsdk", 8, "nested section divider", ;);
            }
          WRITE_RETURN (gint32, output, size, "section divider size", ;);
          WRITE_RETURN (gint32, output, type, "section divider type", ;);
          XFWRITE_RETURN (output, "8BIM", 4, "section divider blend mode signature", ;);
          XFWRITE_RETURN (output, blendMode, 4, "section divider blend mode key", ;);
        }
      ...
      if (! g_seekable_seek (G_SEEKABLE (output), ExtraDataPos, G_SEEK_SET, NULL, error))
        return FALSE;
  ...
  for (iter = g_list_last (PSDImageData.lLayers), i = PSDImageData.nLayers;
       ...
       iter = g_list_previous (iter), i--)
    {
      ...
      if (! write_pixel_data (output, image, GIMP_DRAWABLE (psd_layer->layer), ChannelLengthPos[i-1], 0,
  ...
  if (! g_seekable_seek (G_SEEKABLE (output), LayerMaskPos, G_SEEK_SET, NULL, error))
    return FALSE;

  if (! options->psb)
    {
      WRITE_RETURN (gint32, output, eof_pos - LayerMaskPos - sizeof (gint32),
    ...
    {
      WRITE_RETURN (gint64, output, eof_pos - LayerMaskPos - sizeof (gint64),
                    "layers & mask information length", ;);
  ...
  for (i = 0; i < components; i++)
    {
      ...
      if (ChanLenPosition)
        {
          WRITE_GOTO (gint16, output, 1, "Compression type (RLE)");
          len += 2;
        }
      ...
      if (ltable_offset > 0)
      ...
      else
        {
          ...
          if (! options->psb)
            {
              XFWRITE_GOTO (output, LengthsTable, height * sizeof (gint16),
                            "Dummy RLE length");
              len += height * sizeof (gint16);
            }
          else
            {
              XFWRITE_GOTO (output, LengthsTable, height * sizeof (gint32),
                            "Dummy RLE length");
      ...
      for (y = 0; y < height; y += tile_height)
        {
          ...
          tlen = get_compress_channel_data (&data[chan * bpc],
                                            ...
                                            rledata);
          len += tlen;
          XFWRITE_GOTO (output, rledata, tlen, "Compressed pixel data");
          IFDBG(3) g_debug ("\t\t\t\t. Writing compressed pixels, stream of %d", tlen);
        }

      /* Write compressed lengths table */
      if (! g_seekable_seek (G_SEEKABLE (output), length_table_pos, G_SEEK_SET, NULL, error))
        goto clean;

      for (j = 0; j < height; j++) /* write real length table */
        {
          if (! options->psb)
            WRITE_GOTO (gint16, output, LengthsTable[j], "RLE length");
          else
            WRITE_GOTO (gint32, output, LengthsTable[j], "RLE length");
        }
      ...
      if (ChanLenPosition)    /* Update total compressed length */
        {
          ...
          if (! options->psb)
            WRITE_GOTO (gint32, output, len, "channel data length");
          else
            WRITE_GOTO (gint64, output, len, "channel data length");
          IFDBG(1) g_debug ("\t\tUpdating data len to %" G_GSIZE_FORMAT, len);
        }

      if (! g_seekable_seek (G_SEEKABLE (output), 0, G_SEEK_END, NULL, error))
        goto clean;
  ...
  if (mask != NULL)
    {
      ...
      mbuffer = gimp_drawable_get_buffer (GIMP_DRAWABLE (mask));
      ...
      for (y = 0; y < height; y += tile_height)
        {
          ...
          tlen = get_compress_channel_data (&data[0],
                                            ...
                                            rledata);
          len += tlen;
          XFWRITE_GOTO (output, rledata, tlen, "Compressed mask data");
      ...
      g_seekable_seek (G_SEEKABLE (output), 0, G_SEEK_END, NULL, error);
           ...
           GError             **error)
{
  ...
  WRITE_RETURN (gint16, output, 1, "RLE compression", ;);
  ...
  for (i = 0; i < ChanCount; i++)
    for (j = 0; j < imageHeight; j++)
      {
        if (! options->psb)
          WRITE_RETURN (gint16, output, 0, "junk line lengths", ;);
        else
          WRITE_RETURN (gint32, output, 0, "junk line lengths", ;);
      }

  IFDBG(1) g_debug ("\t\tWriting compressed image data");
  if (! write_pixel_data (output, image, GIMP_DRAWABLE (PSDImageData.merged_layer),
                          NULL, offset, FALSE, options, error))
    return FALSE;
  ...
  for (iter = PSDImageData.lChannels; iter; iter = g_list_next (iter))
    {
      ...
      if (! write_pixel_data (output, image, iter->data, NULL,
                              offset + bpc * imageHeight * chan, FALSE,
                              options, error)) /* check how imgs are channels here */
        return FALSE;
              ...
              GError        **error)
{
  ...
  if (! save_header (output, image, &resource_options, error)                 ||
      ! save_color_mode_data (output, image, resource_options.duotone, error) ||
      ! save_resources (output, image, &resource_options, error))
    goto clean;
