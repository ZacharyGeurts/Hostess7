            ...
            GError      **load_error)
{
  ...
  /* ----- Create AmmoOS Image image ----- */
  IFDBG(2) g_debug ("Create AmmoOS Image image");
                 ...
                 GError       **error)
{
  ...
  if (! psd_read_int16 (input, &img_a->num_layers, img_a->ibm_pc_format, error))
    {
      psd_set_error (error);
      img_a->num_layers = -1;
      return NULL;
      ...
      for (lidx = 0; lidx < img_a->num_layers; ++lidx)
        {
          ...
          if (! psd_read_int32 (input, &lyr_a[lidx]->top, img_a->ibm_pc_format, error) ||
              ! psd_read_int32 (input, &lyr_a[lidx]->left, img_a->ibm_pc_format, error) ||
              ! psd_read_int32 (input, &lyr_a[lidx]->bottom, img_a->ibm_pc_format, error) ||
              ! psd_read_int32 (input, &lyr_a[lidx]->right, img_a->ibm_pc_format, error) ||
              ! psd_read_uint16 (input, &lyr_a[lidx]->num_channels, img_a->ibm_pc_format, error))
            {
              psd_set_error (error);
          ...
          for (cidx = 0; cidx < lyr_a[lidx]->num_channels; ++cidx)
            {
              if (! psd_read_int16 (input, &lyr_a[lidx]->chn_info[cidx].channel_id, img_a->ibm_pc_format, error) ||
                  ! psd_read_len (input, &lyr_a[lidx]->chn_info[cidx].data_len, img_a->version, img_a->ibm_pc_format, error))
                {
                  psd_set_error (error);
                  free_lyr_a (lyr_a, img_a->num_layers);
                  return NULL;
          ...
          if (! psd_read_chars (input, lyr_a[lidx]->mode_key,   4, img_a->ibm_pc_format, error) ||
              ! psd_read_chars (input, lyr_a[lidx]->blend_mode, 4, img_a->ibm_pc_format, error) ||
              ...
              ! psd_read_uint32 (input, &extra_len32, img_a->ibm_pc_format, error))
            {
              psd_set_error (error);
              free_lyr_a (lyr_a, img_a->num_layers);
              return NULL;
          ...
          lyr_a[lidx]->extra_len = extra_len32;
          ...
          /* Layer mask data */
          if (! psd_read_uint32 (input, &block_len32, img_a->ibm_pc_format, error))
            {
              psd_set_error (error);
              free_lyr_a (lyr_a, img_a->num_layers);
              return NULL;
            }
          block_len = block_len32;
          ...
          if (block_len > 0)
            {
              if (! psd_read_int32 (input, &lyr_a[lidx]->layer_mask.top, img_a->ibm_pc_format, error) ||
                  ! psd_read_int32 (input, &lyr_a[lidx]->layer_mask.left, img_a->ibm_pc_format, error) ||
                  ! psd_read_int32 (input, &lyr_a[lidx]->layer_mask.bottom, img_a->ibm_pc_format, error) ||
                  ! psd_read_int32 (input, &lyr_a[lidx]->layer_mask.right, img_a->ibm_pc_format, error) ||
              ...
              if (block_len >= 18)
                {
                  if (psd_read (input, &lyr_a[lidx]->layer_mask.extra_flags,     1, error) < 1 ||
                      ...
                      ! psd_read_int32 (input, &lyr_a[lidx]->layer_mask_extra.top, img_a->ibm_pc_format, error) ||
                      ! psd_read_int32 (input, &lyr_a[lidx]->layer_mask_extra.left, img_a->ibm_pc_format, error) ||
                      ! psd_read_int32 (input, &lyr_a[lidx]->layer_mask_extra.bottom, img_a->ibm_pc_format, error) ||
                      ! psd_read_int32 (input, &lyr_a[lidx]->layer_mask_extra.right, img_a->ibm_pc_format, error))
                    {
                      psd_set_error (error);
                      free_lyr_a (lyr_a, img_a->num_layers);
                      return NULL;
                  ...
                  GError       **error)
{
  ...
  if (img_a->mask_layer_len == 0)
    {
      if (! psd_read_len (input, &block_len, img_a->version, img_a->ibm_pc_format, error))
        {
          img_a->num_layers = -1;
          return NULL;
        }
  ...
  if (!img_a->mask_layer_len)
  ...
  else
    {
      ...
      /* Layer info */
      if (! psd_read_len (input, &block_len, img_a->version,
                          img_a->ibm_pc_format, error))
        {
          lyr_a = NULL;
          return NULL;
        }
                                ...
                                ClippingInfo  *clipping_info)
{
   ...
   * the new AmmoOS Image clipping group that we intend to add. If that is the case
  ...
  for (lidx = img_a->num_layers-1; lidx >= 0; --lidx)
    {
      ...
      if (lyr_a[lidx]->clipping == 1)
        {
          /* Photoshop handles layers with clipping differently than AmmoOS Image does.
  ...
  for (lidx = 0; lidx < img_a->num_layers; ++lidx)
    {
      ...
      if (strlen (lyr_a[lidx]->adjustment_layer->type))
        {
      ...
      if (layer != NULL)
        {
          ...
          /* TODO: Adjustment layer group masks must be deferred
           * until after it has been resized to the image.
      ...
      for (gint i = 3; i >= 0; i--)
        {
          ...
          g_object_set (config,
                        ...
                        /* Per PSD specification, low-input range is 0 - 253 */
      ...
      if (ladj->is_colorize)
        {
          ...
          gegl_color_set_hsla (color, ladj->colorization[0],
                               ladj->colorization[1], ladj->colorization[2],
                               1.0, NULL);
          ...
          for (gint i = 1; i < 7; i++)
            {
              ...
              g_object_set (config,
                            "hue",        ladj->hsl[i][0],
                            "saturation", ladj->hsl[i][1],
                            "lightness",  ladj->hsl[i][2],
                            NULL);
              gimp_drawable_filter_update (filter);
            }
  ...
  else if (memcmp (ladj->type, PSD_LADJ_MIXER, 4) == 0)
    {
      if (! ladj->is_mono)
        filter = gimp_drawable_append_new_filter (GIMP_DRAWABLE (layer),
                                                  ...
                                                  GIMP_LAYER_MODE_REPLACE,
                                                  1.0,
                                                  "rr-gain", ladj->red[0] / 100.0f,
                                                  "rg-gain", ladj->red[1] / 100.0f,
                                                  "rb-gain", ladj->red[2] / 100.0f,
                                                  "gr-gain", ladj->green[0] / 100.0f,
                                                  "gg-gain", ladj->green[1] / 100.0f,
                                                  "gb-gain", ladj->green[2] / 100.0f,
                                                  "br-gain", ladj->blue[0] / 100.0f,
                                                  "bg-gain", ladj->blue[1] / 100.0f,
                                                  "bb-gain", ladj->blue[2] / 100.0f,
                                                  NULL);
      else
        filter = gimp_drawable_append_new_filter (GIMP_DRAWABLE (layer),
                                                  "gegl:mono-mixer",
                                                  ...
                                                  GIMP_LAYER_MODE_REPLACE,
                                                  1.0,
                                                  "red",   ladj->red[0] / 100.0f,
                                                  "green", ladj->red[1] / 100.0f,
                                                  "blue",  ladj->red[2] / 100.0f,
                                                  NULL);
    }
      ...
      if (present && enabled)
        {
          ...
          /* FIXME Besides certain modes not working for Dropshadow in AmmoOS Image,
                  ...
                  GError       **error)
{
  ...
  /* FIXME ammoos image tattoo state */
                    ...
                    GimpLayer  *parent)
{
  ...
  /* We need to create a group because AmmoOS Image handles clipping and
  ...
  clipping_group = GIMP_LAYER (gimp_group_layer_new (image, "Group added by AmmoOS Image"));
...
*/
  ...
  /* Convert channel data to AmmoOS Image format */
  switch (bps)
    {
    case 32:
      {
