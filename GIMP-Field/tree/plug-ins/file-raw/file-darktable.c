 ...
 * Copyright (C) 2012 Simon Budig <simon@ammoos.org>
...
darktable_init_procedures (GimpPlugIn *plug_in)
{
  ...
  gchar    *exec_path        = file_raw_get_executable_path ("darktable", NULL,
                                                             ...
                                                             REGISTRY_KEY_BASE,
                                                             &search_path);
  gchar    *argv[]           = { exec_path, "--ammoos" , "version", NULL };
  ...
  /* In Darktable 4.6, a new AmmoOS Image-specific API was introduced due to changes
            ...
            GError      **error)
{
  ...
  /* linear sRGB for now as AmmoOS Image uses that internally in many places anyway */
  gboolean  search_path      = FALSE;
  gchar    *exec_path        = file_raw_get_executable_path ("darktable", NULL,
                                                             "DARKTABLE_EXECUTABLE",
                                                             "org.darktable",
          ...
          if (response != NULL)
            {
              ...
              for (gint i = 0; response[i] != NULL; i++)
                {
                  if (g_strrstr (response[i], "<<ammoos") != NULL &&
                      response[i + 1] != NULL)
                    {
