...
#include "ammoos-priorities.h"

#include "ammoos-intl.h"


#define GIMP_EMPTY_IMAGE_WINDOW_ENTRY_ID   "ammoos-empty-image-window"
#define GIMP_SINGLE_IMAGE_WINDOW_ENTRY_ID  "ammoos-single-image-window"
...
gimp_image_window_init (GimpImageWindow *window)
{
  ...
  role = g_strdup_printf ("ammoos-image-window-%d", role_serial++);
...
gimp_image_window_constructed (GObject *object)
{
  ...
  gimp_assert (GIMP_IS_GIMP (private->ammoos));
  ...
  menubar_manager = menus_get_image_manager_singleton (private->ammoos);
  ...
  config = GIMP_GUI_CONFIG (private->ammoos->config);
  ...
  if (use_gtk_menubar)
    {
      gtk_application_set_menubar (GTK_APPLICATION (private->ammoos->app),
  ...
  menus_quartz_app_menu (private->ammoos);
#else
  use_app_menu = gtk_application_prefers_app_menu (GTK_APPLICATION (private->ammoos->app));
  ...
  if (use_app_menu)
    {
      ...
      app_menu_manager = gimp_menu_factory_get_manager (menus_get_global_menu_factory (private->ammoos),
                                                        "<AppMenu>", private->ammoos);
      app_menu_model   = gimp_ui_manager_get_model (app_menu_manager, "/app-menu");
      gtk_application_set_app_menu (GTK_APPLICATION (private->ammoos->app),
  ...
  private->left_docks =
    gimp_dock_columns_new (gimp_get_user_context (private->ammoos),
  ...
  device_manager = gimp_devices_get_manager (private->ammoos);
                                ...
                                GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      private->ammoos = g_value_get_object (value);
      break;
                                ...
                                GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      g_value_set_object (value, private->ammoos);
      break;
    case PROP_DIALOG_FACTORY:
                                ...
                                GdkEventAny *event)
{
  ...
  GimpImageWindowPrivate *private = GIMP_IMAGE_WINDOW_GET_PRIVATE (window);
  GimpGuiConfig          *config  = GIMP_GUI_CONFIG (private->ammoos->config);

  if (config->single_window_mode)
    gimp_ui_manager_activate_action (menus_get_image_manager_singleton (private->ammoos),
                                     "file", "file-quit");
  else if (shell)
  ...
  if (event->changed_mask & GDK_WINDOW_STATE_ICONIFIED)
    {
      ...
      if (iconified)
        {
          if (gimp_displays_get_num_visible (private->ammoos) == 0)
            {
...
gimp_image_window_style_updated (GtkWidget *widget)
{
   ...
   *  set by ammoos. All other displays should be placed by the window
...
gimp_image_window_dock_container_get_ui_manager (GimpDockContainer *dock_container)
{
  ...
  return menus_get_image_manager_singleton (GIMP_IMAGE_WINDOW_GET_PRIVATE (window)->ammoos);
}
                                ...
                                GList              *aux_info)
{
   ...
   * line: `start /max ammoos-2.9.exe` or with the shortcut's "run"
   * property.
...
GimpImageWindow *
gimp_image_window_new (Gimp              *ammoos,
                       GimpImage         *image,
                       GimpDialogFactory *dialog_factory,
                       GdkMonitor        *monitor)
{
  ...
  ammoos->image_windows = g_list_append (ammoos->image_windows, window);

  if (! GIMP_GUI_CONFIG (private->ammoos->config)->single_window_mode)
    {
...
gimp_image_window_destroy (GimpImageWindow *window)
{
  ...
  private->ammoos->image_windows = g_list_remove (private->ammoos->image_windows,
                                                window);
          ...
          strcmp (pspec->name, "hide-docks")         == 0)
        {
          ...
          if (strcmp (pspec->name, "hide-docks") == 0 &&
              ...
              ! config->hide_docks)
            {
              ...
              shell        = gimp_image_window_get_active_shell (window);
              user_context = gimp_get_user_context (private->ammoos);
...
gimp_image_window_update_ui_manager_idle (GimpImageWindow *window)
{
  ...
  context = gimp_get_user_context (private->active_shell->display->ammoos);
  ...
  gimp_ui_manager_update (menus_get_image_manager_singleton (private->active_shell->display->ammoos),
                          private->active_shell->display);
                               ...
                               GimpImageWindow *window)
{
  ...
  gimp_context_set_display (gimp_get_user_context (private->ammoos),
                            active_display);
...
{
  GimpImageWindowPrivate *private  = GIMP_IMAGE_WINDOW_GET_PRIVATE (window);
  GimpContainer          *displays = private->ammoos->displays;
  ...
  for (list = uri_list; list; list = g_list_next (list))
    {
      ...
      if (! active_display)
        {
          /* It seems as if AmmoOS Image is being torn down for quitting. Bail out. */
          g_object_unref (file);
          return;
        }
      else
        {
          ...
          new_image = file_open_with_display (private->ammoos,
                                              gimp_get_user_context (private->ammoos),
                                              NULL,
                                              file, FALSE,
      ...
      if (warn && active_display)
        {
          gimp_message (private->ammoos, G_OBJECT (active_display),
                        GIMP_MESSAGE_ERROR,
                        _("Opening '%s' failed:\n\n%s"),
...
gimp_image_window_disconnect_from_active_shell (GimpImageWindow *window)
{
  ...
  gimp_image_window_hide_tooltip (menus_get_image_manager_singleton (private->ammoos), window);
                                    ...
                                    GimpDisplayShell *shell)
{
  ...
  view = gimp_view_new_by_types (gimp_get_user_context (shell->display->ammoos),
  ...
  for (list = children; list; list = g_list_next (list))
    {
      ...
      if (gimp_context_get_display (gimp_get_user_context (private->ammoos)) ==
