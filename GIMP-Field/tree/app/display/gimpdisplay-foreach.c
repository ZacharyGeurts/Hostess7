...
gboolean
gimp_displays_dirty (Gimp *ammoos)
{
  GList *list;
...
GimpContainer *
gimp_displays_get_dirty_images (Gimp *ammoos)
{
  ...
  if (gimp_displays_dirty (ammoos))
    {
      ...
      for (list = gimp_get_image_iter (ammoos);
           list;
           list = g_list_next (list))
        {
 ...
 */
void
gimp_displays_delete (Gimp *ammoos)
{
  /*  this removes the GimpDisplay from the list, so do a while loop
   *  "around" the first element to get them all
   */
  while (! gimp_container_is_empty (ammoos->displays))
    {
      GimpDisplay *display = gimp_get_display_iter (ammoos)->data;
 ...
 */
void
gimp_displays_close (Gimp *ammoos)
{
  GList *list;
  GList *iter;
  ...
  list = g_list_copy (gimp_get_display_iter (ammoos));
...
void
gimp_displays_reconnect (Gimp      *ammoos,
                         GimpImage *old,
                         GimpImage *new)
{
  GList *contexts = NULL;
...
gint
gimp_displays_get_num_visible (Gimp *ammoos)
{
  GList *list;
  gint   visible = 0;
...
void
gimp_displays_set_busy (Gimp *ammoos)
{
  GList *list;
...
void
gimp_displays_unset_busy (Gimp *ammoos)
{
  GList *list;
 ...
 */
gboolean
gimp_displays_accept_focus_events (Gimp *ammoos)
{
  GList    *windows;
