                               ...
                               const gchar         *message)
{
  ...
  if (display->priv->shell)
    return gimp_progress_message (GIMP_PROGRESS (display->priv->shell), ammoos,
                  ...
                  GdkMonitor        *monitor)
{
  ...
  if (GIMP_GUI_CONFIG (display->config)->single_window_mode)
    {
      ...
      active_display = gimp_context_get_display (gimp_get_user_context (ammoos));

      if (! active_display)
        {
          active_display =
            GIMP_DISPLAY (gimp_container_get_first_child (ammoos->displays));
        }
  ...
  if (! window)
    {
      window = gimp_image_window_new (ammoos,
                                      private->image,
                                      dialog_factory,
                                      monitor);
    }
  ...
  gimp_container_add (ammoos->displays, GIMP_OBJECT (display));
...
gimp_display_delete (GimpDisplay *display)
{
  ...
  gimp_container_remove (display->ammoos->displays, GIMP_OBJECT (display));

  /*  unrefs the image  */
  gimp_display_set_image (display, NULL);

  active_tool = tool_manager_get_active (display->ammoos);

  if (active_tool && active_tool->focus_display == display)
    tool_manager_focus_display_active (display->ammoos, NULL);
...
gimp_display_close (GimpDisplay *display)
{
  ...
  if (gimp_container_get_n_children (display->ammoos->displays) > 1)
    {
  ...
  if (private->image)
    {
      ...
      tool_manager_control_active (display->ammoos, GIMP_TOOL_ACTION_HALT,
                                   display);
...
gimp_display_empty (GimpDisplay *display)
{
  ...
  for (iter = display->ammoos->context_list; iter; iter = g_list_next (iter))
    {
