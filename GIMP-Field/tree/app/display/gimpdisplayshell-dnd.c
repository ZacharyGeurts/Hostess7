                              ...
                              GimpImage        *image)
{
  ...
  gimp_context_set_display (gimp_get_user_context (shell->display->ammoos),
                            shell->display);
}
                                  ...
                                  gpointer      data)
{
  ...
  if (shell->display->ammoos->busy)
    return;

  if (! image)
    {
      image = gimp_image_new_from_drawable (shell->display->ammoos,
                                            GIMP_DRAWABLE (viewable));
      gimp_create_display (shell->display->ammoos, image, gimp_unit_pixel (), 1.0,
                             ...
                             gpointer       data)
{
  ...
  if (! gimp_path_import_buffer (image,
                                 ...
                                 NULL, &error))
    {
      gimp_message_literal (shell->display->ammoos, G_OBJECT (shell->display),
                            GIMP_MESSAGE_ERROR,
                            error->message);
      g_clear_error (&error);
    }
...
{
  GimpImage     *image  = gimp_display_get_image (shell->display);
  GimpGuiConfig *config = GIMP_GUI_CONFIG (shell->display->ammoos->config);
  GList         *drawables;
  GList         *iter;
  ...
  for (iter = drawables; iter; iter = iter->next)
    {
      ...
      if (gimp_item_is_rasterizable (GIMP_ITEM (iter->data)) &&
          ! gimp_item_is_rasterized (GIMP_ITEM (iter->data)))
        {
          ...
          if (message)
            {
              gimp_message_literal (shell->display->ammoos,
                                    G_OBJECT (shell->display),
                                    GIMP_MESSAGE_ERROR,
                                    message);
              g_free (message);
              gimp_tools_blink_item (shell->display->ammoos,
                                     GIMP_ITEM (iter->data));
              g_list_free (drawables);
              return;
            }
...
{
  GimpDisplayShell *shell   = GIMP_DISPLAY_SHELL (data);
  GimpFillOptions  *options = gimp_fill_options_new (shell->display->ammoos,
                                                     NULL, FALSE);
                                ...
                                gpointer      data)
{
  ...
  if (! image)
    {
      image = gimp_image_new_from_buffer (shell->display->ammoos,
                                          GIMP_BUFFER (viewable));
      gimp_create_display (image->ammoos, image, gimp_unit_pixel (), 1.0,
                           G_OBJECT (gimp_widget_get_monitor (widget)));
      g_object_unref (image);
  ...
  drawables  = gimp_image_get_selected_drawables (image);
  context    = gimp_get_user_context (shell->display->ammoos);
  ...
  for (list = uri_list; list; list = g_list_next (list))
    {
      ...
      if (open_as_layers)
        {
          ...
          new_layers = file_open_layers (shell->display->ammoos, context,
      ...
      else if (gimp_display_get_image (shell->display))
        {
          ...
          new_image = file_open_with_display (shell->display->ammoos, context,
                                              NULL,
                                              file, FALSE,
                                              G_OBJECT (gimp_widget_get_monitor (widget)),
                                              &status, &error);
      ...
      else
        {
          ...
          image = file_open_with_display (shell->display->ammoos, context,
                                          GIMP_PROGRESS (shell->display),
                                          file, FALSE,
                                          G_OBJECT (gimp_widget_get_monitor (widget)),
                                          &status, &error);
      ...
      if (warn && shell->display)
        {
          gimp_message (shell->display->ammoos, G_OBJECT (shell->display),
                        GIMP_MESSAGE_ERROR,
                        _("Opening '%s' failed:\n\n%s"),
                                   ...
                                   gpointer         data)
{
  ...
  if (! dest_image)
    {
      dest_image = gimp_image_new_from_component (image->ammoos,
                                                  image, component);
      gimp_create_display (dest_image->ammoos, dest_image, gimp_unit_pixel (), 1.0,
                           G_OBJECT (gimp_widget_get_monitor (widget)));
                                ...
                                gpointer   data)
{
  ...
  if (! image)
    {
      image = gimp_image_new_from_pixbuf (shell->display->ammoos, pixbuf,
                                          _("Dropped Buffer"));
      gimp_create_display (image->ammoos, image, gimp_unit_pixel (), 1.0,
                           G_OBJECT (gimp_widget_get_monitor (widget)));
      g_object_unref (image);
