...
#include "core/ammoos-filter-history.h"
...
static void       gimp_display_shell_activate_action          (Gimp              *ammoos,
                           ...
                           GimpDisplayShell *shell)
{
  ...
  ammoos = gimp_display_get_gimp (shell->display);
   ...
   */
  if (! gimp_displays_accept_focus_events (ammoos))
    return TRUE;
  ...
  if (set_display)
    gimp_context_set_display (gimp_get_user_context (ammoos), shell->display);
    ...
    case GDK_2BUTTON_PRESS:
      {
        ...
        if (bevent->button == 1)
          {
            GimpUIManager *manager = menus_get_image_manager_singleton (shell->display->ammoos);
                                       ...
                                       GimpDisplayShell *shell)
{
  ...
  ammoos    = gimp_display_get_gimp (display);
    ...
    case GDK_ENTER_NOTIFY:
      {
        ...
        tool_manager_oper_update_active (ammoos,
                                         &image_coords, state,
                                         shell->proximity,
                                         display);
      }
        ...
        if (bevent->button == 1)
          {
            ...
            if (gimp_display_shell_initialize_tool (shell,
                                                    &image_coords, state))
              {
                ...
                tool_manager_button_press_active (ammoos,
                                                  &image_coords,
                                                  time, state,
        ...
        else
          {
            ...
            switch (action)
              ...
              case GIMP_MODIFIER_ACTION_MENU:
                gimp_display_triggers_context_menu (event, shell, ammoos, &image_coords, TRUE);
                shell->mod_action = GIMP_MODIFIER_ACTION_NONE;
                break;
              ...
              case GIMP_MODIFIER_ACTION_NONE:
                gimp_display_triggers_context_menu (event, shell, ammoos, &image_coords, FALSE);
                break;
              }
          }
            ...
            if (active_tool &&
                 ...
                 gimp_tool_control_get_handle_empty_image (active_tool->control)))
              {
                ...
                if (gimp_tool_control_is_active (active_tool->control))
                  {
                    tool_manager_button_release_active (ammoos,
                                                        &image_coords,
                                                        time, state,
                                                        display);
                  }
        ...
        else
          {
            switch (shell->mod_action)
              ...
              case GIMP_MODIFIER_ACTION_TOOL_OPACITY:
                ...
                break;
              case GIMP_MODIFIER_ACTION_ACTION:
                gimp_display_shell_activate_action (ammoos, shell->mod_action_desc, NULL);
    ...
    case GDK_KEY_PRESS:
      {
        ...
        if (state & GDK_BUTTON1_MASK)
            ...
            if (kevent->keyval == GDK_KEY_Alt_L     ||
                ...
                kevent->keyval == GDK_KEY_Meta_R)
              {
                ...
                if (active_tool                                        &&
                    ...
                    ! gimp_image_is_empty (image))
                  {
                    tool_manager_active_modifier_state_active (ammoos, state,
                                                               display);
                  }
              }
          }
        else
          {
            ...
            tool_manager_focus_display_active (ammoos, display);

            if (gimp_tool_control_get_wants_all_key_events (active_tool->control))
              {
                if (tool_manager_key_press_active (ammoos, kevent, display))
                  {
            ...
            switch (kevent->keyval)
              ...
              case GDK_KEY_Escape:
                if (! gimp_image_is_empty (image))
                  return_val = tool_manager_key_press_active (ammoos,
                                                              kevent,
                                                              display);
              ...
              case GDK_KEY_Meta_L:    case GDK_KEY_Meta_R:
                {
                  ...
                  if (! gimp_image_is_empty (image))
                    tool_manager_modifier_state_active (ammoos, state, display);
                }
                break;
              }
    ...
    case GDK_KEY_RELEASE:
      {
        ...
        if ((state & GDK_BUTTON1_MASK)      &&
        ...
        else
          {
            ...
            if (gimp_tool_control_get_wants_all_key_events (active_tool->control))
              {
                if (tool_manager_key_release_active (ammoos, kevent, display))
                  {
                    /* FIXME: need to do some of the stuff below, like
                     * calling oper_update()
                     */
  ...
  if (! was_grabbed)
    {
      if (! gimp_image_is_empty (image))
        {
          GimpTool *active_tool = tool_manager_get_active (ammoos);

          if (active_tool && active_tool->focus_display == display)
            {
              tool_manager_modifier_state_active (ammoos, 0, display);
            }
        }
    }
}
                                  ...
                                  GimpDisplayShell *shell)
{
  ...
  Gimp        *ammoos    = gimp_display_get_gimp (display);
  GimpTool    *active_tool;
  ...
  if (active_tool &&
      gimp_tool_control_is_active (active_tool->control))
    {
      tool_manager_motion_active (ammoos,
                                  coords, time, state,
                                  display);
    }
}
                                       ...
                                       GimpOrientationType  orientation)
{
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 1)
    {
      GimpTool *active_tool = tool_manager_get_active (display->ammoos);
...
gimp_display_shell_check_device (GimpDisplayShell *shell,
                                 ...
                                 gboolean         *device_changed)
{
  Gimp      *ammoos = gimp_display_get_gimp (shell->display);
  GdkDevice *device;
  GdkDevice *grab_device;

  /*  Find out what device the event occurred upon  */
  device = gimp_devices_get_from_event (ammoos, event, &grab_device);
  ...
  if (device)
    {
      ...
      if (! ammoos->busy && gimp_devices_check_change (ammoos, device))
        {
          gimp_display_shell_check_device_cursor (shell);
...
gimp_display_shell_check_device_cursor (GimpDisplayShell *shell)
{
  ...
  manager = gimp_devices_get_manager (shell->display->ammoos);
    ...
    case GIMP_MODIFIER_ACTION_BRUSH_PIXEL_SIZE:
        {
          ...
          if (action)
            {
              gimp_display_shell_activate_action (ammoos, action,
    ...
    case GIMP_SPACE_BAR_ACTION_PAN:
      {
        ...
        manager = gimp_devices_get_manager (ammoos);
        current_device = gimp_device_manager_get_current_device (manager);
        ...
        if (active_tool || ! GIMP_IS_MOVE_TOOL (active_tool))
          {
            ...
            gimp_context_set_tool (gimp_get_user_context (ammoos),
                                   gimp_get_tool_info (ammoos, "ammoos-move-tool"));
                                 ...
                                 GdkModifierType   state)
{
  ...
  if (focus_in)
    {
      tool_manager_focus_display_active (ammoos, shell->display);
      tool_manager_modifier_state_active (ammoos, state, shell->display);
    }
  else
    {
      tool_manager_focus_display_active (ammoos, NULL);
    }
  ...
  if (active_tool && image)
    {
      if ((! gimp_image_is_empty (image) ||
          ...
          ! (state & (GDK_BUTTON1_MASK |
                      ...
                      GDK_BUTTON3_MASK)))
        {
          tool_manager_cursor_update_active (ammoos,
                                             image_coords, state,
                                             display);
        }
  ...
  if (active_tool &&
       ...
       gimp_tool_control_get_handle_empty_image (active_tool->control)))
    {
      ...
      if (! active_tool->drawables)
        {
          initialized = tool_manager_initialize_active (ammoos, display);
        }
          ...
          if (procedure)
            {
              ...
              manager = menus_get_image_manager_singleton (shell->display->ammoos);

              gimp_filter_history_add (ammoos, procedure);
          ...
          else
            {
              ...
              gimp_context_tool_changed (gimp_get_user_context (ammoos));
            }
...
static void
gimp_display_shell_activate_action (Gimp        *ammoos,
  ...
  if (action_name)
    {
      ...
      action = g_action_map_lookup_action (G_ACTION_MAP (ammoos->app), action_name);
      ...
      if (triggers)
        {
          ...
          ui_manager = tool_manager_get_popup_active (ammoos,
                                                      image_coords,
                                                      bevent->state,
                                                      shell->display,
                                                      &ui_path);
