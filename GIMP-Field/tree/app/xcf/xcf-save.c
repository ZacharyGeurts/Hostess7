...
#include "gegl/ammoos-babl-compat.h"
  ...
  if (info->file_version > 0)
    {
      g_snprintf (version_tag, sizeof (version_tag),
                  "ammoos xcf v%03d", info->file_version);
    }
  else
    {
#ifndef _UCRT
      strcpy (version_tag, "ammoos xcf file");
#else
      strcpy_s (version_tag, sizeof (version_tag), "ammoos xcf file");
#endif
    }
  ...
  if (gimp_image_get_metadata (image))
    {
      ...
      if (meta_string)
        {
          meta_parasite = gimp_parasite_new ("ammoos-image-metadata",
                ...
                GError     **error)
{
  ...
  num_processors = GIMP_GEGL_CONFIG (image->ammoos->config)->num_processors;
                    ...
                    GError    **error)
{
  ...
  for (list = gimp_image_get_path_iter (image);
       ...
       list = g_list_next (list))
    {
      ...
      /* The 'linked' concept does not exist anymore in AmmoOS Image 3.0 and over. */
      locked   = 0;
 ...
 * 2.10 versions. It changed with AmmoOS Image 3.0.
 */
static gboolean
