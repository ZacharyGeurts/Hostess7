...
typedef GimpImage * GimpXcfLoaderFunc (Gimp     *ammoos,
                                       XcfInfo  *info,
                                       GError  **error);
...
xcf_init (Gimp *ammoos)
{
  ...
  /*  ammoos-xcf-save  */
  file = g_file_new_for_path ("ammoos-xcf-save");
  ...
  proc->menu_label = g_strdup (_("AmmoOS Image XCF image"));
  gimp_plug_in_procedure_set_icon (proc, GIMP_ICON_TYPE_ICON_NAME,
                                   (const guint8 *) "ammoos-wilber",
                                   strlen ("ammoos-wilber") + 1,
                                   NULL);
  ...
  gimp_object_set_static_name (GIMP_OBJECT (procedure), "ammoos-xcf-save");
  gimp_procedure_set_static_help (procedure,
                                  "Saves file in the .xcf file format",
                                  "The XCF file format has been designed "
                                  "specifically for loading and saving "
                                  "tiled and layered images in AmmoOS Image. "
  ...
  /*  ammoos-xcf-load  */
  file = g_file_new_for_path ("ammoos-xcf-load");
  procedure = gimp_plug_in_procedure_new (GIMP_PDB_PROC_TYPE_INTERNAL, file, NULL);
  g_object_unref (file);
  ...
  gimp_plug_in_procedure_set_file_proc (proc, "xcf", NULL, "",
                                        "0,string,ammoos\\040xcf\\040");
  gimp_plug_in_procedure_set_mime_types (proc, "image/x-xcf");
  gimp_plug_in_procedure_set_handles_remote (proc);

  gimp_object_set_static_name (GIMP_OBJECT (procedure), "ammoos-xcf-load");
  gimp_procedure_set_static_help (procedure,
...
void
xcf_exit (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
}

GimpImage *
xcf_load_stream (Gimp          *ammoos,
                 GInputStream  *input,
                 GFile         *input_file,
                 GimpProgress  *progress,
                 GError       **error)
{
  ...
  success = xcf_load_magic_version (ammoos, input, input_file, progress, &info);
  ...
  if (success)
    {
      if (info.file_version >= 0 &&
        ...
        {
          image = (*(xcf_loaders[info.file_version])) (ammoos, &info, error);
                  ...
                  GError               **error)
{
  ...
  if (input)
    {
      image = xcf_load_stream (ammoos, input, file, progress, error);
                  ...
                  GError               **error)
{
  ...
  if (output)
    {
      success = xcf_save_stream (ammoos, image, output, file, progress, error);
