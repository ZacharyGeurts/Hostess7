                    ...
                    const gchar   *ui_path)
{
  ...
  g_signal_connect_object (manager->ammoos->displays, "add",
                           G_CALLBACK (windows_menu_display_add),
                           manager, 0);
  g_signal_connect_object (manager->ammoos->displays, "remove",
                           G_CALLBACK (windows_menu_display_remove),
                           manager, 0);
  g_signal_connect_object (manager->ammoos->displays, "reorder",
                           G_CALLBACK (windows_menu_display_reorder),
                           manager, 0);

  for (list = gimp_get_display_iter (manager->ammoos);
       list;
       list = g_list_next (list))
    {
      GimpDisplay *display = list->data;

      windows_menu_display_add (manager->ammoos->displays, display, manager);
    }
...
windows_menu_image_notify (GimpDisplay      *display,
                           ...
                           GimpUIManager    *manager)
{
  windows_menu_display_remove (manager->ammoos->displays, display, manager);
