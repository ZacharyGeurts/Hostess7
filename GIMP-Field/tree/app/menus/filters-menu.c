                    ...
                    const gchar   *ui_path)
{
  ...
  for (i = 0; i < gimp_filter_history_size (manager->ammoos); i++)
    {
      gchar *action_name;
  ...
  for (i = 0; i < g_strv_length (gegl_actions); i++)
    {
      ...
      path    = (gchar *) gegl_operation_get_key (op_name, "ammoos:menu-path");
      ...
      if (root == NULL || root == path)
        {
          g_printerr ("GEGL operation \"%s\" attempted to register a menu item "
                      "with an invalid value for key \"ammoos:menu-path\": \"%s\"\n"
