...
menus_init (Gimp *ammoos)
{
  ...
  global_menu_factory = menus_get_global_menu_factory (ammoos);
...
void
menus_exit (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
  g_return_if_fail (menus_get_global_menu_factory (ammoos) != NULL);

  g_object_unref (menus_get_global_menu_factory (ammoos));
}

void
menus_restore (Gimp *ammoos)
{
  GFile  *file;
  GError *error = NULL;
  ...
  if (g_file_query_exists (file, NULL) &&
      ! shortcuts_rc_parse (GTK_APPLICATION (ammoos->app), file, &error))
            ...
            gboolean  always_save)
{
  ...
  if (! shortcuts_rc_write (GTK_APPLICATION (ammoos->app), file, &error))
  ...
  for (gint i = 0; actions[i] != NULL; i++)
    {
      ...
      action = (GimpAction *) g_action_map_lookup_action (G_ACTION_MAP (ammoos->app),
                                                          actions[i]);
...
GimpMenuFactory *
menus_get_global_menu_factory (Gimp *ammoos)
{
  ...
  if (global_menu_factory == NULL && ! created)
    {
      g_set_weak_pointer (&global_menu_factory,
                          gimp_menu_factory_new (ammoos, global_action_factory));
...
GimpUIManager *
menus_get_image_manager_singleton (Gimp *ammoos)
{
  static GimpUIManager *image_ui_manager = NULL;

  if (image_ui_manager == NULL)
    {
      image_ui_manager = gimp_menu_factory_get_manager (menus_get_global_menu_factory (ammoos),
                                                        "<Image>", ammoos);
...
void
menus_quartz_app_menu (Gimp *ammoos)
{
  NSMenu     *main_menu;
  ...
  [app_menu setTitle:@"AmmoOS Image"];
