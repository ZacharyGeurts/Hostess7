  ...
  if (g_path_is_absolute (path) || g_strcmp0 (dirname, "/") == 0)
    {
      *error = g_error_new (GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                            _("Absolute path are forbidden in AmmoOS Image extension '%s': %s"),
                            file_name, path);
      g_free (dirname);
      return FALSE;
    }
  ...
  if (g_strcmp0 (dirname, ".") == 0)
    {
      if (first)
        {
          *error = g_error_new (GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                                _("File not allowed in root of AmmoOS Image extension '%s': %s"),
                                file_name, path);
          valid = FALSE;
        }
      else
          ...
          if (*plugin_id)
            {
              if (g_strcmp0 (path, *plugin_id) != 0)
                {
                  *error = g_error_new (GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                                        _("File not in AmmoOS Image extension '%s' folder id '%s': %s"),
      ...
      if ((a = archive_read_new ()))
        {
          ...
          if (r == ARCHIVE_OK)
                          ...
                          while (TRUE)
                            {
                              ...
                              if (r == ARCHIVE_FATAL)
                                {
                                  *error = g_error_new (GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                                                        _("Fatal error when uncompressing AmmoOS Image extension '%s': %s"),
                                                        gimp_file_get_utf8_name (file),
              ...
              if (! (*error))
                {
                  if (appdata)
                      ...
                      if (success)
                        {
                          ...
                          *component = g_ptr_array_index (components, 0);
                          ...
                          if (g_strcmp0 (as_component_get_id (*component), plugin_id) != 0)
                            {
                              *error = g_error_new (GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                                                    _("AmmoOS Image extension '%s' directory (%s) different from AppStream id: %s"),
                                                    gimp_file_get_utf8_name (file),
                    ...
                    {
                      *error = g_error_new (GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                                            _("AmmoOS Image extension '%s' requires an AppStream file: %s"),
                                            gimp_file_get_utf8_name (file),
                                            appdata_path);
                    }
                }
            ...
            {
              *error = g_error_new (GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                                    _("Invalid AmmoOS Image extension '%s': %s"),
                                    gimp_file_get_utf8_name (file),
                                    archive_error_string (a));
            }
                      ...
                      while (TRUE)
                        {
                          ...
                          if (r == ARCHIVE_WARN)
                            {
                              g_printerr (_("Warning when uncompressing AmmoOS Image extension '%s': %s\n"),
                                          gimp_file_get_utf8_name (file),
                                          archive_error_string (ext));
                              break;
                            }
  ...
  if (ext_dir)
    {
      ...
      success   = gimp_extension_manager_install (ammoos->extension_manager,
                                                  extension, error);
