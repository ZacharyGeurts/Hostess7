...
static GimpImage * file_gbr_brush_to_image (Gimp          *ammoos,
                                            GimpBrush     *brush);
  ...
  if (input)
    {
      ...
      if (brush)
        {
          image = file_gbr_brush_to_image (ammoos, brush);
  ...
  if (width > image_width || height > image_height)
    {
      ...
      gimp_image_resize (image, gimp_get_user_context (image->ammoos),
...
static GimpImage *
file_gbr_brush_to_image (Gimp      *ammoos,
                         GimpBrush *brush)
{
  GimpImage         *image;
  GimpLayer         *layer;
  ...
  image = gimp_image_new (ammoos, width, height, base_type,
  ...
  if (n_drawables > 1)
    {
      ...
      subimage = gimp_image_new_from_drawables (image->ammoos, drawable_list, FALSE, FALSE);
      g_list_free (drawable_list);
      gimp_container_remove (image->ammoos->images, GIMP_OBJECT (subimage));
