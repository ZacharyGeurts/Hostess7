 ...
 * Copyright (C) 2008  Sven Neumann <sven@ammoos.org>
 *
...
static GFile    * file_remote_get_temp_file      (Gimp            *ammoos,
                                                  GFile           *file);
static gboolean   file_remote_copy_file          (Gimp            *ammoos,
                                                  GFile           *src_file,
                                                  GFile           *dest_file,
                                                  RemoteCopyMode   mode,
                                                  GimpProgress    *progress,
...
gboolean
file_remote_mount_file (Gimp          *ammoos,
                        GFile         *file,
                        GimpProgress  *progress,
                        GError       **error)
{
  ...
  operation = gimp_get_mount_operation (ammoos, progress);
...
GFile *
file_remote_download_image (Gimp          *ammoos,
                            GFile         *file,
                            GimpProgress  *progress,
                            GError       **error)
{
  ...
  local_file = file_remote_get_temp_file (ammoos, file);

  if (! file_remote_copy_file (ammoos, file, local_file, DOWNLOAD,
                               progress, error))
    {
...
GFile *
file_remote_upload_image_prepare (Gimp          *ammoos,
                                  GFile         *file,
                                  GimpProgress  *progress,
                                  GError       **error)
{
...
gboolean
file_remote_upload_image_finish (Gimp          *ammoos,
                                 GFile         *file,
                                 GFile         *local_file,
                                 GimpProgress  *progress,
                                 GError       **error)
{
  ...
  if (! file_remote_copy_file (ammoos, local_file, file, UPLOAD,
                               progress, error))
    {
      return FALSE;
    }
...
static GFile *
file_remote_get_temp_file (Gimp  *ammoos,
                           GFile *file)
{
  gchar *basename;
  ...
  if (basename)
    {
      ...
      if (ext && strlen (ext))
        temp_file = gimp_get_temp_file (ammoos, ext + 1, NULL);

      g_free (basename);
    }

  if (! temp_file)
    temp_file = gimp_get_temp_file (ammoos, "xxx", NULL);

  return temp_file;
}

static gboolean
file_remote_copy_file (Gimp            *ammoos,
                       GFile           *src_file,
                       GFile           *dest_file,
                       RemoteCopyMode   mode,
                       GimpProgress    *progress,
