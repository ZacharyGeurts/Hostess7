           ...
           GError             **error)
{
  ...
  g_return_val_if_fail (GIMP_IS_GIMP (ammoos), GIMP_PDB_CALLING_ERROR);
  ...
  if (! file_proc->handles_remote || ! mounted)
    {
      ...
      if (! my_path)
        {
          local_file = file_remote_upload_image_prepare (ammoos, file, progress,
                                                         &my_error);
  ...
  return_vals =
    gimp_pdb_execute_procedure_by_name (image->ammoos->pdb,
      ...
      if (status == GIMP_PDB_SUCCESS)
        {
          ...
          if (! file_remote_upload_image_finish (ammoos, orig_file, local_file,
                                                 progress, &my_error))
            {
  ...
  if (status == GIMP_PDB_SUCCESS)
    {
      ...
      documents = GIMP_DOCUMENT_LIST (image->ammoos->documents);
