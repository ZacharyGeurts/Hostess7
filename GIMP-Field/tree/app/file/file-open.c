...
#include "core/ammoos-data-factories.h"
...
#include "ammoos-file.h"

#include "ammoos-intl.h"


static GimpImage * file_open_link_image           (Gimp                 *ammoos,
                                                   GimpContext          *context,
...
static gboolean    file_open_valid_permissions    (GFile                *file,
                                                   GError              **error);
static gboolean    file_open_pre_check            (Gimp                 *ammoos,
                                                   GFile                *file,
                                                   GimpImage            *dest_image,
                                                   gboolean              as_link,
                                                   GimpProgress         *progress,
                 ...
                 GError             **error)
{
  ...
  if (! file_open_pre_check (ammoos, file, NULL, FALSE, progress,
  ...
  *status = GIMP_PDB_EXECUTION_ERROR;
  ...
  if (file_proc->handles_vector)
    {
      return_vals =
        gimp_pdb_execute_procedure_by_name (ammoos->pdb,
 ...
 */
GimpImage *
file_open_thumbnail (Gimp           *ammoos,
                     GimpContext    *context,
                     GimpProgress   *progress,
                     GFile          *file,
                     gint            size,
                     ...
                     GError        **error)
{
  ...
  *num_layers   = -1;
  ...
  procedure = gimp_pdb_lookup_procedure (ammoos->pdb, file_proc->thumb_loader);
                    ...
                    case GIMP_INDEXEDA_IMAGE:
                      {
                        ...
                        babl_new_palette ("-ammoos-indexed-format-dummy",
                                          &rgb, &rgba);
...
GimpImage *
file_open_with_display (Gimp               *ammoos,
                        GimpContext        *context,
                        GimpProgress       *progress,
                        GFile              *file,
                        gboolean            as_new,
                        ...
                        GError            **error)
{
  return file_open_with_proc_and_display (ammoos, context, progress,
                                          file, as_new, FALSE, NULL,
                                          monitor, status, error);
}

GimpImage *
file_open_with_proc_and_display (Gimp                *ammoos,
                                 GimpContext         *context,
                                 GimpProgress        *progress,
                                 GFile               *file,
                                 gboolean             as_new,
                                 ...
                                 GError             **error)
{
  ...
  if (as_link)
    image = file_open_link_image (ammoos, context, NULL, progress,
                                  file, 0, 0, TRUE,
                                  as_new,
                                  file_proc,
                                  run_mode,
    ...
    image = file_open_image (ammoos, context, progress,
                             file, 0, 0, TRUE,
                             as_new,
                             file_proc,
                             run_mode,
  ...
  if (image)
    {
      ...
      if (gimp_create_display (image->ammoos, image, gimp_unit_pixel (), 1.0,
                               monitor))
        {
      ...
      if (! as_new)
        {
          GimpDocumentList *documents = GIMP_DOCUMENT_LIST (ammoos->documents);
      ...
      gimp_image_opened (image->ammoos, file);
    }
                  ...
                  GError             **error)
{
  ...
  if (as_link)
    new_image = file_open_link_image (ammoos, context, dest_image, progress,
                                      file,
    ...
    new_image = file_open_image (ammoos, context, progress,
                                 file,
                                 gimp_image_get_width (dest_image),
                                 gimp_image_get_height (dest_image),
                                 TRUE,
      ...
      if (layers)
        {
          ...
          gimp_document_list_add_file (GIMP_DOCUMENT_LIST (ammoos->documents),
                                       file, mime_type);
        }
      else
        {
 ...
 */
gboolean
file_open_from_command_line (Gimp     *ammoos,
                             GFile    *file,
                             gboolean  as_new,
                             GObject  *monitor)

{
  ...
  display = gimp_get_empty_display (ammoos);

  /* show the progress in the last opened display, see bug #704896 */
  if (! display)
    display = gimp_context_get_display (gimp_get_user_context (ammoos));
  ...
  if (image)
    ...
    {
      gimp_message (ammoos, G_OBJECT (display), GIMP_MESSAGE_ERROR,
...
static GimpImage *
file_open_link_image (Gimp                *ammoos,
                      GimpContext         *context,
                      GimpImage           *dest_image,
                      GimpProgress        *progress,
                      GFile               *file,
                      ...
                      GError             **error)
{
  ...
  if (! file_open_pre_check (ammoos, file, dest_image, TRUE, progress,
                             &file_proc, NULL, status, error))
    return NULL;

  link = gimp_link_new (ammoos, file,
  ...
  if (gimp_link_is_broken (link))
  ...
  else
    {
      ...
      image = gimp_image_new (ammoos, width, height,
...
static gboolean
file_open_pre_check (Gimp                 *ammoos,
                     GFile                *file,
                     GimpImage            *dest_image,
                     gboolean              as_link,
                     GimpProgress         *progress,
                     ...
                     GError              **error)
{
  ...
  *status = GIMP_PDB_EXECUTION_ERROR;
          ...
          if (! loop_found)
            {
              ...
              if (input && xcf_load_magic_version (ammoos, input, file, NULL, &info))
                xcf_load_image_header (ammoos, &info, &width, &height, &type, &precision,
  ...
  if (! g_file_is_native (file) &&
      ! file_remote_mount_file (ammoos, file, progress, &my_error))
    {
      if (my_error)
        {
      ...
      if (! my_path)
        {
          ...
          *local_file = file_remote_download_image (ammoos, file, progress,
                                                    &my_error);
  ...
  if (*file_proc != NULL &&
    ...
    return gimp_data_factories_wait (ammoos);
