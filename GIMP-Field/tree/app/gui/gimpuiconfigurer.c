                                 ...
                                 GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      ui_configurer->p->ammoos = g_value_get_object (value); /* don't ref */
      break;
                                 ...
                                 GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      g_value_set_object (value, ui_configurer->p->ammoos);
      break;
  ...
  while (gimp_image_window_get_n_shells (source_image_window) > 1)
    {
      ...
      new_image_window = gimp_image_window_new (ui_configurer->p->ammoos,
                                                NULL,
...
{
  Gimp             *ammoos              = ui_configurer->p->ammoos;
  GList            *windows           = gimp_get_image_windows (ammoos);
  GList            *iter              = NULL;
  GimpImageWindow  *uber_image_window = NULL;
  GimpDisplay      *active_display    = gimp_context_get_display (gimp_get_user_context (ammoos));
