...
static void  gui_unique_win32_init (Gimp *ammoos);
...
static void     gui_unique_quartz_init         (Gimp *ammoos);
...
static void  gui_dbus_service_init (Gimp *ammoos);
...
void
gui_unique_init (Gimp *ammoos)
{
#ifdef G_OS_WIN32
  gui_unique_win32_init (ammoos);
#elif defined (GDK_WINDOWING_QUARTZ)
  gui_unique_quartz_init (ammoos);
#else
  gui_dbus_service_init (ammoos);
#endif
}
...
static void
gui_unique_win32_init (Gimp *ammoos)
{
  WNDCLASSW wc;
...
static void
gui_unique_quartz_init (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
  g_return_if_fail (unique_gimp == NULL);
   ...
   * to update AmmoOS Image's UI theme accordingly.
   */
   ...
   * AmmoOS Image does not follow our standard quit procedure. Instead, macOS forces the
...
gui_unique_quartz_trigger_quit (gpointer data)
{
  ...
  GimpUIManager *ui_manager = menus_get_image_manager_singleton (ammoos);
                       ...
                       Gimp            *ammoos)
{
  ...
  dbus_manager = g_dbus_object_manager_server_new ("/org/ammoos/AmmoOS Image");

  object = g_dbus_object_skeleton_new (GIMP_DBUS_INTERFACE_PATH);

  service = gimp_dbus_service_new (ammoos);
...
static void
gui_dbus_service_init (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
