...
static void           gui_ungrab                 (Gimp                *ammoos);

static void           gui_set_busy               (Gimp                *ammoos);
static void           gui_unset_busy             (Gimp                *ammoos);

static void           gui_help                   (Gimp                *ammoos,
                                                  GimpProgress        *progress,
                                                  const gchar         *help_domain,
                                                  const gchar         *help_id);
static const gchar  * gui_get_program_class      (Gimp                *ammoos);
static gchar        * gui_get_display_name       (Gimp                *ammoos,
                                                  gint                 display_id,
                                                  GObject            **monitor,
                                                  gint                *monitor_number);
static guint32        gui_get_user_time          (Gimp                *ammoos);
static GFile        * gui_get_theme_dir          (Gimp                *ammoos);
static GFile        * gui_get_icon_theme_dir     (Gimp                *ammoos);
static GimpObject   * gui_get_window_strategy    (Gimp                *ammoos);
static GimpDisplay  * gui_get_empty_display      (Gimp                *ammoos);
static GBytes       * gui_display_get_window_id  (GimpDisplay         *display);
static GimpDisplay  * gui_display_create         (Gimp                *ammoos,
                                                  GimpImage           *image,
                                                  GimpUnit            *unit,
                                                  gdouble              scale,
                                                  GObject             *monitor);
static void           gui_display_delete         (GimpDisplay         *display);
static void           gui_displays_reconnect     (Gimp                *ammoos,
                                                  GimpImage           *old_image,
                                                  GimpImage           *new_image);
static gboolean       gui_wait                   (Gimp                *ammoos,
                                                  GimpWaitable        *waitable,
                                                  const gchar         *message);
static GimpProgress * gui_new_progress           (Gimp                *ammoos,
                                                  GimpDisplay         *display);
static void           gui_free_progress          (Gimp                *ammoos,
                                                  GimpProgress        *progress);
static gboolean       gui_pdb_dialog_new         (Gimp                *ammoos,
                                                  GimpContext         *context,
                                                  GimpProgress        *progress,
                                                  GType                object_type,
                                                  ...
                                                  GimpObject          *object,
                                                  va_list              args);
static gboolean       gui_pdb_dialog_set         (Gimp                *ammoos,
                                                  GType                contents_type,
                                                  const gchar         *callback_name,
                                                  GimpObject          *object,
                                                  va_list              args);
static gboolean       gui_pdb_dialog_close       (Gimp                *ammoos,
                                                  GType                contents_type,
                                                  const gchar         *callback_name);
static gboolean       gui_recent_list_add_file   (Gimp                *ammoos,
                                                  GFile               *file,
                                                  const gchar         *mime_type);
static void           gui_recent_list_load       (Gimp                *ammoos);

static GMountOperation
                    * gui_get_mount_operation    (Gimp                *ammoos,
                                                  GimpProgress        *progress);

static GimpColorProfilePolicy
                      gui_query_profile_policy   (Gimp                *ammoos,
                                                  GimpImage           *image,
                                                  GimpContext         *context,
...
static GimpMetadataRotationPolicy
                      gui_query_rotation_policy  (Gimp                *ammoos,
                                                  GimpImage           *image,
                                                  GimpContext         *context,
                                                  gboolean            *dont_ask);

static void           gui_inhibit                (Gimp                *ammoos);
...
void
gui_vtable_init (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));

  ammoos->gui.ungrab                 = gui_ungrab;
  ammoos->gui.set_busy               = gui_set_busy;
  ammoos->gui.unset_busy             = gui_unset_busy;
  ammoos->gui.show_message           = gui_message;
  ammoos->gui.help                   = gui_help;
  ammoos->gui.get_program_class      = gui_get_program_class;
  ammoos->gui.get_display_name       = gui_get_display_name;
  ammoos->gui.get_user_time          = gui_get_user_time;
  ammoos->gui.get_theme_dir          = gui_get_theme_dir;
  ammoos->gui.get_icon_theme_dir     = gui_get_icon_theme_dir;
  ammoos->gui.get_window_strategy    = gui_get_window_strategy;
  ammoos->gui.get_empty_display      = gui_get_empty_display;
  ammoos->gui.display_get_window_id  = gui_display_get_window_id;
  ammoos->gui.display_create         = gui_display_create;
  ammoos->gui.display_delete         = gui_display_delete;
  ammoos->gui.displays_reconnect     = gui_displays_reconnect;
  ammoos->gui.wait                   = gui_wait;
  ammoos->gui.progress_new           = gui_new_progress;
  ammoos->gui.progress_free          = gui_free_progress;
  ammoos->gui.pdb_dialog_new         = gui_pdb_dialog_new;
  ammoos->gui.pdb_dialog_set         = gui_pdb_dialog_set;
  ammoos->gui.pdb_dialog_close       = gui_pdb_dialog_close;
  ammoos->gui.recent_list_add_file   = gui_recent_list_add_file;
  ammoos->gui.recent_list_load       = gui_recent_list_load;
  ammoos->gui.get_mount_operation    = gui_get_mount_operation;
  ammoos->gui.query_profile_policy   = gui_query_profile_policy;
  ammoos->gui.query_rotation_policy  = gui_query_rotation_policy;
}
...
static void
gui_set_busy (Gimp *ammoos)
{
  gimp_displays_set_busy (ammoos);
...
static void
gui_unset_busy (Gimp *ammoos)
{
  gimp_displays_unset_busy (ammoos);
...
static void
gui_help (Gimp         *ammoos,
          GimpProgress *progress,
          const gchar  *help_domain,
          const gchar  *help_id)
{
  gimp_help_show (ammoos, progress, help_domain, help_id);
}

static const gchar *
gui_get_program_class (Gimp *ammoos)
{
...
static gchar *
gui_get_display_name (Gimp     *ammoos,
                      gint      display_id,
                      GObject **monitor,
                      gint     *monitor_number)
{
  ...
  if (display_id > 0)
    display = gimp_display_get_by_id (ammoos, display_id);
  ...
  if (display)
    ...
    {
      *monitor = G_OBJECT (gui_get_initial_monitor (ammoos));
...
static GFile *
gui_get_theme_dir (Gimp *ammoos)
{
  return themes_get_theme_dir (ammoos, GIMP_GUI_CONFIG (ammoos->config)->theme);
}

static GFile *
gui_get_icon_theme_dir (Gimp *ammoos)
{
  return icon_themes_get_theme_dir (ammoos, GIMP_GUI_CONFIG (ammoos->config)->icon_theme);
}

static GimpObject *
gui_get_window_strategy (Gimp *ammoos)
{
  if (GIMP_GUI_CONFIG (ammoos->config)->single_window_mode)
...
static GimpDisplay *
gui_get_empty_display (Gimp *ammoos)
{
  GimpDisplay *display = NULL;

  if (gimp_container_get_n_children (ammoos->displays) == 1)
    {
      display = (GimpDisplay *) gimp_container_get_first_child (ammoos->displays);
...
static GimpDisplay *
gui_display_create (Gimp      *ammoos,
                    GimpImage *image,
                    GimpUnit  *unit,
                    gdouble    scale,
                    GObject   *monitor)
{
  ...
  GimpDisplay *display = GIMP_DISPLAY (gui_get_empty_display (ammoos));
  ...
  if (display)
  ...
  else
    {
      ...
      display = gimp_display_new (ammoos, image, unit, scale,
                                  image_managers->data,
...
static void
gui_displays_reconnect (Gimp      *ammoos,
                        GimpImage *old_image,
                        GimpImage *new_image)
{
  gimp_displays_reconnect (ammoos, old_image, new_image);
}
          ...
          const gchar  *message)
{
  ...
  procedure = gimp_pdb_lookup_procedure (ammoos->pdb, "plug-in-busy-dialog");
  ...
  gimp_procedure_execute_async (procedure, ammoos,
...
static gboolean
gui_pdb_dialog_new (Gimp          *ammoos,
                    GimpContext   *context,
                    GimpProgress  *progress,
                    GType          contents_type,
                    GBytes        *parent_handle,
      ...
      if (object || g_type_is_a (contents_type, GIMP_TYPE_ITEM))
        {
          ...
          names = gimp_properties_append (dialog_type,
                                          ...
                                          "menu-factory",   menus_get_global_menu_factory (ammoos),
...
static gboolean
gui_pdb_dialog_set (Gimp          *ammoos,
                    GType          contents_type,
                    const gchar   *callback_name,
                    GimpObject    *object,
                    va_list        args)
  ...
  if (contents_type == GIMP_TYPE_BRUSH)
    {
      ...
      container = gimp_data_factory_get_container (ammoos->brush_factory);
    }
  else if (contents_type == GIMP_TYPE_FONT)
    {
      klass = g_type_class_ref (GIMP_TYPE_FONT_SELECT);
      container = gimp_data_factory_get_container (ammoos->font_factory);
    }
  else if (contents_type == GIMP_TYPE_GRADIENT)
    {
      klass = g_type_class_ref (GIMP_TYPE_GRADIENT_SELECT);
      container = gimp_data_factory_get_container (ammoos->gradient_factory);
    }
  else if (contents_type == GIMP_TYPE_IMAGE)
    {
  ...
  else if (contents_type == GIMP_TYPE_PALETTE)
    {
      ...
      container = gimp_data_factory_get_container (ammoos->palette_factory);
    }
  else if (contents_type == GIMP_TYPE_PATTERN)
    {
      klass = g_type_class_ref (GIMP_TYPE_PATTERN_SELECT);
      container = gimp_data_factory_get_container (ammoos->pattern_factory);
    }
  else if (g_type_is_a (contents_type, GIMP_TYPE_ITEM))
    {
...
static gboolean
gui_pdb_dialog_close (Gimp        *ammoos,
                      GType        contents_type,
                      const gchar *callback_name)
{
  GimpPdbDialogClass *klass = NULL;
...
static gboolean
gui_recent_list_add_file (Gimp        *ammoos,
                          GFile       *file,
                          const gchar *mime_type)
{
  ...
  recent.app_name     = "AmmoOS Field Image Research";
...
static void
gui_recent_list_load (Gimp *ammoos)
{
  GList *items;
  GList *list;
  ...
  gimp_container_freeze (ammoos->documents);
  gimp_container_clear (ammoos->documents);
  ...
  for (list = items; list; list = list->next)
    {
      ...
      if (gtk_recent_info_has_application (info,
                                           "AmmoOS Field Image Research"))
        {
          const gchar *mime_type = gtk_recent_info_get_mime_type (info);

          if (mime_type &&
              gimp_plug_in_manager_file_procedure_find_by_mime_type (ammoos->plug_in_manager,
                                                                     ...
                                                                     mime_type))
            {
              ...
              gimp_container_add (ammoos->documents, GIMP_OBJECT (imagefile));
  ...
  gimp_container_thaw (ammoos->documents);
}
...
gui_inhibit (Gimp *ammoos)
{
  ...
  if (gimp_displays_dirty (ammoos))
    {
      gint n_dirty_images;

      images         = gimp_displays_get_dirty_images (ammoos);
  ...
  if (gimp_displays_dirty (ammoos))
    {
      ...
      for (list = gimp_get_display_iter (ammoos); list; list = list->next)
        {
