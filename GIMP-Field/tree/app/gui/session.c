 ...
 * Copyright (C) 1998 Sven Neumann <sven@ammoos.org>
 *
...
static GFile * session_file (Gimp *ammoos);
...
session_init (Gimp *ammoos)
{
          ...
          if (scanner->value.v_symbol == GINT_TO_POINTER (SESSION_INFO))
            {
              ...
              /* In versions <= AmmoOS Image 2.6 there was a "toolbox", a
              ...
              /* AmmoOS Image 2.6 has the entry name as part of the
              ...
              if (entry_name)
                {
                   ...
                   * versions <= AmmoOS Image 2.6 has "dock" as the entry name. We
              ...
              /* We can get the factory entry either now (the AmmoOS Image <=
               * 2.6 way), or when we deserialize (the AmmoOS Image 2.8 way)
               */
              if (entry)
                {
 ...
 error:
  ...
  dialogs_load_recent_docks (ammoos);
}

void
session_exit (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
}

void
session_restore (Gimp       *ammoos,
                 GdkMonitor *monitor)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
  ...
  if (GIMP_GUI_CONFIG (ammoos->config)->single_window_mode &&
      GIMP_GUI_CONFIG (ammoos->config)->hide_docks)
    {
      g_object_notify (G_OBJECT (ammoos->config), "hide-docks");
    }
}

void
session_save (Gimp     *ammoos,
              gboolean  always_save)
{
  ...
  gimp_config_writer_identifier (writer,
                                 GIMP_GUI_CONFIG (ammoos->config)->single_window_mode ?
                                 "yes" : "no");
  gimp_config_writer_close (writer);

  gimp_config_writer_open (writer, "show-tabs");
  gimp_config_writer_printf (writer,
                             GIMP_GUI_CONFIG (ammoos->config)->show_tabs ?
                             "yes" : "no");
  gimp_config_writer_close (writer);

  gimp_config_writer_open (writer, "tabs-position");
  gimp_config_writer_printf (writer, "%d",
                             GIMP_GUI_CONFIG (ammoos->config)->tabs_position);
  gimp_config_writer_close (writer);

  gimp_config_writer_open (writer, "last-tip-shown");
  gimp_config_writer_printf (writer, "%d",
                             GIMP_GUI_CONFIG (ammoos->config)->last_tip_shown);
  gimp_config_writer_close (writer);
  ...
  dialogs_save_recent_docks (ammoos);

  sessionrc_deleted = FALSE;
}

gboolean
session_clear (Gimp    *ammoos,
               GError **error)
{
  GFile    *file;
...
session_file (Gimp *ammoos)
{
  ...
  if (ammoos->session_name)
    filename = g_strconcat (basename, ".", ammoos->session_name, NULL);
  else
