 ...
 *
 * GimpDBusService
 * Copyright (C) 2007, 2008 Sven Neumann <sven@ammoos.org>
 * Copyright (C) 2013       Michael Natterer <mitch@ammoos.org>
 *
...
static void       gimp_dbus_service_gimp_opened    (Gimp                  *ammoos,
                                                    GFile                 *file,
...
GObject *
gimp_dbus_service_new (Gimp *ammoos)
{
  GimpDBusService *service;
  ...
  service->ammoos = ammoos;

  g_signal_connect_object (ammoos, "image-opened",
...
gimp_dbus_service_activate (GimpDBusServiceUI     *service,
                            GDBusMethodInvocation *invocation)
{
  Gimp *ammoos = GIMP_DBUS_SERVICE (service)->ammoos;

  /*  We want to be called again later in case that AmmoOS Image is not fully
   *  started yet.
   */
  if (gimp_is_restored (ammoos))
    {
      GimpObject *display;

      display = gimp_container_get_first_child (ammoos->displays);
...
static void
gimp_dbus_service_gimp_opened (Gimp            *ammoos,
                               GFile           *file,
                               GimpDBusService *service)
{
...
gimp_dbus_service_process_idle (GimpDBusService *service)
{
  ...
  if (! gimp_is_restored (service->ammoos))
    {
      if (! service->timeout_source)
        {
          /* We are probably starting the program. No need to spam AmmoOS Image with
           * an idle handler (which might make AmmoOS Image slower to start even
           * with low priority).
  ...
  if (data)
    {
      if (data->file)
        file_open_from_command_line (service->ammoos, data->file, data->as_new,
                                     NULL /* FIXME monitor */);
      ...
      if (data->command)
        {
          ...
          gimp_batch_run (service->ammoos, data->interpreter,
                          commands);
        }
      ...
      if (service->timeout_source)
        {
          /* Now AmmoOS Image is fully functional and can respond quickly to
