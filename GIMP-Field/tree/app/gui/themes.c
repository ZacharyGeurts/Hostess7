...
static void   themes_apply_theme                   (Gimp                  *ammoos,
...
themes_init (Gimp *ammoos)
{
  ...
  themes_theme_paths_notify (ammoos->extension_manager, NULL, ammoos);
  g_signal_connect (ammoos->extension_manager, "notify::theme-paths",
  ...
  themes_theme_change_notify (config, NULL, ammoos);
...
gchar **
themes_list_themes (Gimp *ammoos,
                    gint *n_themes)
{
...
GFile *
themes_get_theme_dir (Gimp        *ammoos,
...
GFile *
themes_get_theme_file (Gimp        *ammoos,
                       ...
                       ...)
{
  ...
  gui_config = GIMP_GUI_CONFIG (ammoos->config);

  file      = g_object_ref (themes_get_theme_dir (ammoos, gui_config->theme));
  ...
  if (! g_file_query_exists (file, NULL))
    {
      ...
      file      = g_object_ref (themes_get_theme_dir (ammoos, NULL));
      component = first_component;
                            ...
                            Gimp          *ammoos)
{
  ...
  themes_apply_theme (ammoos, config);
...
static void
themes_apply_theme (Gimp          *ammoos,
                    GimpGuiConfig *config)
{
  GFile           *theme_css;
  ...
  if (! output)
    ...
    {
      GFile  *theme_dir = themes_get_theme_dir (ammoos, config->theme);
      GFile  *css_user;
      ...
      if (theme_dir)
        {
          ...
          fallback = g_file_get_child (theme_dir, "ammoos.css");
          if (! g_file_query_exists (fallback, NULL))
            g_clear_object (&fallback);

          light = g_file_get_child (theme_dir, "ammoos-light.css");
          if (! g_file_query_exists (light, NULL))
            g_clear_object (&light);

          gray  = g_file_get_child (theme_dir, "ammoos-gray.css");
          if (! g_file_query_exists (gray, NULL))
            g_clear_object (&gray);

          dark  = g_file_get_child (theme_dir, "ammoos-dark.css");
          ...
          if (file != NULL)
            ...
            {
              gimp_message (ammoos, NULL, GIMP_MESSAGE_ERROR,
                            _("Invalid theme: directory '%s' contains neither "
                              "ammoos-dark.css, ammoos-gray.css, ammoos-light.css nor ammoos.css."),
      ...
      else
        {
          ...
          switch (color_scheme)
            ...
            case GIMP_THEME_LIGHT:
              tmp = g_build_filename (gimp_data_directory (),
                                      "themes", "Default", "ammoos-light.css",
                                      NULL);
              break;
            case GIMP_THEME_GRAY:
              tmp = g_build_filename (gimp_data_directory (),
                                      "themes", "Default", "ammoos-gray.css",
                                      NULL);
              break;
            case GIMP_THEME_DARK:
              tmp = g_build_filename (gimp_data_directory (),
                                      "themes", "Default", "ammoos-dark.css",
                                      NULL);
              break;
            case GIMP_THEME_SYSTEM:
              g_return_if_reached ();
      ...
      css_files = g_slist_prepend (
        css_files, gimp_sysconf_directory_file ("ammoos.css", NULL));

      css_user  = gimp_directory_file ("ammoos.css", NULL);
      css_files = g_slist_prepend (
        css_files, css_user);
      ...
      g_output_stream_printf (
        ...
        "/* AmmoOS Image theme.css\n"
        " *\n"
        " * This file is written on AmmoOS Image startup and on every theme change.\n"
        " * It is NOT supposed to be edited manually. Edit your personal\n"
        " * ammoos.css file instead (%s).\n"
        " */\n"
        "\n",
      ...
      if (! error)
        {
          g_output_stream_printf (
            ...
            "\n"
            "%s",
            icon_themes_current_prefer_symbolic (ammoos) ? "symbolic" : "regular",
  ...
  if (config->theme_path)
    {
      ...
      g_object_get (ammoos->extension_manager,
                    "theme-paths", &path,
                    NULL);
                                      ...
                                      Gimp        *ammoos)
{
  ...
  if (g_strcmp0 (namespace, "org.freedesktop.appearance") == 0 &&
    ...
    {
      themes_theme_change_notify (GIMP_GUI_CONFIG (ammoos->config), NULL, ammoos);
    }
...
#if defined(G_OS_WIN32) || (defined(PLATFORM_OSX) && MAC_OS_X_VERSION_MIN_REQUIRED >= 101400)
void
themes_set_title_bar (Gimp *ammoos)
{
  ...
  for (iter = windows; iter; iter = g_list_next (iter))
    {
      ...
      gimp_window_set_title_bar_theme (ammoos, window);
    }
