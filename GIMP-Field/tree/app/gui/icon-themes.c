...
static gboolean icons_apply_theme         (Gimp          *ammoos,
...
icon_themes_init (Gimp *ammoos)
{
  ...
  icons_theme_change_notify (config, NULL, ammoos);
}

void
icon_themes_exit (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
...
gchar **
icon_themes_list_themes (Gimp *ammoos,
                         gint *n_icon_themes)
{
...
GFile *
icon_themes_get_theme_dir (Gimp        *ammoos,
                           const gchar *icon_theme_name)
{
  g_return_val_if_fail (GIMP_IS_GIMP (ammoos), NULL);
...
gboolean
icon_themes_current_prefer_symbolic (Gimp *ammoos)
{
  ...
  return GIMP_GUI_CONFIG (ammoos->config)->prefer_symbolic_icons;
}
  ...
  if (g_getenv ("GIMP_TESTING_ABS_TOP_SRCDIR"))
    {
      ...
      path = g_build_filename (g_getenv ("GIMP_TESTING_ABS_TOP_SRCDIR"),
                               "ammoos-data", "icons", icon_theme_name, NULL);
    ...
    {
      applied = gimp_icons_set_icon_theme (icon_themes_get_theme_dir (ammoos, icon_theme_name));
    }
...
icons_theme_change_notify (GimpGuiConfig *config,
                           ...
                           Gimp          *ammoos)
{
  if (! icons_apply_theme (ammoos, config->icon_theme))
    {
