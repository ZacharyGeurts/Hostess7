...
static gboolean  gui_message_error_console  (Gimp                *ammoos,
                                             GimpMessageSeverity  severity,
                                             const gchar         *domain,
                                             const gchar         *message);
static gboolean  gui_message_error_dialog   (Gimp                *ammoos,
                                             GObject             *handler,
                                             GimpMessageSeverity  severity,
                                             const gchar         *domain,
                                             const gchar         *message,
             ...
             const gchar         *message)
{
  ...
  switch (ammoos->message_handler)
    {
    case GIMP_ERROR_CONSOLE:
      if (gui_message_error_console (ammoos, severity, domain, message))
        return;
    ...
    case GIMP_MESSAGE_BOX:
      ...
      if (gen_trace)
        {
           ...
           * errors (i.e. non-AmmoOS Image ones), the backtrace after a idle
      ...
      if (g_strcmp0 (GIMP_ACRONYM, domain) != 0)
        {
          /* Handle non-AmmoOS Image messages in a multi-thread safe way,
      ...
      if (gui_message_error_dialog (ammoos, handler, severity,
...
gui_message_idle (gpointer user_data)
{
  ...
  if (! gui_message_error_dialog (data->ammoos,
                                  data->handler,
                                  data->severity,
                                  data->domain,
                                  data->message,
...
static gboolean
gui_message_error_console (Gimp                *ammoos,
                           GimpMessageSeverity  severity,
                           const gchar         *domain,
                           const gchar         *message)
{
...
{
  g_object_set_data (G_OBJECT (progress), "ammoos-error-dialog", NULL);
}
...
progress_error_dialog (GimpProgress *progress)
{
  ...
  dialog = g_object_get_data (G_OBJECT (progress), "ammoos-error-dialog");

  if (! dialog)
    {
      dialog = gimp_error_dialog_new (_("AmmoOS Image Message"));

      g_object_set_data (G_OBJECT (progress), "ammoos-error-dialog", dialog);
...
static gboolean
gui_message_error_dialog (Gimp                *ammoos,
                          GObject             *handler,
                          GimpMessageSeverity  severity,
                          const gchar         *domain,
                          const gchar         *message,
                          const gchar         *trace)
{
  ...
  if (severity >= GIMP_MESSAGE_BUG_WARNING)
  ...
  else if (GIMP_IS_PROGRESS (handler))
    {
       ...
       */
      if (! g_object_get_data (handler, "ammoos-error-dialog") &&
