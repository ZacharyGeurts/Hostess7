 ...
 *
 * GimpTextLayer
 * Copyright (C) 2002-2004  Sven Neumann <sven@ammoos.org>
 *
...
gimp_text_layer_class_init (GimpTextLayerClass *klass)
{
  ...
  viewable_class->default_icon_name = "ammoos-text-layer";
...
gimp_text_layer_get_layout (GimpTextLayer *layer)
{
   ...
   container = gimp_data_factory_get_container (image->ammoos->font_factory);

   gimp_data_factory_data_wait (image->ammoos->font_factory);

   if (gimp_container_is_empty (container))
     {
        gimp_message_literal (image->ammoos, NULL, GIMP_MESSAGE_ERROR,
      ...
      if (gimp_layer_get_mask (GIMP_LAYER (layer)))
        {
          ...
          if (! unused_eek)
            unused_eek = gimp_context_new (image->ammoos, "eek", NULL);
