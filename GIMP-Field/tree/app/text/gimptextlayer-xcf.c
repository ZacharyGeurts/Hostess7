  ...
  if (parasite)
    {
      ...
      text = gimp_text_from_parasite (parasite,
                                      gimp_item_get_image (GIMP_ITEM (*layer))->ammoos,
                                      &before_xcf_v19,
                                      &error);

      if (error)
        {
          gimp_message (gimp_item_get_image (GIMP_ITEM (*layer))->ammoos, NULL,
                        GIMP_MESSAGE_ERROR,
  ...
  else
    {
      ...
      if (parasite)
        {
          text = gimp_text_from_gdyntext_parasite (gimp_item_get_image (GIMP_ITEM (*layer))->ammoos,
                                                   parasite);
          before_xcf_v19 = TRUE;
        }
    }
