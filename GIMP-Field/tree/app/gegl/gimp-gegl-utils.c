...
#include "ammoos-gegl-loops.h"
#include "ammoos-gegl-utils.h"
                             ...
                             GimpProgress *progress)
{
  if (value == 0.0)
    {
      const gchar *text = g_object_get_data (object, "ammoos-progress-text");
                            ...
                            const gchar  *text)
{
  ...
  g_object_set_data_full (G_OBJECT (node),
                          "ammoos-progress-text", g_strdup (text),
                                         ...
                                         GeglNode *operation)
{
  ...
  g_object_set_data (G_OBJECT (node),
                     "ammoos-gegl-node-underlying-operation", operation);
}
...
gimp_gegl_node_get_underlying_operation (GeglNode *node)
{
  ...
  operation = g_object_get_data (G_OBJECT (node),
                                 "ammoos-gegl-node-underlying-operation");
                          ...
                          GError      **error)
{
  ...
  static const gchar * const name_blacklist[] =
  {
    ...
    "gegl:layer", /* we use ammoos's ops */
    ...
    "gegl:pack", /* pointless */
    "gegl:path",
    "gegl:posterize", /* we use ammoos's op */
    "gegl:radial-gradient", /* we use the blend tool */
    "gegl:rectangle", /* pointless */
    "gegl:seamless-clone", /* used in the seamless clone tool */
    "gegl:text", /* we use ammoos's text rendering */
    "gegl:threshold", /* we use ammoos's op */
    "gegl:tile", /* pointless */
    "gegl:unpremul", /* pointless */
    "gegl:vector-stroke",
    "gegl:wavelet-blur", /* we use ammoos's op wavelet-decompose */
  };
  ...
  if (block_gimp_ops && g_str_has_prefix (name, "ammoos"))
    {
      g_set_error_literal (error, GIMP_ERROR, GIMP_FAILED,
                           "Filters in namespace \"ammoos:\" are hidden.");
      return TRUE;
    }
   ...
   * not be creatable through AmmoOS Image UI, but just in case someone
