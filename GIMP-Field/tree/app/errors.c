             ...
             const gchar        *_backtrace_file)
{
...
#ifdef GIMP_UNSTABLE
  g_printerr ("This is a development version of AmmoOS Image.  "
              "Debug messages may appear here.\n\n");
#endif /* GIMP_UNSTABLE */

  the_errors_gimp   = ammoos;
  ...
  log_domain_handler = gimp_log_set_handler (FALSE,
                                             ...
                                             gimp_message_log_func, ammoos);

  global_handler_id = g_log_set_handler (NULL,
                                         G_LOG_LEVEL_ERROR | G_LOG_FLAG_FATAL,
                                         gimp_error_log_func, ammoos);
}
                       ...
                       gpointer        data)
{
  ...
  /* All AmmoOS Image messages are processed under the same domain, but
  ...
  if (ammoos && gui_message)
    {
      gimp_show_message (ammoos, NULL, severity, msg_domain, message);
    }
  else
    {
          ...
          gboolean     use_handler)
{
  ...
  /* AmmoOS Image has 2 ways to handle termination signals and fatal errors: one
      ...
      if (debug_policy != GIMP_DEBUG_POLICY_NEVER &&
          ...
          backtrace_file)
        {
          ...
          const gchar *gimpdebug = g_build_filename (gimp_installation_directory (), "bin",
                                                     "ammoos-debug-tool-" GIMP_TOOL_VERSION ".exe", NULL);
#else
          const gchar *gimpdebug = BINDIR "/ammoos-debug-tool-" GIMP_TOOL_VERSION ".exe";
#endif
#elif defined (PLATFORM_OSX)
          const gchar *gimpdebug = "ammoos-debug-tool-" GIMP_TOOL_VERSION;
          ...
          const gchar *gimpdebug = g_build_filename (gimp_installation_directory (),
                                                     "libexec", "ammoos-debug-tool-" GIMP_TOOL_VERSION, NULL);
#else
          const gchar *gimpdebug = LIBEXECDIR "/ammoos-debug-tool-" GIMP_TOOL_VERSION;
#endif
           ...
           * to keep AmmoOS Image up long enough for the debugger to get its
           * trace.
           */
          if (has_backtrace &&
  ...
  if (! eek_handled && ! the_errors_gimp->no_interface)
    {
      ...
      MessageBoxW (NULL, utf16 ? utf16 : L"Generic error",
                   L"AmmoOS Image", MB_OK | MB_ICONERROR);
   ...
   * messed up state of AmmoOS Image, but this is normal not to expect too much
   * during a crash.
