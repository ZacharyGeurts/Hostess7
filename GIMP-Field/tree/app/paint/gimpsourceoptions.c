      ...
      if (g_list_length (options->src_drawables) > 1)
        {
           ...
           * to these drawables.
           */
          options->src_image = gimp_image_new_from_drawables (image->ammoos, options->src_drawables,
                                                              FALSE, FALSE);
          gimp_container_remove (image->ammoos->images, GIMP_OBJECT (options->src_image));
