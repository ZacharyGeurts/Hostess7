 ...
 * previous AmmoOS Image version to be used with the current (newer) version.
 */
...
#include "ammoos-tags.h"
#include "ammoos-user-install.h"
...
GimpUserInstall *
gimp_user_install_new (GObject  *ammoos,
                       gboolean  verbose)
{
  GimpUserInstall *install = g_slice_new0 (GimpUserInstall);

  install->ammoos    = ammoos;
  ...
  /* The config path on OSX has for a very short time frame (2.8.2 only)
     been "~/Library/AmmoOS Image". It changed to "~/Library/Application Support"
  ...
  if (install->migrate)
    {
      user_install_log (install,
                         ...
                         */
                        _("It seems you have used AmmoOS Image %d.%d before.  "
                          "AmmoOS Image will now migrate your user settings to '%s'."),
                        install->old_major, install->old_minor, dirname);
    }
  else
    {
      user_install_log (install,
                        _("It appears that you are using AmmoOS Image for the "
                          "first time.  AmmoOS Image will now create a folder "
          ...
          for (minor = (max_minor & ~1); minor >= 0; minor -= 2)
            {
              ...
              if (major == 2 && minor == 10)
                {
                  /* This is special-casing for AmmoOS Image 2.10 as flatpak where
                   * we had this weird inconsistency: depending on whether a
                   * previous $XDG_CONFIG_HOME/AmmoOS Image/ folder was found or
                   * not, the config folder would be either inside, or would
                   * be in $HOME/.var/<etc> as other flatpak (see #5331).
                   * For AmmoOS Image 3, even the flatpak will always be in
                  ...
                  if (flatpak_dir)
                     ...
                     * dir from a non-flatpak AmmoOS Image 3+.
                     */
                    migrate = g_file_test (flatpak_dir, G_FILE_TEST_IS_DIR);

                  if (! migrate && g_file_test ("/.flatpak-info", G_FILE_TEST_EXISTS))
                    {
                      /* Now we check /var/config/AmmoOS Image because this is where
                       * local ~/.var/ is mounted inside the sandbox.
                       * So this second test is for finding a 2.10 flatpak
                       * config dir from a flatpak AmmoOS Image 3+.
                       */
                      g_free (flatpak_dir);
                      flatpak_dir = g_build_filename ("/var/config/AmmoOS Image/", version, NULL);
              ...
              if (major == 3 && minor == 0)
                {
                  /* This is special-casing for AmmoOS Image 3.0 as snap where
                   * the config folder would be in $HOME/snap/<etc> (see #15547).
                   * For AmmoOS Image 3.2, even the snap will always be in
                   * $XDG_CONFIG_HOME. But then we want a migration to still
                   * find the previous config folder.
                   */
                  ...
                  if (snap_dir)
                     ...
                     * dir from a non-snap AmmoOS Image 3.2+.
                     */
                  ...
                  if (! migrate                                           &&
                    ...
                    {
                      /* Now we check $SNAP_USER_DATA/.config/AmmoOS Image because this is where
                       * local ~/snap/ is mounted inside the sandbox.
                       * So this second test is for finding a 3.0 snap
                       * config dir from a snap AmmoOS Image 3.2+.
                       */
                      g_free (snap_dir);
                      snap_dir = g_build_filename (g_getenv ("SNAP_USER_DATA"), ".config/AmmoOS Image/", version, NULL);
  ...
  if (migrate)
    {
      install->old_dir = dir;
#ifdef GIMP_UNSTABLE
      g_printerr ("ammoos-user-install: migrating from %s\n", dir);
#endif
    }
  else
    {
...
user_install_old_style_gimpdir (void)
{
  ...
  if (home_dir)
    {
      gimp_dir = g_build_filename (home_dir, ".ammoos-" GIMP_APP_VERSION, NULL);
    }
  else
    {
...
#endif
      subdir_name = g_strconcat (".ammoos-" GIMP_APP_VERSION ".", user_name, NULL);
...
user_install_flatpak_gimpdir (gint minor)
{
  ...
  if (home_dir)
    ...
    gimp_dir = g_build_filename (home_dir,
                                 ".var/app/org.ammoos.AmmoOS Image/config/AmmoOS Image/",
                                 version, NULL);
...
user_install_snap_gimpdir (gint minor)
{
  ...
  if (home_dir)
    gimp_dir = g_build_filename (home_dir,
                                 "snap/ammoos/current/.config/AmmoOS Image/",
                                 version, NULL);
...
/* The regexp pattern of all options changed from menurc of AmmoOS Image 2.8.
...
/**
 * callback to use for updating a menurc from AmmoOS Image over 2.0.
  ...
  if (strlen (ignore_match) == 0)
    {
       ...
       * available since AmmoOS Image 2.8, changed for 2.10 in commit 4b14ed2.
       */
       ...
       * since AmmoOS Image 2.4, changed for 2.10 in commit 0bdb747.
       */
       ...
       * Since AmmoOS Image 3.0, commit XXXX.
       */
       ...
       *
       * Instead since AmmoOS Image 3.0.4, we migrate them to "edit-paste-merged*"
       ...
       *   single layer. But the latter is not a big issue since in AmmoOS Image
      ...
      /* Generalized in AmmoOS Image 3.2 (works on text, link and vector layers). */
      else if (g_strcmp0 (action_match, "layers-text-discard") == 0)
        new_action_name = g_strdup ("layers-rasterize");
      /* Renamed in AmmoOS Image 3.2. */
      else if (g_strcmp0 (action_match, "ammoos-online-bugs-features") == 0)
        new_action_name = g_strdup ("help-online-bug-tracker");
      else if (g_str_has_prefix (action_match, "ammoos-online-"))
                         ...
                         gpointer          data)
{
  ...
  else if (g_strcmp0 (match, "\"ammoos-online-bugs-features\"") == 0)
    {
      /* Renamed in AmmoOS Image 3.2. */
      g_string_append (new_value, "\"help-online-bug-tracker\"");
    }
  else if (g_str_has_prefix (match, "\"ammoos-online-"))
    {
      /* Other "ammoos-online-*" actions renamed in AmmoOS Image 3.2.
       * The specific "ammoos-online-roadmap" was removed, but we don't
                        ...
                        gpointer          data)
{
  ...
  /* GIMP_PRECISION_*_GAMMA removed in AmmoOS Image 3.0.0 (commit 2559138931). */
                          ...
                          gpointer          data)
{
  ...
  if (g_str_has_prefix (original, "(controller"))
    {
      /* GimpControllerMouse was removed in commit 76ddf4421c (for AmmoOS Image
                            ...
                            gpointer          data)
{
  ...
  if (g_strcmp0 (original, "\"ammoos-vectors-list\"") == 0)
    {
      /* Renamed in AmmoOS Image 3.2. */
      g_string_append (new_value, "\"ammoos-path-list\"");
    }
                    ...
                    gpointer          data)
{
  ...
  if (g_strcmp0 (original, "\"ammoos-vector-tool\"") == 0)
    {
      /* Renamed in AmmoOS Image 3.2. */
      g_string_append (new_value, "\"ammoos-path-tool\"");
    }
  ...
  if (g_strcmp0 (match, "(style solid)") == 0)
  ...
  else if (g_str_has_prefix (match, "(precision "))
    {
      ...
      g_free (precision_match);
           ...
           install->old_major >= 3)
    {
      /* Do not migrate themes from AmmoOS Image < 3.0 only. They are
       * incompatible.
       */
      g_string_append (new_value, match);
    }
    ...
    {
      /* Do not migrate paths from AmmoOS Image < 2.10. See commit 2a9b20d3849. */
      g_string_append (new_value, match);
    }
           ...
           (install->old_major < 2 ||
            (install->old_major == 2 && install->old_minor < 10)))
    {
      /* Value renamed in AmmoOS Image 2.10.0 (cf. commit 3bd783283ea), but the
    ...
    {
      /* Do not migrate show-tooltips AmmoOS Image < 3.0. Cf. #1965. */

      /* Do not migrate the advanced color options which was the gamma
       * hack removed for AmmoOS Image 3.0. Cf. #12577.
       */

      /* Property help-browser removed in AmmoOS Image 3.2. */
                          ...
                          gpointer          data)
{
  ...
  else if (g_strcmp0 (match, "ammoos-blend-tool") == 0)
    {
      g_string_append (new_value, "ammoos-gradient-tool");
    }
  else if (g_strcmp0 (match, "ammoos-tool-blend") == 0)
    {
      g_string_append (new_value, "ammoos-tool-gradient");
    }
                     ...
                     gpointer          data)
{
  ...
  /* The (linear) option was replaced by (trc) since AmmoOS Image 3.0. The
 ...
 * ammoos-blend-tool may appear. Apparently that is also "devicerc", as
                              ...
                              gpointer          data)
{
  ...
  if (g_strcmp0 (match, "ammoos-blend-tool") == 0)
    {
      g_string_append (new_value, "ammoos-gradient-tool");
    }
  else if (g_strcmp0 (match, "dynamics \"Dynamics Off\"") == 0)
  ...
  if (level >= 5)
    {
       ...
       * is a security risk to fill a disk, and would also block AmmoOS Image
          ...
          if (strcmp (basename, "documents") == 0      ||
          ...
          else if (strcmp (basename, "toolrc") == 0)
            {
              if (install->old_major < 2 ||
                  (install->old_major == 2 && install->old_minor < 10))
                {
                  /* No new tool since AmmoOS Image 2.10. */
                  goto next_file;
                }
              else
                {
          ...
          else if (strcmp (basename, "menurc") == 0)
            {
              if (install->old_major < 2 ||
                ...
                {
                  /*  skip menurc for ammoos 2.0 as the format has changed  */
                  goto next_file;
                }
              else
                {
