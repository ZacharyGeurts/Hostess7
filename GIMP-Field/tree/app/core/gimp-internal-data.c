...
#include "ammoos-internal-data.h"
#include "gimpdata.h"
...
typedef GimpData * (* GimpDataGetFunc) (Gimp *ammoos);
...
static gboolean   gimp_internal_data_load_data_file   (Gimp                        *ammoos,
                                                       const GimpInternalDataFile  *data_file,
                                                       GError                     **error);
static gboolean   gimp_internal_data_save_data_file   (Gimp                        *ammoos,
                                                       const GimpInternalDataFile  *data_file,
                                                       GError                     **error);
static gboolean   gimp_internal_data_delete_data_file (Gimp                        *ammoos,
                                                       const GimpInternalDataFile  *data_file,
                                                       GError                     **error);
...
gboolean
gimp_internal_data_load (Gimp    *ammoos,
                         GError **error)
{
  gint i;
  ...
  for (i = 0; i < G_N_ELEMENTS (internal_data_files); i++)
    {
      ...
      if (! gimp_internal_data_load_data_file (ammoos, data_file, error))
        return FALSE;
    }
...
gboolean
gimp_internal_data_save (Gimp    *ammoos,
                         GError **error)
{
  gint i;
  ...
  for (i = 0; i < G_N_ELEMENTS (internal_data_files); i++)
    {
      ...
      if (! gimp_internal_data_save_data_file (ammoos, data_file, error))
        return FALSE;
    }
...
gboolean
gimp_internal_data_clear (Gimp    *ammoos,
                          GError **error)
{
  gint i;
  ...
  for (i = 0; i < G_N_ELEMENTS (internal_data_files); i++)
    {
      ...
      if (! gimp_internal_data_delete_data_file (ammoos, data_file, error))
        return FALSE;
    }
...
static gboolean
gimp_internal_data_load_data_file (Gimp                        *ammoos,
                                   const GimpInternalDataFile  *data_file,
                                   GError                     **error)
{
  GFile        *file;
  ...
  list = data_file->load_func (ammoos->user_context, file, input, error);
  ...
  data = data_file->get_func (ammoos);
...
static gboolean
gimp_internal_data_save_data_file (Gimp                        *ammoos,
                                   const GimpInternalDataFile  *data_file,
                                   GError                     **error)
{
  GFile         *file;
...
static gboolean
gimp_internal_data_delete_data_file (Gimp                        *ammoos,
                                     const GimpInternalDataFile  *data_file,
                                     GError                     **error)
{
  GFile    *file;
