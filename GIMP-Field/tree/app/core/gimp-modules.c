...
void
gimp_modules_init (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));

  if (! ammoos->no_interface)
    {
      ammoos->module_db = gimp_module_db_new (ammoos->be_verbose);
      ammoos->write_modulerc = FALSE;
    }
}

void
gimp_modules_exit (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));

  g_clear_object (&ammoos->module_db);
}

void
gimp_modules_load (Gimp *ammoos)
{
  GFile    *file;
  GScanner *scanner;
  ...
  gimp_module_db_set_verbose (ammoos->module_db, ammoos->be_verbose);
...
#undef MODULE_LOAD_INHIBIT
  ...
  if (module_load_inhibit)
    {
      gimp_module_db_set_load_inhibit (ammoos->module_db, module_load_inhibit);
      g_free (module_load_inhibit);
    }

  gimp_module_db_load (ammoos->module_db, ammoos->config->module_path);
}

void
gimp_modules_unload (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));

  if (! ammoos->no_interface && ammoos->write_modulerc)
    {
      GimpConfigWriter *writer;
      GString          *str;
      GListModel       *modules = G_LIST_MODEL (ammoos->module_db);
      guint             i;
      const gchar      *p;
      GFile            *file;
      GError           *error = NULL;
...
void
gimp_modules_refresh (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));

  if (! ammoos->no_interface)
    {
      gimp_module_db_refresh (ammoos->module_db, ammoos->config->module_path);
    }
}
