...
gimp_imagefile_class_init (GimpImagefileClass *klass)
{
  ...
  creator = g_strdup_printf ("ammoos-%d.%d",
...
GimpImagefile *
gimp_imagefile_new (Gimp  *ammoos,
                    GFile *file)
{
  ...
  GET_PRIVATE (imagefile)->ammoos = ammoos;
  ...
  if (uri)
    {
      GimpImagefile *documents_imagefile = (GimpImagefile *)
        gimp_container_get_child_by_name (private->ammoos->documents, uri);
                                 ...
                                 GError        **error)
{
  ...
  if (image_state == GIMP_THUMB_STATE_REMOTE ||
      image_state >= GIMP_THUMB_STATE_EXISTS)
    {
      ...
      image = file_open_thumbnail (private->ammoos, context, progress,
                                   private->file, size,
      ...
      if (image)
      ...
      else
        {
          ...
          image = gimp_image_metadata_load_thumbnail (private->ammoos, private->file, &width, &height, &format, error);
          if (image)
            {
              gimp_thumbnail_set_info (private->thumbnail,
                                       mime_type, width, height,
          ...
          else
            {
              ...
              image = file_open_image (private->ammoos, context, progress,
                                      ...
                                      gboolean       replace)
{
  ...
  local = gimp_imagefile_new (private->ammoos, private->file);
...
gimp_imagefile_check_thumbnail (GimpImagefile *imagefile)
{
  ...
  size = private->ammoos->config->thumbnail_size;
      ...
      if (error)
        {
          ...
          gimp_message (private->ammoos, NULL, GIMP_MESSAGE_ERROR,
