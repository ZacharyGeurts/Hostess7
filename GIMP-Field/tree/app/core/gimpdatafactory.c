 ...
 *
 * gimpdatafactory.c
 * Copyright (C) 2001-2018 Michael Natterer <mitch@ammoos.org>
 *
...
gimp_data_factory_constructed (GObject *object)
{
  ...
  gimp_assert (GIMP_IS_GIMP (priv->ammoos));
                                ...
                                GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      priv->ammoos = g_value_get_object (value); /* don't ref */
      break;
                                ...
                                GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      g_value_set_object (value, priv->ammoos);
      break;
...
gimp_data_factory_real_data_save (GimpDataFactory *factory)
{
  ...
  if (! writable_dir)
    {
      gimp_message (priv->ammoos, NULL, GIMP_MESSAGE_ERROR,
                             ...
                             gboolean         no_data)
{
  ...
  g_signal_connect_object (priv->ammoos->config, signal_name,
                           G_CALLBACK (gimp_data_factory_path_notify),
                           factory, 0);
  g_free (signal_name);

  signal_name = g_strdup_printf ("notify::%s", priv->ext_property_name);
  g_signal_connect_object (priv->ammoos->extension_manager, signal_name,
                           G_CALLBACK (gimp_data_factory_path_notify),
                           factory, 0);
  g_free (signal_name);
}
...
gimp_data_factory_data_wait (GimpDataFactory *factory)
{
  ...
  gimp_wait (priv->ammoos, waitable,
                            ...
                            gboolean         is_internal)
{
  ...
  if (! data && ! strcmp (name, "Standard"))
    data = (GimpObject *) gimp_data_factory_data_get_standard (factory,
                                                               gimp_get_user_context (priv->ammoos));
...
gimp_data_factory_get_data_path (GimpDataFactory *factory)
{
  ...
  g_object_get (priv->ammoos->config,
...
gimp_data_factory_get_data_path_ext (GimpDataFactory *factory)
{
  ...
  g_object_get (priv->ammoos->extension_manager,
                               ...
                               GimpDataFactory  *factory)
{
  ...
  gimp_set_busy (priv->ammoos);

  gimp_data_factory_data_refresh (factory, gimp_get_user_context (priv->ammoos));

  gimp_unset_busy (priv->ammoos);
}
