    ...
    case PROP_GIMP:
      {
        ...
        display->ammoos = g_value_get_object (value); /* don't ref the ammoos */
        display->config = GIMP_DISPLAY_CONFIG (display->ammoos->config);

        do
          {
            id = display->ammoos->next_display_id++;

            if (display->ammoos->next_display_id == G_MAXINT)
              display->ammoos->next_display_id = 1;
          }
        while (gimp_display_get_by_id (display->ammoos, id));
                           ...
                           GParamSpec *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_GIMP:
      g_value_set_object (value, display->ammoos);
      break;
...
GimpDisplay *
gimp_display_get_by_id (Gimp *ammoos,
                        gint  id)
{
  GList *list;
