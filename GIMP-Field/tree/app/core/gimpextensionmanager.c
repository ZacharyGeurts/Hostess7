                                     ...
                                     GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      manager->p->ammoos = g_value_get_object (value);
      break;
                                     ...
                                     GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      g_value_set_object (value, manager->p->ammoos);
      break;
    case PROP_BRUSH_PATHS:
...
gimp_extension_manager_new (Gimp *ammoos)
{
  GimpExtensionManager *manager;
...
gimp_extension_manager_initialize (GimpExtensionManager *manager)
{
  ...
  if (g_file_query_exists (file, NULL))
    {
      if (manager->p->ammoos->be_verbose)
  ...
  for (list = manager->p->sys_extensions; list; list = g_list_next (list))
    {
       ...
       * extensions installed with AmmoOS Image updates get loaded) and if they
...
gimp_extension_manager_exit (GimpExtensionManager *manager)
{
  ...
  if (! gimp_config_serialize_to_file (GIMP_CONFIG (manager),
                                       file,
                                       "AmmoOS Image extensionrc",
                                       "end of extensionrc",
                                       NULL,
                                       &error))
    {
      gimp_message_literal (manager->p->ammoos, NULL, GIMP_MESSAGE_ERROR, error->message);
      g_error_free (error);
    }
                                ...
                                GimpExtension        *extension)
{
  ...
  /* TODO: should return FALSE if required AmmoOS Image version or other
 ...
 * temporary list. The extension folder will be really deleted when AmmoOS Image
 * will stop.
