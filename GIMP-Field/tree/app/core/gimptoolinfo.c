...
GimpToolInfo *
gimp_tool_info_new (Gimp                *ammoos,
                    GType                tool_type,
                    ...
                    const gchar         *icon_name)
{
  ...
  paint_info = (GimpPaintInfo *)
    gimp_container_get_child_by_name (ammoos->paint_info_list, paint_core_name);
  ...
  if (tool_info->tool_options_type != GIMP_TYPE_TOOL_OPTIONS)
    {
      ...
      presets = gimp_data_factory_get_container (ammoos->tool_preset_factory);
...
void
gimp_tool_info_set_standard (Gimp         *ammoos,
                             GimpToolInfo *tool_info)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
  g_return_if_fail (! tool_info || GIMP_IS_TOOL_INFO (tool_info));

  g_set_object (&ammoos->standard_tool_info, tool_info);
}

GimpToolInfo *
gimp_tool_info_get_standard (Gimp *ammoos)
{
  g_return_val_if_fail (GIMP_IS_GIMP (ammoos), NULL);

  return ammoos->standard_tool_info;
}
...
gimp_tool_info_get_action_name (GimpToolInfo *tool_info)
{
  ...
  g_return_val_if_fail (g_str_has_prefix (identifier, "ammoos-"), NULL);
  g_return_val_if_fail (g_str_has_suffix (identifier, "-tool"), NULL);

  tmp = g_strndup (identifier + strlen ("ammoos-"),
                    strlen (identifier) - strlen ("ammoos--tool"));
