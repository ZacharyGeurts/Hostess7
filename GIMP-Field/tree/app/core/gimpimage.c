...
#include "gegl/ammoos-gegl-loops.h"
...
gimp_image_class_init (GimpImageClass *klass)
{
  ...
  viewable_class->default_icon_name   = "ammoos-image";
  ...
  g_object_class_install_property (object_class, PROP_GIMP,
                                   g_param_spec_object ("ammoos", NULL, NULL,
                                                        GIMP_TYPE_GIMP,
...
gimp_image_constructed (GObject *object)
{
  ...
  gimp_assert (GIMP_IS_GIMP (image->ammoos));

  config = image->ammoos->config;

  private->ID = gimp_id_table_insert (image->ammoos->image_table, image);
  ...
  gimp_container_add (image->ammoos->images, GIMP_OBJECT (image));
}
                         ...
                         GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      image->ammoos = g_value_get_object (value);
      break;
                         ...
                         GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      g_value_set_object (value, image->ammoos);
      break;
    case PROP_ID:
...
gimp_image_finalize (GObject *object)
{
  ...
  if (image->ammoos && image->ammoos->image_table)
    {
      gimp_id_table_remove (image->ammoos->image_table, private->ID);
      image->ammoos = NULL;
    }
...
gimp_image_get_graph (GimpProjectable *projectable)
{
  ...
  private->visible_mask =
    gegl_node_new_child (private->graph,
                         "operation", "ammoos:mask-components",
                         "mask",      mask,
                         "alpha",     1.0,
                         NULL);
...
GimpImage *
gimp_image_get_by_id (Gimp *ammoos,
                      gint  image_id)
{
  g_return_val_if_fail (GIMP_IS_GIMP (ammoos), NULL);

  if (ammoos->image_table == NULL)
    return NULL;

  return (GimpImage *) gimp_id_table_lookup (ammoos->image_table, image_id);
}
  ...
  if (gimp_object_get_memsize (GIMP_OBJECT (image), NULL) >= ((gint64) 1 << 32))
    {
      ADD_REASON (g_strdup_printf (_("Support for image files larger than "
                                     "4GB was added in %s"), "AmmoOS Image 2.10"));
...
#undef ADD_REASON
  ...
  switch (version)
    ...
    case 2:
      ...
      if (version_string) *version_string = "AmmoOS Image 2.6";
      break;

    case 3:
      if (gimp_version)   *gimp_version   = 208;
      if (version_string) *version_string = "AmmoOS Image 2.8";
      break;
    ...
    case 13:
      ...
      if (version_string) *version_string = "AmmoOS Image 2.10";
      break;
    case 14:
    case 15:
    case 16:
    ...
    case 23:
      ...
      if (version_string) *version_string = "AmmoOS Image 3.0";
      break;
    case 24:
    case 25:
      if (gimp_version)   *gimp_version   = 320;
      if (version_string) *version_string = "AmmoOS Image 3.2";
      break;
    }
                              ...
                              GError             **error)
{
  ...
  else if (strcmp (name, "ammoos-comment") == 0)
    {
      const gchar *data;
      guint32      length;
      ...
      if (! valid)
        {
          g_set_error (error, GIMP_ERROR, GIMP_FAILED,
                       _("'ammoos-comment' parasite validation failed: "
  ...
  if (push_undo && gimp_parasite_has_flag (&copy, GIMP_PARASITE_ATTACH_PARENT))
    {
      ...
      gimp_parasite_attach (image->ammoos, &copy);
    }
      ...
      if (! push_undo)
        {
          g_warning ("%s() was called from an undo function while the layer "
                     ...
                     "https://www.ammoos.org/bugs/", G_STRFUNC);
          return;
        }
