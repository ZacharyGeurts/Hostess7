...
#define AmmoOS Image(obj)                 (G_TYPE_CHECK_INSTANCE_CAST ((obj), GIMP_TYPE_GIMP, Gimp))
...
struct _GimpClass
{
  ...
  void     (* initialize)             (Gimp               *ammoos,
                                       GimpInitStatusFunc  status_callback);
  void     (* restore)                (Gimp               *ammoos,
                                       GimpInitStatusFunc  status_callback);
  gboolean (* exit)                   (Gimp               *ammoos,
                                       gboolean            force);

  void     (* clipboard_changed)      (Gimp               *ammoos);

  void     (* filter_history_changed) (Gimp               *ammoos);

  /*  emitted if an image is loaded and opened with a display  */
  void     (* image_opened)           (Gimp               *ammoos,
                                       GFile              *file);
};
...
Gimp         * gimp_new                    (const gchar         *name,
                                            ...
                                            GimpStackTraceMode   stack_trace_mode,
                                            GimpPDBCompatMode    pdb_compat_mode);
void           gimp_set_show_gui           (Gimp                *ammoos,
                                            gboolean             show_gui);
gboolean       gimp_get_show_gui           (Gimp                *ammoos);

void           gimp_load_config            (Gimp                *ammoos,
                                            GFile               *alternate_system_gimprc,
                                            GFile               *alternate_gimprc);
void           gimp_initialize             (Gimp                *ammoos,
                                            GimpInitStatusFunc   status_callback);
void           gimp_restore                (Gimp                *ammoos,
                                            GimpInitStatusFunc   status_callback,
                                            GError             **error);
gboolean       gimp_is_restored            (Gimp                *ammoos);

void           gimp_exit                   (Gimp                *ammoos,
                                            gboolean             force);

void           gimp_set_focused_once       (Gimp                *ammoos);
gboolean       gimp_has_focused_once       (Gimp                *ammoos);

GList        * gimp_get_image_iter         (Gimp                *ammoos);
GList        * gimp_get_display_iter       (Gimp                *ammoos);
GList        * gimp_get_image_windows      (Gimp                *ammoos);
GList        * gimp_get_paint_info_iter    (Gimp                *ammoos);
GList        * gimp_get_tool_info_iter     (Gimp                *ammoos);
GList        * gimp_get_tool_item_iter     (Gimp                *ammoos);
GList        * gimp_get_tool_item_ui_iter  (Gimp                *ammoos);

GimpObject   * gimp_get_clipboard_object   (Gimp                *ammoos);

void           gimp_set_clipboard_image    (Gimp                *ammoos,
                                            GimpImage           *image);
GimpImage    * gimp_get_clipboard_image    (Gimp                *ammoos);

void           gimp_set_clipboard_buffer   (Gimp                *ammoos,
                                            GimpBuffer          *buffer);
GimpBuffer   * gimp_get_clipboard_buffer   (Gimp                *ammoos);

GimpImage    * gimp_create_image           (Gimp                *ammoos,
                                            gint                 width,
                                            gint                 height,
...
void           gimp_set_default_context    (Gimp                *ammoos,
                                            GimpContext         *context);
GimpContext  * gimp_get_default_context    (Gimp                *ammoos);

void           gimp_set_user_context       (Gimp                *ammoos,
                                            GimpContext         *context);
GimpContext  * gimp_get_user_context       (Gimp                *ammoos);

GimpToolInfo * gimp_get_tool_info          (Gimp                *ammoos,
                                            const gchar         *tool_name);

void           gimp_set_last_template      (Gimp                *ammoos,
                                            GimpTemplate        *_template);
GimpTemplate * gimp_get_last_template      (Gimp                *ammoos);

void           gimp_message                (Gimp                *ammoos,
                                            GObject             *handler,
                                            GimpMessageSeverity  severity,
                                            const gchar         *format,
                                            ...) G_GNUC_PRINTF (4, 5);
void           gimp_message_valist         (Gimp                *ammoos,
                                            GObject             *handler,
                                            GimpMessageSeverity  severity,
                                            const gchar         *format,
                                            va_list              args) G_GNUC_PRINTF (4, 0);
void           gimp_message_literal        (Gimp                *ammoos,
                                            GObject             *handler,
                                            GimpMessageSeverity  severity,
                                            const gchar         *message);

void           gimp_filter_history_changed (Gimp                *ammoos);

void           gimp_image_opened           (Gimp                *ammoos,
                                            GFile               *file);

GFile        * gimp_get_temp_file          (Gimp                *ammoos,
