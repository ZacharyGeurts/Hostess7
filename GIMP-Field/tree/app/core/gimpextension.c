                     ...
                     GError        **error)
{
  ...
  if (success &&
      ! g_ptr_array_find_with_equal_func (extends, "org.ammoos.AmmoOS Image",
                                          g_str_equal, NULL))
    {
      ...
      if (error && *error == NULL)
        *error = g_error_new (GIMP_EXTENSION_ERROR,
                              GIMP_EXTENSION_BAD_APPDATA,
                              _("Extension AppData must extend \"org.ammoos.AmmoOS Image\"."));
      success = FALSE;
    }
      ...
      for (guint i = 0; i < relations->len; i++)
        {
          ...
          if (as_relation_get_item_kind(relation) == AS_RELATION_ITEM_KIND_ID &&
              g_strcmp0(as_relation_get_value_str(relation), "org.ammoos.AmmoOS Image") == 0)
            {
              has_require = TRUE;
      ...
      if (error && *error == NULL)
        {
          *error = g_error_new (GIMP_EXTENSION_ERROR,
                                GIMP_EXTENSION_FAILED,
                                _("<requires><id>org.ammoos.AmmoOS Image</id></requires> for version comparison is mandatory."));
        }
    }
                    ...
                    GError        **error)
{
  ...
  value = g_hash_table_lookup (metadata, "AmmoOS Image::brush-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::dynamics-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::mypaint-brush-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::pattern-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::gradient-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::palette-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::tool-preset-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::plug-in-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::splash-path");
  ...
  if (! (*error))
    {
      value = g_hash_table_lookup (metadata, "AmmoOS Image::theme-path");
