 ...
 * Copyright (C) 2015-2018 Michael Natterer <mitch@ammoos.org>
 *
  ...
  if ((profile = gimp_image_get_color_profile (image)))
    {
      ...
      policy = GIMP_DIALOG_CONFIG (image->ammoos->config)->color_profile_policy;
      ...
      if (policy == GIMP_COLOR_PROFILE_POLICY_ASK)
        {
          if (gimp_color_profile_is_equal (profile, gimp_image_get_builtin_color_profile (image)) ||
          ...
          else if (interactive)
            {
              ...
              policy = gimp_query_profile_policy (image->ammoos, image, context,
                                                  &dest_profile,
                                                  &intent, &bpc,
                                                  &dont_ask);
              ...
              if (dont_ask)
                {
                  g_object_set (G_OBJECT (image->ammoos->config),
          ...
          if (! dest_profile)
            {
              if (policy == GIMP_COLOR_PROFILE_POLICY_CONVERT_PREFERRED)
                {
                  if (gimp_image_get_base_type (image) == GIMP_GRAY)
                    dest_profile = gimp_color_config_get_gray_color_profile (image->ammoos->config->color_management, NULL);
                  else
                    dest_profile = gimp_color_config_get_rgb_color_profile (image->ammoos->config->color_management, NULL);
                }
