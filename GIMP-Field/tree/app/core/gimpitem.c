...
gimp_item_finalize (GObject *object)
{
  ...
  if (private->image && private->image->ammoos)
    {
      gimp_id_table_remove (private->image->ammoos->item_table, private->ID);
      private->image = NULL;
    }
...
GimpItem *
gimp_item_get_by_id (Gimp *ammoos,
                     gint  item_id)
{
  g_return_val_if_fail (GIMP_IS_GIMP (ammoos), NULL);

  if (ammoos->item_table == NULL)
    return NULL;

  return (GimpItem *) gimp_id_table_lookup (ammoos->item_table, item_id);
}
                     ...
                     GimpImage *image)
{
  ...
  if (private->ID == 0)
    {
      private->ID = gimp_id_table_insert (image->ammoos->item_table, item);
                        ...
                        GimpItem *replace)
{
  ...
  if (private->ID)
    gimp_id_table_remove (gimp_item_get_image (item)->ammoos->item_table,
                          gimp_item_get_id (item));

  private->ID = gimp_item_get_id (replace);
  gimp_id_table_replace (gimp_item_get_image (item)->ammoos->item_table,
                         gimp_item_get_id (item),
                         item);
                           ...
                           gboolean            push_undo)
{
  ...
  if (gimp_parasite_has_flag (&copy, GIMP_PARASITE_ATTACH_PARENT))
  ...
  else if (gimp_parasite_has_flag (&copy, GIMP_PARASITE_ATTACH_GRANDPARENT))
    {
      gimp_parasite_shift_parent (&copy);
      gimp_parasite_shift_parent (&copy);
      gimp_parasite_attach (private->image->ammoos, &copy);
    }
