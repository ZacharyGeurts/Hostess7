...
gimp_context_constructed (GObject *object)
{
  ...
  ammoos = GIMP_CONTEXT (object)->ammoos;

  gimp_assert (GIMP_IS_GIMP (ammoos));

  ammoos->context_list = g_list_prepend (ammoos->context_list, object);

  g_signal_connect_object (ammoos->images, "remove",
                           G_CALLBACK (gimp_context_image_removed),
                           object, 0);
  g_signal_connect_object (ammoos->displays, "remove",
                           G_CALLBACK (gimp_context_display_removed),
                           object, 0);

  g_signal_connect_object (ammoos->tool_info_list, "remove",
                           G_CALLBACK (gimp_context_tool_removed),
                           object, 0);
  g_signal_connect_object (ammoos->tool_info_list, "thaw",
                           G_CALLBACK (gimp_context_tool_list_thaw),
                           object, 0);

  g_signal_connect_object (ammoos->paint_info_list, "remove",
                           G_CALLBACK (gimp_context_paint_info_removed),
                           object, 0);
  g_signal_connect_object (ammoos->paint_info_list, "thaw",
  ...
  container = gimp_data_factory_get_container (ammoos->dynamics_factory);
  g_signal_connect_object (container, "remove",
  ...
  container = gimp_data_factory_get_container (ammoos->mybrush_factory);
  g_signal_connect_object (container, "remove",
  ...
  container = gimp_data_factory_get_container (ammoos->tool_preset_factory);
  g_signal_connect_object (container, "remove",
  ...
  g_signal_connect_object (ammoos->named_buffers, "remove",
                           G_CALLBACK (gimp_context_buffer_removed),
                           object, 0);
  g_signal_connect_object (ammoos->named_buffers, "thaw",
                           G_CALLBACK (gimp_context_buffer_list_thaw),
                           object, 0);

  g_signal_connect_object (ammoos->documents, "remove",
                           G_CALLBACK (gimp_context_imagefile_removed),
                           object, 0);
  g_signal_connect_object (ammoos->documents, "thaw",
                           G_CALLBACK (gimp_context_imagefile_list_thaw),
                           object, 0);

  g_signal_connect_object (ammoos->templates, "remove",
                           G_CALLBACK (gimp_context_template_removed),
                           object, 0);
  g_signal_connect_object (ammoos->templates, "thaw",
                           G_CALLBACK (gimp_context_template_list_thaw),
                           object, 0);

  gimp_context_set_paint_info (GIMP_CONTEXT (object),
                               gimp_paint_info_get_standard (ammoos));
}
...
gimp_context_dispose (GObject *object)
{
  ...
  if (context->ammoos)
    {
      context->ammoos->context_list = g_list_remove (context->ammoos->context_list,
                                                   context);
      context->ammoos = NULL;
    }
                           ...
                           GParamSpec   *pspec)
{
  ...
  switch (property_id)
    ...
    case GIMP_CONTEXT_PROP_GIMP:
      context->ammoos = g_value_get_object (value);
      break;
                           ...
                           GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case GIMP_CONTEXT_PROP_GIMP:
      g_value_set_object (value, context->ammoos);
      break;
    case GIMP_CONTEXT_PROP_IMAGE:
                                   ...
                                   GTokenType *expected)
{
  ...
  switch (property_id)
    {
    case GIMP_CONTEXT_PROP_TOOL:
      container = context->ammoos->tool_info_list;
      standard  = gimp_tool_info_get_standard (context->ammoos);
      name_loc  = &context->tool_name;
      break;

    case GIMP_CONTEXT_PROP_PAINT_INFO:
      container = context->ammoos->paint_info_list;
      standard  = gimp_paint_info_get_standard (context->ammoos);
      name_loc  = &context->paint_name;
      break;

    case GIMP_CONTEXT_PROP_BRUSH:
      container = gimp_data_factory_get_container (context->ammoos->brush_factory);
      standard  = gimp_brush_get_standard (context);
      name_loc  = &context->brush_name;
      break;

    case GIMP_CONTEXT_PROP_DYNAMICS:
      container = gimp_data_factory_get_container (context->ammoos->dynamics_factory);
      standard  = gimp_dynamics_get_standard (context);
      name_loc  = &context->dynamics_name;
      break;

    case GIMP_CONTEXT_PROP_MYBRUSH:
      container = gimp_data_factory_get_container (context->ammoos->mybrush_factory);
      standard  = gimp_mybrush_get_standard (context);
      name_loc  = &context->mybrush_name;
      break;

    case GIMP_CONTEXT_PROP_PATTERN:
      container = gimp_data_factory_get_container (context->ammoos->pattern_factory);
      standard  = gimp_pattern_get_standard (context);
      name_loc  = &context->pattern_name;
      break;

    case GIMP_CONTEXT_PROP_GRADIENT:
      container = gimp_data_factory_get_container (context->ammoos->gradient_factory);
      standard  = gimp_gradient_get_standard (context);
      name_loc  = &context->gradient_name;
      break;

    case GIMP_CONTEXT_PROP_PALETTE:
      container = gimp_data_factory_get_container (context->ammoos->palette_factory);
      standard  = gimp_palette_get_standard (context);
      name_loc  = &context->palette_name;
      break;

    case GIMP_CONTEXT_PROP_FONT:
      container = gimp_data_factory_get_container (context->ammoos->font_factory);
      standard  = gimp_font_get_standard ();
      name_loc  = &context->font_name;
      break;

    case GIMP_CONTEXT_PROP_TOOL_PRESET:
      container = gimp_data_factory_get_container (context->ammoos->tool_preset_factory);
      standard  = NULL;
  ...
  if (pspec->owner_type == GIMP_TYPE_CONTEXT)
    {
       ...
       *  example notifications on the context's "ammoos" property
       */
                             ...
                             GimpContext   *context)
{
  ...
  if (! context->tool_name)
    context->tool_name = g_strdup ("ammoos-paintbrush-tool");

  tool_info = gimp_context_find_object (context, container,
                                        context->tool_name,
                                        gimp_tool_info_get_standard (context->ammoos));
                            ...
                            GimpToolInfo *tool_info)
{
  ...
  if (context->tool_name &&
      tool_info != gimp_tool_info_get_standard (context->ammoos))
    {
      g_clear_pointer (&context->tool_name, g_free);
    }
  ...
  if (tool_info)
    {
      ...
      if (tool_info != gimp_tool_info_get_standard (context->ammoos))
        context->tool_name = g_strdup (gimp_object_get_name (tool_info));
                                   ...
                                   GimpContext   *context)
{
  ...
  if (! context->paint_name)
    context->paint_name = g_strdup ("ammoos-paintbrush");

  paint_info = gimp_context_find_object (context, container,
                                         context->paint_name,
                                         gimp_paint_info_get_standard (context->ammoos));
                                  ...
                                  GimpPaintInfo *paint_info)
{
  ...
  if (context->paint_name &&
      paint_info != gimp_paint_info_get_standard (context->ammoos))
    {
      g_clear_pointer (&context->paint_name, g_free);
    }
  ...
  if (paint_info)
    {
      ...
      if (paint_info != gimp_paint_info_get_standard (context->ammoos))
                              ...
                              GimpContext   *context)
{
  ...
  if (! context->brush_name)
    context->brush_name = g_strdup (context->ammoos->config->default_brush);
                                 ...
                                 GimpContext   *context)
{
  ...
  if (! context->dynamics_name)
    context->dynamics_name = g_strdup (context->ammoos->config->default_dynamics);
                                ...
                                GimpContext   *context)
{
  ...
  if (! context->mybrush_name)
    context->mybrush_name = g_strdup (context->ammoos->config->default_mypaint_brush);
                                ...
                                GimpContext   *context)
{
  ...
  if (! context->pattern_name)
    context->pattern_name = g_strdup (context->ammoos->config->default_pattern);
                                 ...
                                 GimpContext   *context)
{
  ...
  if (! context->gradient_name)
    context->gradient_name = g_strdup (context->ammoos->config->default_gradient);
                                ...
                                GimpContext   *context)
{
  ...
  if (! context->palette_name)
    context->palette_name = g_strdup (context->ammoos->config->default_palette);
                             ...
                             GimpContext   *context)
{
  ...
  if (! context->font_name)
    context->font_name = g_strdup (context->ammoos->config->default_font);
