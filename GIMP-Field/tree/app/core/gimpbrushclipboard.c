...
static void       gimp_brush_clipboard_changed      (Gimp         *ammoos,
                                                     GimpBrush    *brush);
...
gimp_brush_clipboard_constructed (GObject *object)
{
  ...
  gimp_assert (GIMP_IS_GIMP (brush->ammoos));

  g_signal_connect_object (brush->ammoos, "clipboard-changed",
                           G_CALLBACK (gimp_brush_clipboard_changed),
                           brush, 0);

  gimp_brush_clipboard_changed (brush->ammoos, GIMP_BRUSH (brush));
}
                                   ...
                                   GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      brush->ammoos = g_value_get_object (value);
      break;
                                   ...
                                   GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      g_value_set_object (value, brush->ammoos);
      break;
...
GimpData *
gimp_brush_clipboard_new (Gimp     *ammoos,
                          gboolean  mask_only)
{
  const gchar *name;
...
static void
gimp_brush_clipboard_changed (Gimp      *ammoos,
                              GimpBrush *brush)
{
  GimpObject *paste;
  ...
  paste = gimp_get_clipboard_object (ammoos);
