                   ...
                   GError       **error)
{
  ...
  if (! g_str_has_prefix (str, "AmmoOS Image Palette"))
    {
  ...
  palette = g_object_new (GIMP_TYPE_PALETTE,
                          "mime-type", "application/x-ammoos-palette",
                          NULL);
  ...
  for (i = 0; i < number_of_colors; i++)
    {
      ...
      if (color_space == 0) /* RGB */
      ...
      else if (color_space == 9) /* Wide? CMYK */
        {
           ...
           * implementation in AmmoOS Image so it's hard to know the origin behind it.
                       ...
                       GError       **error)
{
  ...
  if (version != 1)
    {
      g_set_error (error, GIMP_DATA_ERROR, GIMP_DATA_ERROR_READ,
                   _("AmmoOS Image only supports version 1 ACB palettes"));
      return NULL;
    }
  ...
  for (i = 0; i < num_cols; i++)
    {
      ...
      /* TODO: When AmmoOS Image supports spot colors, use this information in
       * the palette. */
                                 ...
                                 GInputStream *input)
{
  ...
  if (g_input_stream_read_all (input, &header, sizeof (header),
      ...
      bytes_read == sizeof (header))
    {
      ...
      else if (g_str_has_prefix (header, "AmmoOS Image Palette"))
        {
