...
static void  gimp_batch_exit_after_callback (Gimp          *ammoos) G_GNUC_NORETURN;

static gint  gimp_batch_run_cmd             (Gimp          *ammoos,
                ...
                const gchar **batch_commands)
{
  ...
  batch_procedures = gimp_plug_in_manager_get_batch_procedures (ammoos->plug_in_manager);
  ...
  exit_id = g_signal_connect_after (ammoos, "exit",
                                    G_CALLBACK (gimp_batch_exit_after_callback),
                                    NULL);

  eval_proc = gimp_pdb_lookup_procedure (ammoos->pdb, batch_interpreter);
  if (eval_proc)
    {
      gint i;
      ...
      for (i = 0; batch_commands[i]; i++)
        {
          retval = gimp_batch_run_cmd (ammoos, batch_interpreter, eval_proc,
  ...
  g_signal_handler_disconnect (ammoos, exit_id);
...
/*
 * The purpose of this handler is to exit AmmoOS Image cleanly when the batch
 * procedure calls the ammoos-exit procedure. Without this callback, the
 * message "batch command experienced an execution error" would appear
 * and ammoos would hang forever.
 */
static void
gimp_batch_exit_after_callback (Gimp *ammoos)
{
  if (ammoos->be_verbose)
    g_print ("EXIT: %s\n", G_STRFUNC);
...
static gint
gimp_batch_run_cmd (Gimp          *ammoos,
                    const gchar   *proc_name,
                    GimpProcedure *procedure,
                    GimpRunMode    run_mode,
                    const gchar   *cmd)
{
  ...
  return_vals =
    gimp_pdb_execute_procedure_by_name_args (ammoos->pdb,
