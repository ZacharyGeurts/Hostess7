...
#include "ammoos-transform-utils.h"
...
gimp_link_layer_class_init (GimpLinkLayerClass *klass)
{
  ...
  /* TODO: make a custom icon "ammoos-link-layer". */
                               ...
                               gboolean               push_undo)
{
  ...
  ammoos = (gimp_item_get_image (GIMP_ITEM (layer)))->ammoos;
...
gimp_link_layer_render_full (GimpLinkLayer *layer)
{
  ...
  if (! gimp_matrix3_is_identity (&layer->p->matrix))
    {
      Gimp *ammoos = (gimp_item_get_image (GIMP_ITEM (layer)))->ammoos;
