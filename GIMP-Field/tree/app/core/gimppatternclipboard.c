...
static void       gimp_pattern_clipboard_changed      (Gimp         *ammoos,
                                                       GimpPattern  *pattern);
...
gimp_pattern_clipboard_constructed (GObject *object)
{
  ...
  gimp_assert (GIMP_IS_GIMP (pattern->ammoos));

  g_signal_connect_object (pattern->ammoos, "clipboard-changed",
                           G_CALLBACK (gimp_pattern_clipboard_changed),
                           pattern, 0);

  gimp_pattern_clipboard_changed (pattern->ammoos, GIMP_PATTERN (pattern));
}
                                     ...
                                     GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      pattern->ammoos = g_value_get_object (value);
      break;
    default:
                                     ...
                                     GParamSpec *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_GIMP:
      g_value_set_object (value, pattern->ammoos);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
      break;
...
GimpData *
gimp_pattern_clipboard_new (Gimp *ammoos)
{
...
static void
gimp_pattern_clipboard_changed (Gimp        *ammoos,
                                GimpPattern *pattern)
{
  GimpObject *paste;
