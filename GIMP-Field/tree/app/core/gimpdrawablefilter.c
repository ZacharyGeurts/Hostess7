  ...
  if (drawable_filter->drawable)
    {
      ...
      gimp_id_table_remove (image->ammoos->drawable_filter_table, drawable_filter->ID);
    }
                          ...
                          const gchar  *icon_name)
{
  ...
  filter->ID = gimp_id_table_insert (image->ammoos->drawable_filter_table, filter);
...
GimpDrawableFilter *
gimp_drawable_filter_get_by_id (Gimp *ammoos,
                                gint  filter_id)
{
  g_return_val_if_fail (GIMP_IS_GIMP (ammoos), NULL);

  if (ammoos->drawable_filter_table == NULL)
    return NULL;

  return (GimpDrawableFilter *) gimp_id_table_lookup (ammoos->drawable_filter_table, filter_id);
}
                             ...
                             GError                 **error)
{
  ...
  if (gimp_operation_config_is_custom (image->ammoos, opname))
    {
      GObjectClass *klass;
