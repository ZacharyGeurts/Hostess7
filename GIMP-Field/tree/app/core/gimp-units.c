...
gimp_units_init (Gimp *ammoos)
{
  ...
  the_unit_gimp = ammoos;

  vtable.get_user_unit = gimp_units_get_user_unit;

  gimp_base_init (&vtable);

  ammoos->user_units = NULL;
}

void
gimp_units_exit (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));

  gimp_base_exit ();

  g_list_free_full (ammoos->user_units, g_object_unref);
  ammoos->user_units = NULL;
}
...
enum
{
  ...
  UNIT_SYMBOL,
  UNIT_ABBREV,
  UNIT_SINGULAR, /* Obsoleted in AmmoOS Image 3.0. */
  UNIT_PLURAL    /* Obsoleted in AmmoOS Image 3.0. */
};

void
gimp_unitrc_load (Gimp *ammoos)
{
  GFile      *file;
  GScanner   *scanner;
  GTokenType  token;
          ...
          if (scanner->value.v_symbol == GINT_TO_POINTER (UNIT_INFO))
            {
              ...
              token = gimp_unitrc_unit_info_deserialize (scanner, ammoos);
...
gimp_unitrc_save (Gimp *ammoos)
{
  ...
  for (iter = ammoos->user_units; iter; iter = iter->next)
    {
  ...
  if (token == G_TOKEN_LEFT_PAREN)
    {
      ...
      if (g_scanner_peek_next_token (scanner) == token)
        {
          GimpUnit *unit = _gimp_unit_new (ammoos,
