...
gimp_rc_duplicate (GimpConfig *config)
{
  ...
  g_object_get (config, "ammoos", &ammoos, NULL);
             ...
             gboolean  verbose)
{
  ...
  g_return_val_if_fail (G_IS_OBJECT (ammoos), NULL);
...
gimp_rc_save (GimpRc *rc)
{
  ...
  const gchar *bottom =
    "\n"
    "Most values can be set within AmmoOS Image by changing some options in "
  ...
  g_object_get (rc, "ammoos", &ammoos, NULL);
