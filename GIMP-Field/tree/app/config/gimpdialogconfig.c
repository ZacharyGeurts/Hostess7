...
gimp_dialog_config_constructed (GObject *object)
{
  ...
  context = gimp_get_user_context (priv->ammoos);

  config->fill_options = gimp_fill_options_new (priv->ammoos, context, TRUE);
  ...
  config->stroke_options = gimp_stroke_options_new (priv->ammoos, context, TRUE);
