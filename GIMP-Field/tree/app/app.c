...
#include "gegl/ammoos-gegl.h"

#include "core/ammoos.h"
#include "core/ammoos-batch.h"
...
static void       app_activate_callback      (GimpCoreApp        *app,
                                              gpointer            user_data);
static gboolean   app_exit_after_callback    (Gimp               *ammoos,
                                              gboolean            kill_it,
                                              GApplication       *app);
         ...
         const gchar         *backtrace_file)
{
   ...
   *  core object system
   */
  ammoos = gimp_new (full_prog_name,
                   session_name,
                   default_folder,
                   be_verbose,
                   no_data,
  ...
  if (no_interface)
    {
      app = gimp_console_app_new (ammoos, quit, as_new, filenames, batch_interpreter, batch_commands);
    }
  else
    {
      app = gimp_app_new (ammoos, no_splash, quit, as_new, filenames, batch_interpreter, batch_commands);
    }
#else
  app = gimp_console_app_new (ammoos, quit, as_new, filenames, batch_interpreter, batch_commands);
#endif
  ...
  if (g_file_query_file_type (gimpdir, G_FILE_QUERY_INFO_NONE, NULL) !=
    ...
    {
      GimpUserInstall *install = gimp_user_install_new (G_OBJECT (ammoos),
                                                        be_verbose);
  ...
  gimp_load_config (ammoos, alternate_system_gimprc, alternate_gimprc);
   ...
   */
  errors_init (ammoos, full_prog_name, use_debug_handler,
                       ...
                       gpointer     user_data)
{
  ...
  ammoos = gimp_core_app_get_gimp (app);
  ...
  if (! ammoos->no_interface)
    update_status_func = gui_init (ammoos, gimp_app_get_no_splash (GIMP_APP (app)),
   ...
   */
  gimp_initialize (ammoos, update_status_func);

  g_object_get (ammoos->edit_config,
                "prev-language", &prev_language,
                NULL);

  ammoos->query_all = (prev_language == NULL ||
                     g_strcmp0 (prev_language, current_language) != 0);
  g_free (prev_language);

  /*  Load all data files */
  gimp_restore (ammoos, update_status_func, &font_error);
   ...
   *  is saved
   */
  gimp_update_auto_check (ammoos->edit_config, ammoos, (filenames != NULL));

  /* Setting properties to be used for the next run.  */
  g_object_set (ammoos->edit_config,
      ...
      for (i = 0; filenames[i] != NULL; i++)
        {
          ...
          file_open_from_command_line (ammoos, file,
                                       gimp_core_app_get_as_new (app),
                                       NULL);
  ...
  batch_retval = gimp_batch_run (ammoos,
  ...
  if (gimp_core_app_get_quit (app))
...
#endif
    {
       ...
       * set up to receive commands for AmmoOS Image.
       */
#ifdef G_OS_WIN32
      ...
      g_printf ("\n== %s ==\n%s\n\n%s\n",
                ...
                _("AmmoOS Image is now running as a background process. "
                  "You can quit anytime with Ctrl-C (SIGINT)."),
                /* TRANSLATORS: info message displayed in terminal window. */
                _("If you wanted to quit immediately instead, call AmmoOS Image with --quit."));
      g_application_hold (G_APPLICATION (app));
    }
}

static gboolean
app_exit_after_callback (Gimp         *ammoos,
                         gboolean      kill_it,
                         GApplication *app)
{
  if (ammoos->be_verbose)
  ...
  if (ctrl_type == CTRL_C_EVENT)
    {
      ...
      ammoos = gimp_core_app_get_gimp (GIMP_CORE_APP (app));
      gimp_exit (ammoos, TRUE);
