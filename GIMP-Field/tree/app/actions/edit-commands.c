                        ...
                        gpointer    data)
{
  ...
  if (tool_manager_undo_active (image->ammoos, display) ||
                        ...
                        gpointer    data)
{
  ...
  if (tool_manager_redo_active (image->ammoos, display) ||
                                      ...
                                      gpointer    data)
{
  ...
  paste = gimp_clipboard_get_object (ammoos);

  if (paste)
    {
      image = gimp_edit_paste_as_new_image (ammoos, paste,
  ...
  if (image)
    {
      gimp_create_display (ammoos, image, gimp_unit_pixel (), 1.0,
                        ...
                        gpointer    data)
{
  ...
  if (gimp_fill_options_set_by_fill_type (options,
    ...
    {
      gimp_message_literal (image->ammoos, NULL, GIMP_MESSAGE_WARNING,
                            error->message);
      g_clear_error (&error);
    }
      ...
      if (ammoos && display)
        {
          gimp_message_literal (
            ammoos, G_OBJECT (display), GIMP_MESSAGE_WARNING,
            _("A selected layer's alpha channel is locked."));

          gimp_tools_blink_lock_box (ammoos, GIMP_ITEM (locked_layer));
        }
            ...
            gboolean       try_svg)
{
  ...
  if (try_svg)
    {
      ...
      svg = gimp_clipboard_get_svg (display->ammoos, &svg_size);
  ...
  paste = gimp_clipboard_get_object (display->ammoos);
  ...
  if (paste)
    {
      ...
      if ((pasted_layers = gimp_edit_paste (image, drawables, paste, paste_type,
                                            gimp_get_user_context (display->ammoos),
                           ...
                           gpointer     data)
{
  ...
  if (gimp_edit_named_cut (image, name, drawables,
                           gimp_get_user_context (image->ammoos), &error))
    {
      gimp_image_flush (image);
    }
  else
