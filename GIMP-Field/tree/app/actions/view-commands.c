...
#define IS_ACTIVE_DISPLAY(display) \
  ((display) == \
   gimp_context_get_display (gimp_get_user_context ((display)->ammoos)))
                       ...
                       gpointer    data)
{
  ...
  gimp_create_display (display->ammoos,
  ...
  if (active != shell->show_all)
    {
      ...
      if (window)
        SET_ACTIVE (menus_get_image_manager_singleton (display->ammoos),
...
#define FILTERS_DIALOG_KEY "ammoos-display-filters-dialog"
...
#define PADDING_COLOR_DIALOG_KEY "ammoos-padding-color-dialog"
