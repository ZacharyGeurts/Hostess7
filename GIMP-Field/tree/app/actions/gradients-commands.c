...
#define SAVE_AS_POV_DIALOG_KEY "ammoos-save-as-pov-ray-dialog"
  ...
  if (! dialog)
    {
      ...
      g_object_set_data (G_OBJECT (dialog), "ammoos", context->ammoos);
  ...
  if (response_id == GTK_RESPONSE_ACCEPT)
    {
      ...
      if (! gimp_gradient_save_pov (gradient, file, &error))
        {
          gimp_message_literal (g_object_get_data (G_OBJECT (dialog), "ammoos"),
