  ...
  if (data && gimp_data_get_file (data))
    {
      ...
      image = file_open_with_display (context->ammoos, context, NULL,
                                      file, FALSE,
      ...
      if (! image && status != GIMP_PDB_CANCEL)
        {
          gimp_message (context->ammoos, G_OBJECT (view),
                        GIMP_MESSAGE_ERROR,
                        _("Opening '%s' failed:\n\n%s"),
      ...
      if (file)
        {
          ...
          gimp_clipboard_set_text (context->ammoos, uri);
          g_free (uri);
        }
    }
}
                           ...
                           gpointer    user_data)
{
  GimpDataFactoryView *view = GIMP_DATA_FACTORY_VIEW (user_data);
  Gimp                *ammoos;
  return_if_no_gimp (ammoos, user_data);
  ...
  if (data && gimp_data_factory_view_have (view, GIMP_OBJECT (data)))
    {
      ...
      dockable =
        gimp_window_strategy_show_dockable_dialog (GIMP_WINDOW_STRATEGY (gimp_get_window_strategy (context->ammoos)),
                                                   context->ammoos,
