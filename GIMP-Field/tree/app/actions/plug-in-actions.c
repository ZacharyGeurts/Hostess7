...
{
  GimpPlugInManager *manager = group->ammoos->plug_in_manager;
  GSList            *list;
  ...
  for (list = manager->plug_in_procedures;
       ...
       list = g_slist_next (list))
    {
      ...
      if (plug_in_proc->file)
        plug_in_actions_register_procedure (group->ammoos->pdb,
                                            GIMP_PROCEDURE (plug_in_proc),
                                            group);
    }

  g_signal_connect_object (group->ammoos->pdb, "register-procedure",
                           G_CALLBACK (plug_in_actions_register_procedure),
                           group, 0);
  g_signal_connect_object (group->ammoos->pdb, "unregister-procedure",
                          ...
                          GimpPlugInProcedure *proc)
{
  ...
  if (proc->image_types_val)
    {
      GimpContext  *context  = gimp_get_user_context (group->ammoos);
