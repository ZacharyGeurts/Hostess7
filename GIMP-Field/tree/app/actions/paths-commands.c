                         ...
                         gpointer    data)
{
  ...
  active_tool = tool_manager_get_active (image->ammoos);

  if (! GIMP_IS_PATH_TOOL (active_tool))
    {
      GimpToolInfo *tool_info = gimp_get_tool_info (image->ammoos,
                                                    "ammoos-path-tool");
...
#define EDIT_DIALOG_KEY "ammoos-path-edit-attributes-dialog"
...
#define NEW_DIALOG_KEY "ammoos-path-new-dialog"
  ...
  for (GList *iter = paths; iter; iter = iter->next)
    {
      if (gimp_path_attached_to_vector_layer (GIMP_PATH (iter->data), image))
        {
          gimp_message_literal (image->ammoos, G_OBJECT (widget),
                                GIMP_MESSAGE_WARNING,
                                _("Cannot delete paths attached to vector "
                                  "layers"));
          gimp_tools_blink_item (image->ammoos, GIMP_ITEM (iter->data));
          g_list_free (paths);
          return;
        }
    }
          ...
          gimp_path_attached_to_vector_layer (path, image))
        {
          gimp_message_literal (image->ammoos, G_OBJECT (widget),
                                ...
                                _("Cannot merge paths attached to vector "
                                  "layers"));
          gimp_tools_blink_item (image->ammoos, GIMP_ITEM (path));
          return;
        }
    }
                                       ...
                                       gpointer    data)
{
  ...
  procedure = gimp_pdb_lookup_procedure (image->ammoos->pdb,
                                         "plug-in-sel2path");
  ...
  gimp_procedure_execute_async (procedure, image->ammoos,
                                action_data_get_context (data),
                         ...
                         gpointer    data)
{
  ...
  if (svg)
    {
      gimp_clipboard_set_svg (image->ammoos, svg);
      g_free (svg);
    }
}
                          ...
                          gpointer    data)
{
  ...
  svg = gimp_clipboard_get_svg (image->ammoos, &svg_size);
  ...
  if (svg)
    {
      ...
      if (! gimp_path_import_buffer (image, svg, svg_size,
                                     ...
                                     NULL, &error))
        {
          gimp_message (image->ammoos, G_OBJECT (widget), GIMP_MESSAGE_ERROR,
                        "%s", error->message);
          g_clear_error (&error);
        }
      else
...
#define EXPORT_DIALOG_KEY "ammoos-paths-export-dialog"
...
#define IMPORT_DIALOG_KEY "ammoos-paths-import-dialog"
