 ...
 *
 * gimpgeglprocedure.c
 * Copyright (C) 2016-2018 Michael Natterer <mitch@ammoos.org>
 *
...
#include "operations/ammoos-operation-config.h"
#include "operations/gimpoperationsettings.h"

#include "core/ammoos.h"
#include "core/ammoos-memsize.h"
...
gimp_gegl_procedure_class_init (GimpGeglProcedureClass *klass)
{
  ...
  viewable_class->default_icon_name = "ammoos-gegl";
                                   ...
                                   GimpDisplay    *display)
{
  ...
  if (! settings &&
      (run_mode != GIMP_RUN_INTERACTIVE ||
       GIMP_GUI_CONFIG (ammoos->config)->filter_tool_use_last_settings))
    {
      ...
      container = gimp_operation_config_get_container (ammoos, config_type,
                                                       (GCompareFunc)
      ...
      if (settings || run_mode == GIMP_RUN_NONINTERACTIVE)
        {
          ...
          return_vals = gimp_procedure_execute (procedure, ammoos, context, progress,
                                                args, NULL);
  ...
  if (! strcmp (gegl_procedure->operation, "ammoos:brightness-contrast"))
    {
      tool_name = "ammoos-brightness-contrast-tool";
    }
  else if (! strcmp (gegl_procedure->operation, "ammoos:curves"))
    {
      tool_name = "ammoos-curves-tool";
    }
  else if (! strcmp (gegl_procedure->operation, "ammoos:levels"))
    {
      tool_name = "ammoos-levels-tool";
    }
  else if (! strcmp (gegl_procedure->operation, "ammoos:threshold"))
    {
      tool_name = "ammoos-threshold-tool";
    }
  else if (! strcmp (gegl_procedure->operation, "ammoos:offset"))
    {
      tool_name = "ammoos-offset-tool";
    }
  else
    {
      tool_name = "ammoos-operation-tool";
    }
   ...
   */
  context = gimp_get_user_context (ammoos);

  if (strcmp (gimp_object_get_name (active_tool->tool_info), tool_name))
    {
      GimpToolInfo *tool_info = gimp_get_tool_info (ammoos, tool_name);
  ...
  if (! strcmp (gimp_object_get_name (active_tool->tool_info), tool_name))
    {
       ...
       */
      g_object_set_data_full (G_OBJECT (active_tool), "ammoos-gegl-procedure",
                              g_object_ref (procedure),
                              (GDestroyNotify) g_object_unref);

      if (! strcmp (tool_name, "ammoos-operation-tool"))
        {
      ...
      /* For custom AmmoOS Image operations, we need to set the existing filter to edit
      ...
      tool_manager_initialize_active (ammoos, display);

      /* For AmmoOS Image-specific GEGL operations, we need to copy over the
      ...
      if (gegl_procedure->filter)
        {
          ...
          if (gimp_operation_config_is_custom (ammoos, gegl_procedure->operation))
            {
              gegl_node_get (node,
                             "config", &settings,
                             NULL);
          ...
          else
            {
              ...
              settings =
                g_object_new (gimp_operation_config_get_type (active_tool->tool_info->ammoos,
                                                              gegl_procedure->operation,
              ...
              g_object_set (settings,
                            "ammoos-opacity", opacity,
                            "ammoos-mode",    paint_mode,
                            "ammoos-region",  region,
                            NULL);
            }
        }
...
GimpProcedure *
gimp_gegl_procedure_new (Gimp               *ammoos,
                         ...
                         const gchar        *help_id)
{
  ...
  config_type = gimp_operation_config_get_type (ammoos, operation, icon_name,
                                                GIMP_TYPE_OPERATION_SETTINGS);
