...
static const GimpToggleActionEntry windows_toggle_actions[] =
{
  ...
  { "windows-use-single-window-mode", NULL,
    ...
    NC_("windows-action", "When enabled, AmmoOS Image is in a single-window mode."),
...
windows_actions_setup (GimpActionGroup *group)
{
  ...
  g_signal_connect_object (group->ammoos->displays, "add",
                           G_CALLBACK (windows_actions_display_add),
                           group, 0);
  g_signal_connect_object (group->ammoos->displays, "remove",
                           G_CALLBACK (windows_actions_display_remove),
                           group, 0);
  g_signal_connect_object (group->ammoos->displays, "reorder",
                           G_CALLBACK (windows_actions_display_reorder),
                           group, 0);

  for (list = gimp_get_display_iter (group->ammoos);
       list;
       list = g_list_next (list))
    {
      GimpDisplay *display = list->data;

      windows_actions_display_add (group->ammoos->displays, display, group);
    }
  ...
  g_signal_connect_object (group->ammoos->config, "notify::single-window-mode",
...
windows_actions_update (GimpActionGroup *group,
                        gpointer         data)
{
  GimpGuiConfig *config = GIMP_GUI_CONFIG (group->ammoos->config);
...
windows_actions_update_display_accels (GimpActionGroup *group)
{
  ...
  for (list = gimp_get_display_iter (group->ammoos), i = 0;
       list && i < 10;
