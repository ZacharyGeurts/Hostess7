...
actions_init (Gimp *ammoos)
{
  ...
  global_action_factory = gimp_action_factory_new (ammoos);
...
void
actions_exit (Gimp *ammoos)
{
  g_return_if_fail (GIMP_IS_GIMP (ammoos));
  g_return_if_fail (global_action_factory != NULL);
  g_return_if_fail (global_action_factory->ammoos == ammoos);
  ...
  if (! result)
    {
      ...
      if (ammoos)
        result = gimp_get_user_context (ammoos);
    }
