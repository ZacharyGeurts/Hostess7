  ...
  if (! GIMP_IS_PATH_TOOL (active_tool))
    {
      ...
      tool_info = (GimpToolInfo *)
        gimp_container_get_child_by_name (image->ammoos->tool_info_list,
                                          "ammoos-path-tool");
...
#define EDIT_DIALOG_KEY "ammoos-layer-edit-attributes-dialog"
...
#define NEW_DIALOG_KEY "ammoos-layer-new-dialog"
  ...
  if (error)
    {
      gimp_message_literal (image->ammoos,
                            ...
                            error->message);
      if (item)
        gimp_tools_blink_item (image->ammoos, item);
      g_clear_error (&error);
      return;
    }
  ...
  for (iter = merge_layers; iter; iter = iter->next)
    {
      if (gimp_layer_get_mode (iter->data) == GIMP_LAYER_MODE_PASS_THROUGH)
        {
          gimp_message_literal (image->ammoos, G_OBJECT (widget), GIMP_MESSAGE_WARNING,
  ...
  if (! dialog)
    {
      ...
      if (layer_scale_interp == -1)
        layer_scale_interp = image->ammoos->config->interpolation_type;

      dialog = scale_dialog_new (GIMP_VIEWABLE (layer),
                                 action_data_get_context (data),
                                 _("Scale Layer"), "ammoos-layer-scale",
                                 widget,
...
#define ADD_MASK_DIALOG_KEY "ammoos-add-mask-dialog"
