  ...
  if (template && gimp_container_have (container, GIMP_OBJECT (template)))
    {
      ...
      image = gimp_image_new_from_template (ammoos, template, context);
...
#define NEW_DIALOG_KEY "ammoos-template-new-dialog"

  dialog = dialogs_get_dialog (G_OBJECT (context->ammoos), NEW_DIALOG_KEY);
  ...
  if (! dialog)
    {
      ...
      dialogs_attach_dialog (G_OBJECT (context->ammoos), NEW_DIALOG_KEY, dialog);
    }
...
#define EDIT_DIALOG_KEY "ammoos-template-edit-dialog"
...
templates_new_callback (GtkWidget    *dialog,
                        ...
                        gpointer      user_data)
{
  gimp_container_add (context->ammoos->templates, GIMP_OBJECT (edit_template));
  gimp_context_set_template (gimp_get_user_context (context->ammoos),
                             edit_template);
