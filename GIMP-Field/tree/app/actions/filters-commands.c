...
static gchar * filters_parse_operation (Gimp          *ammoos,
                            ...
                            gpointer    data)
{
  ...
  operation = filters_parse_operation (image->ammoos,
                                       g_variant_get_string (value, NULL),
                                       gimp_action_get_icon_name (action),
                                       &settings);

  procedure = gimp_gegl_procedure_new (image->ammoos,
                                       NULL,
  ...
  gimp_filter_history_add (image->ammoos, procedure);
                             ...
                             gpointer    data)
{
  ...
  procedure = gimp_filter_history_nth (ammoos, 0);

  if (procedure)
    filters_run_procedure (ammoos, display, procedure, run_mode);
}
                              ...
                              gpointer    data)
{
  ...
  filters_run_procedure (ammoos, display, procedure, GIMP_RUN_INTERACTIVE);
}
...
static gchar *
filters_parse_operation (Gimp         *ammoos,
                         const gchar  *operation_str,
                         const gchar  *icon_name,
                         GimpObject  **settings)
{
      ...
      if (*serialized)
        {
          ...
          *settings =
            g_object_new (gimp_operation_config_get_type (ammoos, operation,
                                                          icon_name,
...
void
filters_run_procedure (Gimp          *ammoos,
                       GimpDisplay   *display,
                       GimpProcedure *procedure,
                       GimpRunMode    run_mode)
{
      ...
      for (iter = drawables; iter; iter = iter->next)
        {
          if (! gimp_item_is_visible (GIMP_ITEM (iter->data)) &&
            ...
            {
              gimp_message_literal (display->ammoos, G_OBJECT (display),
                                    GIMP_MESSAGE_ERROR,
  ...
  if (args)
    {
      ...
      if (run_mode == GIMP_RUN_NONINTERACTIVE)
        {
          success =
            procedure_commands_run_procedure (procedure, ammoos,
        ...
        {
          success =
            procedure_commands_run_procedure_async (procedure, ammoos,
                                                    GIMP_PROGRESS (display),
                                                    run_mode, args,
                                                    display);
        }
