  ...
  if (gimp_container_view_get_selected (editor->view, &images) > 1)
  ...
  else
    {
      if (imagefile && gimp_container_have (container, GIMP_OBJECT (imagefile)))
        ...
        {
          file_file_open_dialog (context->ammoos, NULL, GTK_WIDGET (editor));
        }
    }
  if (images)
    g_list_free (images);
  ...
  if (imagefile && gimp_container_have (container, GIMP_OBJECT (imagefile)))
    {
      ...
      gimp_container_foreach (GIMP_CONTAINER (context->ammoos->displays),
                                         ...
                                         gpointer    data)
{
  ...
  if (imagefile && gimp_container_have (container, GIMP_OBJECT (imagefile)))
    {
      file_file_open_dialog (context->ammoos,
                                      ...
                                      gpointer    data)
{
  ...
  if (imagefile)
    gimp_clipboard_set_text (context->ammoos,
  ...
  if (imagefile)
    {
      ...
      if (! gimp_file_show_in_file_manager (file, &error))
        {
          gimp_message (context->ammoos, G_OBJECT (editor),
                        GIMP_MESSAGE_ERROR,
  ...
  if (imagefile && gimp_container_have (container, GIMP_OBJECT (imagefile)))
    {
      ...
      if (! gimp_imagefile_create_thumbnail (imagefile,
                                             context, NULL,
                                             context->ammoos->config->thumbnail_size,
                                             FALSE, &error))
        {
          gimp_message_literal (context->ammoos,
                      ...
                      GimpImagefile *imagefile)
{
  ...
  image = file_open_with_display (context->ammoos, context, NULL, file, FALSE,
                                  G_OBJECT (gimp_widget_get_monitor (editor)),
                                  &status, &error);

  if (! image && status != GIMP_PDB_CANCEL)
    {
      gimp_message (context->ammoos, G_OBJECT (editor), GIMP_MESSAGE_ERROR,
                    _("Opening '%s' failed:\n\n%s"),
