                        ...
                        gpointer    data)
{
  ...
  dialog = gimp_dialog_factory_dialog_new (gimp_dialog_factory_get_singleton (),
                                           ...
                                           NULL /*ui_manager*/,
                                           widget,
                                           "ammoos-image-new-dialog", -1, FALSE);
                              ...
                              gpointer    data)
{
  ...
  gimp_create_display (new_image->ammoos, new_image, shell->unit,
...
#define CONVERT_TYPE_DIALOG_KEY "ammoos-convert-type-dialog"
  ...
  config = GIMP_DIALOG_CONFIG (image->ammoos->config);
...
#define CONVERT_PRECISION_DIALOG_KEY "ammoos-convert-precision-dialog"
...
#define PROFILE_ASSIGN_DIALOG_KEY "ammoos-profile-assign-dialog"
...
#define PROFILE_CONVERT_DIALOG_KEY "ammoos-profile-convert-dialog"

  dialog = dialogs_get_dialog (G_OBJECT (image), PROFILE_CONVERT_DIALOG_KEY);

  if (! dialog)
    {
      GimpDialogConfig *config = GIMP_DIALOG_CONFIG (image->ammoos->config);
      GimpColorProfile *current_profile;
      GimpColorProfile *default_profile;
  ...
  if (response_id == GTK_RESPONSE_ACCEPT)
    {
      ...
      if (! gimp_color_profile_save_to_file (profile, file, &error))
        {
          gimp_message (image->ammoos, NULL,
...
#define PROFILE_SAVE_DIALOG_KEY "ammoos-profile-save-dialog"
...
#define RESIZE_DIALOG_KEY "ammoos-resize-dialog"
...
#define PRINT_SIZE_DIALOG_KEY "ammoos-print-size-dialog"
...
#define SCALE_DIALOG_KEY "ammoos-scale-dialog"
  ...
  if (! dialog)
    {
      ...
      if (image_scale_interp == -1)
        image_scale_interp = display->ammoos->config->interpolation_type;
...
#define MERGE_LAYERS_DIALOG_KEY "ammoos-merge-layers-dialog"
...
#define GRID_DIALOG_KEY "ammoos-grid-dialog"
...
#define PROPERTIES_DIALOG_KEY "ammoos-image-properties-dialog"
                            ...
                            gpointer                  user_data)
{
  ...
  if (! gimp_image_convert_type (image, GIMP_RGB, new_profile,
                                 progress, &error))
    {
      gimp_message (image->ammoos, G_OBJECT (dialog),
                    GIMP_MESSAGE_ERROR,
                    "%s", error->message);
      g_clear_error (&error);
                                ...
                                gpointer                user_data)
{
  ...
  if (! gimp_image_convert_indexed (image,
                                    ...
                                    &error))
    {
      gimp_message_literal (image->ammoos, G_OBJECT (display),
...
#define SOFTPROOF_PROFILE_DIALOG_KEY "ammoos-softproof-profile-dialog"
