...
static void     filters_actions_set_tooltips    (GimpActionGroup             *group,
                                                 ...
                                                 gint                         n_entries);
static void     filters_actions_history_changed (Gimp                        *ammoos,
...
filters_actions_setup (GimpActionGroup *group)
{
  ...
  for (iter = op_classes; iter; iter = iter->next)
    {
      ...
      if (gegl_operation_class_get_key (op_class, "ammoos:menu-path") &&
      ...
      if (gegl_operation_class_get_key (op_class, "ammoos:menu-label"))
        {
          GimpAction *action;
  ...
  n_entries = gimp_filter_history_size (group->ammoos);
  ...
  g_signal_connect_object (group->ammoos, "filter-history-changed",
                           G_CALLBACK (filters_actions_history_changed),
                           group, 0);

  filters_actions_history_changed (group->ammoos, group);
...
#undef SET_SENSITIVE
  ...
  {
    GimpProcedure *proc   = gimp_filter_history_nth (group->ammoos, 0);
    ...
    for (i = 0; i < gimp_filter_history_length (group->ammoos); i++)
      {
        gchar    *name = g_strdup_printf ("filters-recent-%02d", i + 1);
        gboolean  sensitive;

        proc = gimp_filter_history_nth (group->ammoos, i);
...
static void
filters_actions_history_changed (Gimp            *ammoos,
                                 GimpActionGroup *group)
{
  GimpProcedure   *proc;
  ...
  proc = gimp_filter_history_nth (ammoos, 0);
  ...
  for (i = 0; i < gimp_filter_history_length (ammoos); i++)
    {
      GimpAction  *action;
      GimpAction  *actual_action = NULL;
      const gchar *label;
      ...
      proc = gimp_filter_history_nth (ammoos, i);
  ...
  for (; i < gimp_filter_history_size (ammoos); i++)
    {
      GimpAction *action;
      gchar      *name = g_strdup_printf ("filters-recent-%02d", i + 1);
