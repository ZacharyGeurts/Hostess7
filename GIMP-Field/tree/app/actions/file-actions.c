...
static const GimpActionEntry file_actions[] =
{
  ...
  { "file-quit", GIMP_ICON_APPLICATION_EXIT,
    ...
    NC_("file-action", "Quit the AmmoOS Field Image Research"),
...
file_actions_setup (GimpActionGroup *group)
{
  ...
  n_entries = GIMP_GUI_CONFIG (group->ammoos->config)->last_opened_size;
  ...
  g_signal_connect_object (group->ammoos->documents, "add",
                           G_CALLBACK (file_actions_last_opened_update),
                           group, 0);
  g_signal_connect_object (group->ammoos->documents, "remove",
                           G_CALLBACK (file_actions_last_opened_update),
                           group, 0);
  g_signal_connect_object (group->ammoos->documents, "reorder",
                           G_CALLBACK (file_actions_last_opened_reorder),
                           group, 0);

  file_actions_last_opened_update (group->ammoos->documents, NULL, group);

  /*  also listen to image adding/removal so we catch the case where
   *  the last image is closed but its display stays open.
   */
  g_signal_connect_object (group->ammoos->images, "add",
                           G_CALLBACK (file_actions_close_all_update),
                           group, 0);
  g_signal_connect_object (group->ammoos->images, "remove",
                           G_CALLBACK (file_actions_close_all_update),
                           group, 0);

  file_actions_close_all_update (group->ammoos->displays, NULL, group);
}
                                 ...
                                 GimpActionGroup *group)
{
  gint num_documents;
  gint i;
  gint n = GIMP_GUI_CONFIG (group->ammoos->config)->last_opened_size;
...
file_actions_close_all_update (GimpContainer   *images,
                               ...
                               GimpActionGroup *group)
{
  GimpContainer *container  = group->ammoos->displays;
