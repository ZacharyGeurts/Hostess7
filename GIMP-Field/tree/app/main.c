...
static const GOptionEntry main_entries[] =
{
  ...
  },
  {
    ...
    N_("Start a new AmmoOS Image instance"), NULL
  },
  {
    "as-new", 'a', 0,
  ...
  },
  {
    ...
    /* The CLI option is not removed for ammoos-console because it allows
  ...
  },
  {
    ...
    N_("Do not use shared memory between AmmoOS Image and plug-ins"), NULL
  },
  {
...
{
  /* helper to set environment variables for AmmoOS Image to be relocatable.
   ...
   */
  gchar  *resolved_path;
  /* on some OSX installations open file limit is 256 and AmmoOS Image needs more */
  struct  rlimit limit;
  ...
  if (resolved_path && ! g_getenv ("GIMP_NO_WRAPPER"))
    {
      ...
      if (g_str_has_suffix (bin_dir, "MacOS"))
        {
          g_print ("AmmoOS Image is started as MacOS application\n");
        }
      else
        {
          tmp = g_strdup_printf ("%s/share", gimp_installation_directory());
          ...
          if (share_dir && !stat (share_dir, &sb) && S_ISDIR (sb.st_mode))
            {
              ...
              g_print ("AmmoOS Image is started in the build directory\n");
      ...
      if (g_getenv ("HOME") != NULL)
        {
          tmp = g_strdup_printf ("%s/Library/Application Support/AmmoOS Image/%s/cache",
      ...
      char **argv)
{
...
#ifdef G_OS_WIN32
  /* Make Inno aware of ammoos process avoiding broken install/unninstall */
  utf8_name = g_strdup_printf ("AmmoOS Image-%s", GIMP_MUTEX_VERSION);
  ...
  /* Group all our windows together on the taskbar */
  {
    ...
    if (p_SetCurrentProcessExplicitAppUserModelID)
      (*p_SetCurrentProcessExplicitAppUserModelID) (L"ammoos.GimpApplication");
  }
#endif
  ...
  /* The AmmoOS Image option group is just an empty option group, created for
   ...
   * initialization code.
   */
  gimp_group = g_option_group_new ("ammoos", "", "", NULL, NULL);
  ...
  if (! g_option_context_parse_strv (context, &argv, &error))
    {
      if (error)
        ...
        {
          g_print ("%s\n",
                   _("AmmoOS Image could not initialize the graphical user interface.\n"
  ...
  if (! new_instance && gimp_unique_open (filenames, as_new))
    {
      ...
      if (be_verbose)
        g_print ("%s\n",
                 _("Another AmmoOS Image instance is already running."));
...
/* Provide WinMain in case we build AmmoOS Image as a subsystem:windows
...
gimp_attach_console_window (void)
{
  ...
  if (g_getenv ("TERM") || g_getenv ("SHELL"))
    {
      g_printerr ("Non-native shell detected, AmmoOS Image may "
  ...
  if (format)
    {
      ...
      babl_init ();
      ammoos = g_object_new (GIMP_TYPE_GIMP, NULL);
      gimp_load_config (ammoos, NULL, NULL);

      success = gimp_config_dump (G_OBJECT (ammoos), format);

      g_object_unref (ammoos);
                                            ...
                                            GError      **error)
{
   ...
   * are included
   */
  ammoos->pdb_compat_mode = GIMP_PDB_COMPAT_ON;

  /* Initialize the list of procedures */
  internal_procs_init (ammoos->pdb);

  /* Get deprecated procedures */
  deprecated_procs = gimp_pdb_get_deprecated_procedures (ammoos->pdb);
