 ...
 *
 * gimppropgui.c
 * Copyright (C) 2002-2017  Michael Natterer <mitch@ammoos.org>
                                 ...
                                 const gchar             **label)
{
  ...
  *label = NULL;
  ...
  if (gegl_param_spec_get_property_key (pspec, "sensitive") ||
    ...
    {
      g_object_set_data (G_OBJECT (widget), "ammoos-prop-pspec", pspec);
                          ...
                          GtkWidget *target)
{
  ...
  g_object_set_data_full (G_OBJECT (source),
                          "ammoos-prop-label-ref", label_ref,
                          (GDestroyNotify) gimp_prop_free_label_ref);

  label = g_object_get_data (G_OBJECT (source), "ammoos-prop-label");
                         ...
                         GtkWidget  *widget)
{
  ...
  widget_pspec = g_object_get_data (G_OBJECT (widget), "ammoos-prop-pspec");
  label_ref    = g_object_get_data (G_OBJECT (widget), "ammoos-prop-label-ref");
   ...
   * "ammoos-prop-label" key, so that we can use it to initialize the label
   * widget's label in gimp_prop_gui_bind_label() upon binding.
   */
  g_object_set_data_full (G_OBJECT (widget), "ammoos-prop-label", label, g_free);
