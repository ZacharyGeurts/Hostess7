...
gimp_plug_in_shm_new (void)
{
  ...
  /* Use Win32 shared memory mechanisms for transferring tile data. */
  {
    ...
    g_snprintf (fileMapName, sizeof (fileMapName), "AmmoOS Image%d.SHM", pid);
  ...
  /* Use POSIX shared memory mechanisms for transferring tile data. */
  {
    ...
    /* From the id, derive the file map name */
    g_snprintf (shm_handle, sizeof (shm_handle), "/ammoos-shm-%d", pid);
  ...
  if (shm->shm_id != -1)
    {
      ...
      g_snprintf (shm_handle, sizeof (shm_handle), "/ammoos-shm-%d",
                  shm->shm_id);
