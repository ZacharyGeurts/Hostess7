...
#define PLUG_IN_RC_FILE_VERSION 18
...
static GTokenType plug_in_def_deserialize        (Gimp                 *ammoos,
                                                  GScanner             *scanner,
                                                  GSList              **plug_in_defs);
        ...
        case G_TOKEN_SYMBOL:
          switch (GPOINTER_TO_INT (scanner->value.v_symbol))
            ...
            case PLUG_IN_DEF:
              ...
              token = plug_in_def_deserialize (ammoos, scanner, &plug_in_defs);
      ...
      if (protocol_version != GIMP_PROTOCOL_VERSION)
        {
          g_set_error (error,
                       ...
                       _("Skipping '%s': wrong AmmoOS Image protocol version."),
...
static GTokenType
plug_in_def_deserialize (Gimp      *ammoos,
                         GScanner  *scanner,
                         GSList   **plug_in_defs)
{
  GimpPlugInDef       *plug_in_def;
      ...
      switch (token)
        ...
        case G_TOKEN_SYMBOL:
          switch (GPOINTER_TO_INT (scanner->value.v_symbol))
            {
            case PROC_DEF:
              token = plug_in_procedure_deserialize (scanner, ammoos,
                                                     plug_in_def,
                                                     &proc);
                               ...
                               GimpPlugInProcedure **proc)
{
  ...
  *proc = GIMP_PLUG_IN_PROCEDURE (procedure);
  ...
  for (i = 0; i < n_args; i++)
    {
      token = plug_in_proc_arg_deserialize (scanner, ammoos, procedure, FALSE);
      if (token != G_TOKEN_LEFT_PAREN)
        return token;
    }
  ...
  for (i = 0; i < n_return_vals; i++)
    {
      token = plug_in_proc_arg_deserialize (scanner, ammoos, procedure, TRUE);
      if (token != G_TOKEN_LEFT_PAREN)
        return token;
    }
          ...
          for (int i = 0; i < n_choices; i++)
            {
              ...
              if (! gimp_scanner_parse_string (scanner, &reason))
                {
                  token = G_TOKEN_STRING;
                  goto error;
                }
          ...
          for (GList *iter = choices; iter; iter = iter->next)
            {
              ...
              gimp_config_writer_string (writer, reason);
            }
        }
      break;
