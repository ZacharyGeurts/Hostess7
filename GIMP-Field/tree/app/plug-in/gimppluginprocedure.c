                                ...
                                GError        **error)
{
  ...
  if (! gimp_plug_in_procedure_validate_args (plug_in_procedure, ammoos,
                                              args, &pdb_error))
    {
  ...
  if (procedure->proc_type == GIMP_PDB_PROC_TYPE_INTERNAL)
    return GIMP_PROCEDURE_CLASS (parent_class)->execute (procedure, ammoos,
                                                         context, progress,
                                                         args, error);

  return gimp_plug_in_manager_call_run (ammoos->plug_in_manager,
                                        context, progress,
                                      ...
                                      GimpDisplay    *display)
{
  ...
  if (gimp_plug_in_procedure_validate_args (plug_in_procedure, ammoos,
                                            args, &error))
    {
      ...
      return_vals = gimp_plug_in_manager_call_run (ammoos->plug_in_manager,
                                                   context, progress,
                                                   plug_in_procedure,
                                                   args, FALSE, display);
    ...
    {
      gimp_message_literal (ammoos, G_OBJECT (progress), GIMP_MESSAGE_ERROR,
                            error->message);
      g_error_free (error);
    }
}
  ...
  if (uri_value)
    {
      ...
      file = file_utils_filename_to_file (ammoos,
...
gimp_plug_in_procedure_is_xcf_load (GimpPlugInProcedure *file_proc)
{
   ...
   */
  return (g_strcmp0 (proc_name, "ammoos-xcf-load") == 0);
}
