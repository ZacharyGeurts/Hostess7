...
GimpPlugInManager *
gimp_plug_in_manager_new (Gimp *ammoos)
{
  ...
  manager->interpreter_db = gimp_interpreter_db_new (ammoos->be_verbose);
  manager->environ_table  = gimp_environ_table_new (ammoos->be_verbose);
  ...
  for (list = manager->plug_in_procedures; list; list = list->next)
    {
      ...
      if (strcmp (gimp_object_get_name (procedure),
                  gimp_object_get_name (tmp_proc)) == 0)
        {
          ...
          gimp_filter_history_remove (manager->ammoos, GIMP_PROCEDURE (tmp_proc));
                                    ...
                                    GimpTemporaryProcedure *procedure)
{
  ...
  gimp_pdb_register_procedure (manager->ammoos->pdb, GIMP_PROCEDURE (procedure));
                                       ...
                                       GimpTemporaryProcedure *procedure)
{
  ...
  gimp_filter_history_remove (manager->ammoos,
                              GIMP_PROCEDURE (procedure));

  gimp_pdb_unregister_procedure (manager->ammoos->pdb,
                                 GIMP_PROCEDURE (procedure));
                                    ...
                                    GimpPlugIn *plug_in)
{
  ...
  if (plug_in->manager->ammoos->be_verbose &&
