...
gimp_plug_in_class_init (GimpPlugInClass *klass)
{
  ...
  /* initialize the ammoos protocol library and set the read and
   *  write handlers.
   */
  gp_init ();
  ...
  if (! got_message)
    {
      ...
      gimp_message (plug_in->manager->ammoos, G_OBJECT (progress),
                    GIMP_MESSAGE_ERROR,
                    _("Plug-in crashed: \"%s\"\n(%s)\n\n"
                      "The dying plug-in may have messed up AmmoOS Image's internal "
                      "state. You may want to save your images and restart "
                      "AmmoOS Image to be on the safe side."),
                   ...
                   gboolean            synchronous)
{
  ...
  stm = g_strdup_printf ("%d", plug_in->manager->ammoos->stack_trace_mode);
