                              ...
                              GimpInitStatusFunc  status_callback)
{
  ...
  ammoos = manager->ammoos;

  /* need a GimpPDBContext for calling gimp_plug_in_manager_run_foo() */
  context = gimp_pdb_context_new (ammoos, context, TRUE);
                             ...
                             GimpInitStatusFunc  status_callback)
{
  ...
  g_object_get (manager->ammoos->extension_manager,
  ...
  if (! path_str)
    path_str = manager->ammoos->config->plug_in_path;
          ...
          if (g_file_query_file_type (child,
                  ...
                  while ((info2 = g_file_enumerator_next_file (enumerator2, NULL, NULL)))
                    {
                      ...
                      if (g_strcmp0 (file_name, g_file_info_get_name (info)) == 0 &&
                        ...
                        {
                          gimp_plug_in_manager_add_from_file (manager, child2, child,
          ...
          else if (gimp_file_is_executable (child))
            {
              if (g_getenv ("GIMP_TESTING_PLUGINDIRS"))
                {
                  gimp_plug_in_manager_add_from_file (manager, child, directory,
                                                      gimp_plug_in_manager_get_file_time (info));
                }
              else
                {
...
{
  Gimp  *ammoos = manager->ammoos;
  GFile *pluginrc;

  if (ammoos->config->plug_in_rc_path)
    {
      gchar *path = gimp_config_path_expand (ammoos->config->plug_in_rc_path,
                                             TRUE, NULL);
                                    ...
                                    GimpInitStatusFunc  status_callback)
{
  ...
  if (manager->ammoos->be_verbose)
    g_print ("Parsing '%s'\n", gimp_file_get_utf8_name (pluginrc));

  rc_defs = plug_in_rc_parse (manager->ammoos, pluginrc, &error);
  ...
  if (rc_defs)
  ...
  else if (error)
    {
      if (error->code != GIMP_CONFIG_ERROR_OPEN_ENOENT)
        gimp_message_literal (manager->ammoos, NULL, GIMP_MESSAGE_ERROR,
                              error->message);
...
gimp_plug_in_manager_add_to_db (GimpPlugInManager   *manager,
                                ...
                                GimpPlugInProcedure *proc)
{
  gimp_pdb_register_procedure (manager->ammoos->pdb, GIMP_PROCEDURE (proc));
...
{
  GimpCoreConfig *config         = manager->ammoos->config;
