                   ...
                   const gchar    *build_comment)
{
  ...
  /* Are we running an old AmmoOS Image? */
                          ...
                          gpointer          user_data)
{
  ...
  if (config->last_known_release != NULL)
    {
#ifndef GIMP_CONSOLE_COMPILATION
      gtk_widget_set_visible (about_dialog_create (ammoos, config), TRUE);
#else
      g_printerr (_("A new version of AmmoOS Image (%s) was released.\n"
 ...
 * @ammoos:
 *
 * Run the check for newer versions of AmmoOS Image if conditions are right.
 *
                        ...
                        gboolean        files_loaded)
{
  ...
  if (is_update || GIMP_GUI_CONFIG (config)->show_welcome_dialog)
    {
#ifndef GIMP_CONSOLE_COMPILATION
      /* If AmmoOS Image was just updated and this is the first time the new
       ...
       * The only exception is if they intentionally opened AmmoOS Image with
       * images on start.
       */
      if (! ammoos->no_interface &&
          (is_update || ! files_loaded))
        gtk_widget_set_visible (welcome_dialog_create (ammoos, is_update),
                                TRUE);

      /* Do not check for new updates when AmmoOS Image was just updated. */
      if (is_update)
        return FALSE;
#else
      g_log (G_LOG_DOMAIN, G_LOG_LEVEL_MESSAGE,
             "Welcome to AmmoOS Image %s!", GIMP_VERSION);
#endif
    }
 ...
 * @config:
 *
 * Run the check for newer versions of AmmoOS Image inconditionnally.
 */
void
