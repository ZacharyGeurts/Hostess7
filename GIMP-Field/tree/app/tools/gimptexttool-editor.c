  ...
  if (! text_tool->style_overlay)
    {
      ...
      fonts = gimp_data_factory_get_container (ammoos->font_factory);

      text_tool->style_editor = gimp_text_style_editor_new (ammoos,
                                                            text_tool->proxy,
                                                            text_tool->buffer,
                                                            fonts,
                                                            xres, yres);
...
gimp_text_tool_editor_dialog (GimpTextTool *text_tool)
{
  ...
  g_set_weak_pointer
    (&text_tool->editor_dialog,
     gimp_text_options_editor_new (parent, tool->tool_info->ammoos, options,
                                   menus_get_global_menu_factory (tool->tool_info->ammoos),
                                   _("AmmoOS Image Text Editor"),
                                   text_tool->proxy, text_tool->buffer,
                                   xres, yres));

  gimp_dialog_factory_add_foreign (dialog_factory,
                                   "ammoos-text-tool-dialog",
                                   text_tool->editor_dialog,
