                             ...
                             GError      **error)
{
  ...
  GimpGuiConfig    *config      = GIMP_GUI_CONFIG (display->ammoos->config);
  ...
  if (gimp_item_is_content_locked (GIMP_ITEM (drawable), &locked_item))
    {
      ...
      if (error)
        gimp_tools_blink_lock_box (display->ammoos, locked_item);
  ...
  if (! filter_tool->gui)
    {
      ...
      /*  disabled for at least AmmoOS Image 2.8  */
                                  ...
                                  GimpConfig     *config)
{
  ...
  /* copy the "ammoos-region" property first, to avoid incorrectly adjusting the
  ...
  if (filter_tool->filter)
    {
      ...
      if (pspec)
        {
          if (! strcmp (pspec->name, "ammoos-clip")    ||
              ! strcmp (pspec->name, "ammoos-mode")    ||
              ! strcmp (pspec->name, "ammoos-opacity"))
            {
              gimp_filter_tool_update_filter (filter_tool);
            }
          else if (! strcmp (pspec->name, "ammoos-region"))
            {
              gimp_filter_tool_update_filter (filter_tool);
...
gimp_filter_tool_halt (GimpFilterTool *filter_tool)
{
  ...
  if (tool_manager_get_active (tool->tool_info->ammoos) == tool)
    tool_manager_swap_tools (tool->tool_info->ammoos, TRUE);
}
  ...
  if (filter_tool->filter)
    {
      ...
      if (filter_tool->config && filter_tool->has_settings)
        {
          GimpGuiConfig *config = GIMP_GUI_CONFIG (tool->tool_info->ammoos->config);
      ...
      if (filter_tool->config)
        {
          ...
          mode_box = gimp_prop_layer_mode_box_new (
            filter_tool->config, "ammoos-mode",
                                ...
                                GimpDrawableFilter *existing_filter)
{
  ...
  filter_tool->config =
    g_object_new (gimp_operation_config_get_type (tool->tool_info->ammoos,
                                                  operation_name,
                                                  gimp_tool_get_icon_name (tool),
  ...
  if (filter_tool->existing_filter)
    {
      ...
      if (! strcmp (gimp_object_get_name (tool->tool_info), "ammoos-operation-tool") &&
          ! gimp_operation_config_is_custom (tool->tool_info->ammoos, operation_name))
        {
          GParamSpec **pspecs;
          guint        n_pspecs;
  ...
  filter_tool->settings =
    gimp_operation_config_get_container (tool->tool_info->ammoos,
                          ...
                          GimpConfig     *config)
{
  ...
  user_context = gimp_get_user_context (display->ammoos);

  tool_info = (GimpToolInfo *)
    gimp_container_get_child_by_name (display->ammoos->tool_info_list,
                                      new_tool_id);

  gimp_tool_control (GIMP_TOOL (filter_tool), GIMP_TOOL_ACTION_HALT, display);
  gimp_context_set_tool (user_context, tool_info);
  tool_manager_initialize_active (display->ammoos, display);

  new_tool = tool_manager_get_active (display->ammoos);
