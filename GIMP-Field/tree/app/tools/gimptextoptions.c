...
gimp_text_options_gui (GimpToolOptions *tool_options)
{
  ...
  async_set =
    gimp_data_factory_get_async_set (tool_options->tool_info->ammoos->font_factory);
  ...
  gimp_grid_attach_aligned (GTK_GRID (grid), 0, row++,
                            _("Box:"), 0.0, 0.5,
                            combo, 1);
  stroke_options = gimp_stroke_options_new (GIMP_CONTEXT (options)->ammoos,
                                            NULL, FALSE);
#define BIND(a)                                    \
                              ...
                              gdouble          yres)
{
  ...
  editor = gimp_text_editor_new (title, parent, ammoos, menu_factory,
