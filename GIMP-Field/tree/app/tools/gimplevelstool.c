...
gimp_levels_tool_dialog (GimpFilterTool *filter_tool)
{
  ...
  GimpGuiConfig    *gui_config   = GIMP_GUI_CONFIG (context->ammoos->config);
  GtkListStore     *store;
  GtkWidget        *main_vbox;
                                  ...
                                  GError         **error)
{
  ...
  if (g_str_has_prefix (header, "# AmmoOS Image Levels File\n"))
                         ...
                         GimpLevelsTool *levels_tool)
{
  ...
  gimp_wait (tool->tool_info->ammoos, waitable, _("Calculating histogram..."));
