...
gimp_filter_tool_get_settings_box (GimpFilterTool *filter_tool)
{
  ...
  box = gimp_settings_box_new (tool_info->ammoos,
                               filter_tool->config,
                                       ...
                                       GError         **error)
{
  ...
  header = g_strdup_printf ("AmmoOS Image '%s' settings",
                                  ...
                                  GimpFilterTool  *filter_tool)
{
  ...
  if (GIMP_TOOL (filter_tool)->tool_info->ammoos->be_verbose)
    g_print ("Parsing '%s'\n", gimp_file_get_utf8_name (file));

  input = G_INPUT_STREAM (g_file_read (file, NULL, &error));
  if (! input)
    {
      gimp_message (GIMP_TOOL (filter_tool)->tool_info->ammoos,
                                  ...
                                  GimpFilterTool  *filter_tool)
{
  ...
  if (! output)
    {
      gimp_message_literal (GIMP_TOOL (filter_tool)->tool_info->ammoos,
                            G_OBJECT (gimp_tool_gui_get_dialog (filter_tool->gui)),
                            GIMP_MESSAGE_ERROR,
                            error->message);
      g_clear_error (&error);
