...
#include "gegl/ammoos-gegl-nodes.h"
...
gimp_bucket_fill_tool_constructed (GObject *object)
{
  ...
  Gimp                  *ammoos        = GIMP_CONTEXT (options)->ammoos;
                                    ...
                                    GimpDisplay         *display)
{
  ...
  if (options->fill_area == GIMP_BUCKET_FILL_LINE_ART &&
      ! gimp_line_art_get_input (bucket_tool->priv->line_art))
    {
      ...
      gimp_blink_dockable (display->ammoos, "ammoos-tool-options", "line-art-source", NULL, NULL);
      return;
    }
  ...
  if (press_type == GIMP_BUTTON_PRESS_NORMAL &&
                                                       ...
                                                       display, coords))
    {
      ...
      fill_options = gimp_fill_options_new (image->ammoos, NULL, FALSE);
                                    ...
                                    GimpDisplay     *display)
{
  ...
  if (key == GDK_MOD1_MASK)
    ...
    {
      GimpToolInfo *info = gimp_get_tool_info (display->ammoos,
                                               "ammoos-color-picker-tool");
...
gimp_bucket_fill_tool_line_art_computing_end (GimpBucketFillTool *tool)
{
  ...
  gtk_widget_set_visible (options->line_art_busy_box, FALSE);
}
  ...
  if (options->fill_area == GIMP_BUCKET_FILL_LINE_ART)
    {
      ...
      context = gimp_get_user_context (GIMP_CONTEXT (options)->ammoos);
