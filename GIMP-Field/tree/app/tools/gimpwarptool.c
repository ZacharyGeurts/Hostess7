...
#include "gegl/ammoos-gegl-apply-operation.h"
...
gimp_warp_tool_constructed (GObject *object)
{
  ...
  display_config = GIMP_DISPLAY_CONFIG (tool->tool_info->ammoos->config);
      ...
      if (show_message)
        {
          ...
          gimp_tools_show_tool_options (display->ammoos);
...
gimp_warp_tool_get_node_bounds (GeglNode *node)
{
  ...
  bounds = g_object_get_data (G_OBJECT (node), "ammoos-warp-tool-bounds");
  ...
  if (! bounds)
    {
      ...
      g_object_set_data_full (G_OBJECT (node), "ammoos-warp-tool-bounds",
                              bounds, g_free);
    }
...
{
  if (node && ! strcmp (gegl_node_get_operation (node), "gegl:warp"))
    g_object_set_data (G_OBJECT (node), "ammoos-warp-tool-bounds", NULL);
}
...
gimp_warp_tool_animate (GimpWarpTool *wt)
{
  ...
  image = gimp_create_image (orig_image->ammoos,
  ...
  scale_node = gegl_node_new_child (NULL,
                                    "operation",    "ammoos:scalar-multiply",
                                    "n-components", 2,
                                    NULL);
  ...
  gimp_create_display (orig_image->ammoos, image, gimp_unit_pixel (), 1.0,
