  ...
  if (! options->sample_merged)
    {
      if (g_list_length (drawables) == 1)
        ...
        {
          select_image = gimp_image_new_from_drawables (image->ammoos, drawables, FALSE, FALSE);
          gimp_container_remove (image->ammoos->images, GIMP_OBJECT (select_image));
