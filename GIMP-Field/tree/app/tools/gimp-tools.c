...
#include "core/ammoos-contexts.h"
#include "core/ammoos-internal-data.h"
...
#include "ammoos-tool-options-manager.h"
#include "ammoos-tools.h"
...
static void   gimp_tools_copy_structure (Gimp                   *ammoos,
...
gimp_tools_init (Gimp *ammoos)
{
  ...
  gimp_container_freeze (ammoos->tool_info_list);

  for (i = 0; i < G_N_ELEMENTS (register_funcs); i++)
    {
      register_funcs[i] (gimp_tools_register, ammoos);
    }

  gimp_container_thaw (ammoos->tool_info_list);

  gimp_tool_options_manager_init (ammoos);

  tool_manager_init (ammoos);

  toolbox_groups_binding = g_object_bind_property (
    ammoos->config,            "toolbox-groups",
    ammoos->tool_item_ui_list, "flat",
    G_BINDING_INVERT_BOOLEAN |
    G_BINDING_SYNC_CREATE);
}

void
gimp_tools_exit (Gimp *ammoos)
{
  GList *list;
  ...
  tool_manager_exit (ammoos);

  gimp_tool_options_manager_exit (ammoos);

  for (list = gimp_get_tool_info_iter (ammoos);
       list;
...
void
gimp_tools_restore (Gimp *ammoos)
{
  GimpObject *object;
  GList      *list;
  GError     *error = NULL;
  ...
  /* restore tool order */
  gimp_tools_reset (ammoos, ammoos->tool_item_list, TRUE);

  /* make the generic operation tool invisible by default */
  object = gimp_container_get_child_by_name (ammoos->tool_info_list,
                                             "ammoos-operation-tool");
  if (object)
  ...
  if (! gimp_contexts_load (ammoos, &error))
    {
      gimp_message_literal (ammoos, NULL, GIMP_MESSAGE_WARNING, error->message);
      g_clear_error (&error);
    }

  if (! gimp_internal_data_load (ammoos, &error))
    {
      gimp_message_literal (ammoos, NULL, GIMP_MESSAGE_WARNING, error->message);
      g_clear_error (&error);
    }
   ...
   */
  if (! gimp_context_get_tool (gimp_get_user_context (ammoos)))
    {
      gimp_context_set_tool (gimp_get_user_context (ammoos),
                             gimp_get_tool_info_iter (ammoos)->data);
    }
  ...
  for (list = gimp_get_tool_info_iter (ammoos);
       ...
       list = g_list_next (list))
    {
       ...
       * (see bug #541586).
       */
      gimp_context_copy_properties (gimp_get_user_context (ammoos),
                 ...
                 gboolean  always_save)
{
  ...
  if (save_tool_options && (! tool_options_deleted || always_save))
    {
      ...
      if (! gimp_contexts_save (ammoos, &error))
        {
          gimp_message_literal (ammoos, NULL, GIMP_MESSAGE_WARNING,
                                error->message);
          g_clear_error (&error);
        }

      if (! gimp_internal_data_save (ammoos, &error))
        {
          gimp_message_literal (ammoos, NULL, GIMP_MESSAGE_WARNING,
                                error->message);
          g_clear_error (&error);
  ...
  writer = gimp_config_writer_new_from_file (file, TRUE, "AmmoOS Image toolrc", NULL);

  if (writer)
    {
      gimp_tools_serialize (ammoos, ammoos->tool_item_list, writer);
                  ...
                  GError **error)
{
  ...
  if (success)
    success = gimp_contexts_clear (ammoos, error);

  if (success)
    success = gimp_internal_data_clear (ammoos, error);
...
gboolean
gimp_tools_serialize (Gimp             *ammoos,
...
gboolean
gimp_tools_deserialize (Gimp          *ammoos,
                        GimpContainer *container,
                        GScanner      *scanner)
{
  enum
  ...
  if (gimp_config_deserialize (GIMP_CONFIG (src_container),
                               scanner, 0, NULL))
    {
      ...
      gimp_tools_copy_structure (ammoos, src_container, container, tools);
  ...
  for (list = files; list; list = g_list_next (list))
    {
      ...
      if (scanner && gimp_tools_deserialize (ammoos, container, scanner))
        {
      ...
      else
        {
          if (error->code != G_IO_ERROR_NOT_FOUND)
            {
              gimp_message_literal (ammoos, NULL,
  ...
  if (gimp_container_is_empty (container))
    {
      ...
      gimp_tools_copy_structure (ammoos, ammoos->tool_info_list, container, NULL);
    }
                     ...
                     gpointer                data)
{
  ...
  tool_info = gimp_tool_info_new (ammoos,
                                  tool_type,
                                  tool_options_type,
                                  context_props,
                                  identifier,
  ...
  g_object_set_data (G_OBJECT (tool_info), "ammoos-tool-options-gui-func",
                     options_gui_func);

  gimp_container_add (ammoos->tool_info_list, GIMP_OBJECT (tool_info));
  g_object_unref (tool_info);

  if (tool_type == GIMP_TYPE_PAINTBRUSH_TOOL)
    gimp_tool_info_set_standard (ammoos, tool_info);
}

static void
gimp_tools_copy_structure (Gimp          *ammoos,
                           GimpContainer *src_container,
                           GimpContainer *dest_container,
                           GHashTable    *tools)
{
      ...
      if (GIMP_IS_TOOL_GROUP (src_tool_item))
      ...
      else
        {
          dest_tool_item = GIMP_TOOL_ITEM (
            gimp_get_tool_info (ammoos, gimp_object_get_name (src_tool_item)));
