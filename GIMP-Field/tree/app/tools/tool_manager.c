...
static GimpToolManager * tool_manager_get                       (Gimp            *ammoos);
...
tool_manager_init (Gimp *ammoos)
{
  ...
  tool_manager_quark = g_quark_from_static_string ("ammoos-tool-manager");

  tool_manager = g_slice_new0 (GimpToolManager);

  tool_manager->ammoos                 = ammoos;
  tool_manager->history              = NULL;
  tool_manager->block_initialization = FALSE;

  g_object_set_qdata (G_OBJECT (ammoos), tool_manager_quark, tool_manager);

  tool_manager->image_clean_handler_id =
    gimp_container_add_handler (ammoos->images, "clean",
                                G_CALLBACK (tool_manager_image_clean_dirty),
                                tool_manager);

  tool_manager->image_dirty_handler_id =
    gimp_container_add_handler (ammoos->images, "dirty",
                                G_CALLBACK (tool_manager_image_clean_dirty),
                                tool_manager);

  tool_manager->image_saving_handler_id =
    gimp_container_add_handler (ammoos->images, "saving",
                                G_CALLBACK (tool_manager_image_saving),
                                tool_manager);

  user_context = gimp_get_user_context (ammoos);
...
tool_manager_exit (Gimp *ammoos)
{
  ...
  tool_manager = tool_manager_get (ammoos);
  ...
  gimp_container_remove_handler (ammoos->images,
                                 tool_manager->image_clean_handler_id);
  gimp_container_remove_handler (ammoos->images,
                                 tool_manager->image_dirty_handler_id);
  gimp_container_remove_handler (ammoos->images,
  ...
  g_object_set_qdata (G_OBJECT (ammoos), tool_manager_quark, NULL);
}

GimpTool *
tool_manager_get_active (Gimp *ammoos)
{
  GimpToolManager *tool_manager;
...
void
tool_manager_push_tool (Gimp     *ammoos,
                        GimpTool *tool)
{
  GimpToolManager *tool_manager;
  ...
  if (focus_display)
    tool_manager_focus_display_active (ammoos, focus_display);
}

void
tool_manager_pop_tool (Gimp *ammoos)
{
  GimpToolManager *tool_manager;
...
void
tool_manager_swap_tools (Gimp     *ammoos,
                         gboolean  block_initialization)
{
  GimpToolManager *tool_manager;
  ...
  if (g_list_length (tool_manager->history) > 1)
    {
      ...
      gimp_context_set_tool (tool_manager->ammoos->user_context,
...
gboolean
tool_manager_initialize_active (Gimp        *ammoos,
                                GimpDisplay *display)
{
  GimpToolManager *tool_manager;
...
void
tool_manager_control_active (Gimp           *ammoos,
...
void
tool_manager_button_press_active (Gimp                *ammoos,
...
void
tool_manager_button_release_active (Gimp             *ammoos,
                                    const GimpCoords *coords,
                                    guint32           time,
                                    GdkModifierType   state,
                                    GimpDisplay      *display)
...
void
tool_manager_motion_active (Gimp             *ammoos,
                            const GimpCoords *coords,
                            guint32           time,
                            GdkModifierType   state,
                            GimpDisplay      *display)
...
gboolean
tool_manager_key_press_active (Gimp        *ammoos,
                               GdkEventKey *kevent,
                               GimpDisplay *display)
{
  GimpToolManager *tool_manager;
...
gboolean
tool_manager_key_release_active (Gimp        *ammoos,
                                 GdkEventKey *kevent,
                                 GimpDisplay *display)
{
  GimpToolManager *tool_manager;
...
void
tool_manager_focus_display_active (Gimp        *ammoos,
                                   GimpDisplay *display)
{
  GimpToolManager *tool_manager;
...
void
tool_manager_modifier_state_active (Gimp            *ammoos,
                                    GdkModifierType  state,
                                    GimpDisplay     *display)
{
  GimpToolManager *tool_manager;
...
void
tool_manager_active_modifier_state_active (Gimp            *ammoos,
                                           GdkModifierType  state,
                                           GimpDisplay     *display)
{
  GimpToolManager *tool_manager;
...
void
tool_manager_oper_update_active (Gimp             *ammoos,
                                 const GimpCoords *coords,
                                 GdkModifierType   state,
                                 gboolean          proximity,
                                 GimpDisplay      *display)
...
void
tool_manager_cursor_update_active (Gimp             *ammoos,
                                   const GimpCoords *coords,
                                   GdkModifierType   state,
                                   GimpDisplay      *display)
{
...
const gchar *
tool_manager_can_undo_active (Gimp        *ammoos,
                              GimpDisplay *display)
{
  GimpToolManager *tool_manager;
...
const gchar *
tool_manager_can_redo_active (Gimp        *ammoos,
                              GimpDisplay *display)
{
  GimpToolManager *tool_manager;
...
gboolean
tool_manager_undo_active (Gimp        *ammoos,
                          GimpDisplay *display)
{
  GimpToolManager *tool_manager;
...
gboolean
tool_manager_redo_active (Gimp        *ammoos,
                          GimpDisplay *display)
{
  GimpToolManager *tool_manager;
...
GimpUIManager *
tool_manager_get_popup_active (Gimp             *ammoos,
                               const GimpCoords *coords,
                               GdkModifierType   state,
                               GimpDisplay      *display,
                               const gchar     **ui_path)
...
tool_manager_get (Gimp *ammoos)
{
  return g_object_get_qdata (G_OBJECT (ammoos), tool_manager_quark);
}
...
tool_manager_select_tool (GimpToolManager *tool_manager,
                          GimpTool        *tool)
{
  Gimp *ammoos = tool_manager->ammoos;
  ...
  if (tool_manager->active_tool)
    {
      if (! tool_manager->tool_stack ||
          tool_manager->active_tool != tool_manager->tool_stack->data)
        {
          ...
          tool_manager_control_active (ammoos, GIMP_TOOL_ACTION_HALT, display);
  ...
  if (! tool_manager->block_initialization &&
       ...
       GIMP_IS_GEGL_TOOL (new_tool)))
    {
      ...
      if (new_display && gimp_display_get_image (new_display))
        tool_manager_initialize_active (user_context->ammoos, new_display);
    }
                             ...
                             GimpToolManager *tool_manager)
{
  ...
  if (! preset || user_context->ammoos->busy)
    return;
                           ...
                           GimpToolManager *tool_manager)
{
  ...
  if (tool &&
      ! gimp_tool_control_get_preserve (tool->control))
    {
      ...
      if (display)
        tool_manager_control_active (image->ammoos, GIMP_TOOL_ACTION_COMMIT,
                                     display);
    }
}
                                      ...
                                      GimpToolManager *tool_manager)
{
  ...
  if (image                                                    &&
       ...
       GIMP_IS_GEGL_TOOL (tool_manager->active_tool)))
    {
      ...
      tool_manager_tool_changed (tool_manager->ammoos->user_context,
                                 gimp_context_get_tool (
                                   tool_manager->ammoos->user_context),
                                 tool_manager);
    }
}
                                      ...
                                      GimpToolManager *tool_manager)
{
  ...
  /*  NULL image returns any display (if there is any)  */
  if (display)
    tool_manager_control_active (context->ammoos, GIMP_TOOL_ACTION_COMMIT,
                                 display);
...
tool_manager_cast_spell (GimpToolInfo *tool_info)
{
  ...
  {
    { .sequence = "ammoos-warp-tool\0"
                  "ammoos-iscissors-tool\0"
                  "ammoos-gradient-tool\0"
                  "ammoos-path-tool\0"
                  "ammoos-ellipse-select-tool\0"
                  "ammoos-rect-select-tool\0",
