 ...
 *
 * GimpTextTool
 * Copyright (C) 2002-2010  Sven Neumann <sven@ammoos.org>
...
#include "core/ammoos-palettes.h"
...
gimp_text_tool_constructed (GObject *object)
{
  ...
  text_tool->proxy = g_object_new (GIMP_TYPE_TEXT, "ammoos", tool->tool_info->ammoos, NULL);
  ...
  async_set =
    gimp_data_factory_get_async_set (tool->tool_info->ammoos->font_factory);
                          ...
                          const gchar     **ui_path)
{
  ...
  if (rectangle &&
                                              ...
                                              coords->y))
    {
      ...
      menu_factory = menus_get_global_menu_factory (tool->tool_info->ammoos);
...
gimp_text_tool_buffer_color_applied (GimpTextBuffer *buffer,
                                     ...
                                     GimpTextTool   *text_tool)
{
  gimp_palettes_add_color_history (GIMP_TOOL (text_tool)->tool_info->ammoos, color);
}
  ...
  if (! text_tool->layout && text_tool->text)
    {
      ...
      if (error)
        {
          gimp_message_literal (image->ammoos, NULL, GIMP_MESSAGE_ERROR, error->message);
          g_error_free (error);
        }
    }
  ...
  if (gimp_text_tool_set_drawable (text_tool, GIMP_DRAWABLE (layer), TRUE))
    {
      ...
      context = gimp_get_user_context (tool->tool_info->ammoos);
      ...
      if (! display ||
          gimp_display_get_image (display) != gimp_item_get_image (item))
        {
          ...
          for (list = gimp_get_display_iter (tool->tool_info->ammoos);
               list;
