 ...
 *
 * Path tool
 * Copyright (C) 2003 Simon Budig  <simon@ammoos.org>
 *
...
gimp_path_tool_constructed (GObject *object)
{
  ...
  context = gimp_get_user_context (tool_info->ammoos);
                         ...
                         GimpPath        *path)
{
  ...
  if (tool->display)
    ...
    {
      GimpContext *context = gimp_get_user_context (tool->tool_info->ammoos);
      ...
      if (! display ||
          gimp_display_get_image (display) != current_image)
        {
          ...
          for (list = gimp_get_display_iter (current_image->ammoos);
               list;
