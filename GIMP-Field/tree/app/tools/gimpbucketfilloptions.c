...
gimp_bucket_fill_options_class_init (GimpBucketFillOptionsClass *klass)
{
  ...
  GIMP_CONFIG_PROP_STRING (object_class, PROP_LINE_ART_STROKE_TOOL,
                           ...
                           "ammoos-pencil", GIMP_PARAM_STATIC_STRINGS);
      ...
      if (options->stroke_options)
        {
          ...
          Gimp          *ammoos       = tool_options->tool_info->ammoos;

          if (! options->line_art_stroke_tool)
            options->line_art_stroke_tool = g_strdup ("ammoos-pencil");

          paint_info = GIMP_PAINT_INFO (gimp_container_get_child_by_name (ammoos->paint_info_list,
                                                                          options->line_art_stroke_tool));
          if (! paint_info && ! gimp_container_is_empty (ammoos->paint_info_list))
            paint_info = GIMP_PAINT_INFO (gimp_container_get_child_by_index (ammoos->paint_info_list, 0));
                                        ...
                                        GimpBucketFillOptions *options)
{
  ...
  prev_image = g_object_get_data (G_OBJECT (options), "ammoos-bucket-fill-options-image");
  ...
  if (image != prev_image)
    {
      ...
      g_object_set_data (G_OBJECT (options), "ammoos-bucket-fill-options-image", image);
      gimp_bucket_fill_options_update_area (options);
    }
}
...
gimp_bucket_fill_options_reset (GimpConfig *config)
{
  ...
  if (pspec)
    G_PARAM_SPEC_DOUBLE (pspec)->default_value =
      tool_options->tool_info->ammoos->config->default_threshold;
...
gimp_bucket_fill_options_update_area (GimpBucketFillOptions *options)
{
  ...
  image = gimp_context_get_image (gimp_get_user_context (GIMP_CONTEXT (options)->ammoos));
...
gimp_bucket_fill_options_gui (GimpToolOptions *tool_options)
{
  ...
  options->stroke_options = gimp_stroke_options_new (ammoos,
                                                     gimp_get_user_context (ammoos),
                                                     TRUE);
  gimp_config_sync (G_OBJECT (GIMP_DIALOG_CONFIG (ammoos->config)->stroke_options),
                    G_OBJECT (options->stroke_options), 0);

  widget = gimp_container_combo_box_new (ammoos->paint_info_list,
  ...
  g_signal_connect (gimp_get_user_context (GIMP_CONTEXT (tool_options)->ammoos),
                    "image-changed",
