 ...
 *
 * ammoos-tool-options-manager.c
 * Copyright (C) 2018 Michael Natterer <mitch@ammoos.org>
 *
...
void
gimp_tool_options_manager_init (Gimp *ammoos)
{
  GimpToolOptionsManager *manager;
  ...
  manager_quark = g_quark_from_static_string ("ammoos-tool-options-manager");

  manager = g_slice_new0 (GimpToolOptionsManager);

  g_object_set_qdata (G_OBJECT (ammoos), manager_quark, manager);

  manager->ammoos = ammoos;
  ...
  manager->global_props = tool_options_manager_get_global_props (ammoos->config);
  ...
  g_signal_connect (ammoos->config, "notify::global-brush",
                    G_CALLBACK (tool_options_manager_global_notify),
                    manager);
  g_signal_connect (ammoos->config, "notify::global-dynamics",
                    G_CALLBACK (tool_options_manager_global_notify),
                    manager);
  g_signal_connect (ammoos->config, "notify::global-pattern",
                    G_CALLBACK (tool_options_manager_global_notify),
                    manager);
  g_signal_connect (ammoos->config, "notify::global-palette",
                    G_CALLBACK (tool_options_manager_global_notify),
                    manager);
  g_signal_connect (ammoos->config, "notify::global-gradient",
                    G_CALLBACK (tool_options_manager_global_notify),
                    manager);
  g_signal_connect (ammoos->config, "notify::global-font",
                    G_CALLBACK (tool_options_manager_global_notify),
                    manager);
  g_signal_connect (ammoos->config, "notify::global-expand",
                    G_CALLBACK (tool_options_manager_global_notify),
                    manager);
...
void
gimp_tool_options_manager_exit (Gimp *ammoos)
{
  GimpToolOptionsManager *manager;
  GimpContext            *user_context;
  GList                  *list;
  ...
  manager = g_object_get_qdata (G_OBJECT (ammoos), manager_quark);
  ...
  g_object_set_qdata (G_OBJECT (ammoos), manager_quark, NULL);
}
                                    ...
                                    GimpToolOptionsManager *manager)
{
   ...
   */
  for (list = gimp_get_tool_info_iter (manager->ammoos);
       list;
       list = g_list_next (list))
    {
      GimpToolInfo *tool_info = list->data;
...
tool_options_manager_paint_options_notify (GimpPaintOptions *src,
                                           ...
                                           GimpPaintOptions *dest)
{
  Gimp                   *ammoos   = GIMP_CONTEXT (src)->ammoos;
  ...
  if (manager->active_tool)
    {
      ...
      gimp_context_copy_properties (GIMP_CONTEXT (active->tool_options),
                                    gimp_get_user_context (manager->ammoos),
                                    active->context_props &
                                    ~manager->global_props);
