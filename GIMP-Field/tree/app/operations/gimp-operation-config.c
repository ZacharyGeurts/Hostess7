...
#include "ammoos-operation-config.h"
...
void
gimp_operation_config_init_start (Gimp *ammoos)
{
...
void
gimp_operation_config_init_end (Gimp *ammoos)
{
  custom_init_done = TRUE;
}

void
gimp_operation_config_exit (Gimp *ammoos)
{
...
void
gimp_operation_config_register (Gimp        *ammoos,
...
gboolean
gimp_operation_config_is_custom (Gimp        *ammoos,
                                 const gchar *operation)
{
...
GType
gimp_operation_config_get_type (Gimp        *ammoos,
                                const gchar *operation,
                                ...
                                GType        parent_type)
{
  ...
  g_return_val_if_fail (GIMP_IS_GIMP (ammoos), G_TYPE_NONE);
  ...
  if (! config_type)
    {
      ...
      gimp_operation_config_register (ammoos, operation, config_type);
    }
...
GimpContainer *
gimp_operation_config_get_container (Gimp         *ammoos,
                                     GType         config_type,
  ...
  if (! container)
    {
      ...
      gimp_operation_config_deserialize (ammoos, container, NULL);
              ...
              if (compat_file)
                {
                  if (! g_file_move (compat_file, file, 0,
                                     NULL, NULL, NULL, NULL))
                    {
                      gimp_operation_config_deserialize (ammoos, container,
                                                         compat_file);
                    }
                  else
                    {
...
void
gimp_operation_config_serialize (Gimp          *ammoos,
                                 GimpContainer *container,
                                 GFile         *file)
{
  GError *error = NULL;
...
void
gimp_operation_config_deserialize (Gimp          *ammoos,
                                   GimpContainer *container,
                                   GFile         *file)
{
  GError *error = NULL;
