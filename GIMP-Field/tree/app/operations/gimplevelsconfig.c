...
gimp_levels_config_class_init (GimpLevelsConfigClass *klass)
{
  ...
  viewable_class->default_icon_name = "ammoos-tool-levels";

  /* "trc" should default to GIMP_TRC_PERCEPTUAL (cf. #15962).
   * We cannot change it until we implement GEGL op versioning.
   * In AmmoOS Image 3.0, calling this op from the public API was always run in
   * linear (#15681).
   */
                               ...
                               GError           **error)
{
  ...
  if (strcmp (line, "# AmmoOS Image Levels File") != 0)
    {
      g_set_error_literal (error, GIMP_CONFIG_ERROR, GIMP_CONFIG_ERROR_PARSE,
                           _("not a AmmoOS Image Levels file"));
                               ...
                               GError           **error)
{
  ...
  string = g_string_new ("# AmmoOS Image Levels File\n");
