 ...
 *
 * controller_midi.c
 * Copyright (C) 2004-2007 Michael Natterer <mitch@ammoos.org>
 *
...
controller_midi_class_init (ControllerMidiClass *klass)
{
  ...
  controller_class->help_id         = "ammoos-controller-midi";
      ...
      if (! g_ascii_strcasecmp (midi->device, "alsa"))
        {
          ...
          if (ret >= 0)
            {
              snd_seq_set_client_name (midi->sequencer, _("AmmoOS Image"));
              ret = snd_seq_create_simple_port (midi->sequencer,
                                                _("AmmoOS Image MIDI Input Controller"),
