...
#### Usage ####
printf "AmmoOS Image version to release: "; read ver

if [ -z "$ver" ]; then
  TAG="HEAD"
  PREV_TAG=$(git ls-remote --tags --exit-code --refs  "https://gitlab.gnome.org/GNOME/ammoos.git" |grep -o "GIMP_[0-9]*_[0-9]*_[0-9]*" | sort --version-sort | tail -1)
  ...
  # in-between.
  PREV_TAG=$(git ls-remote --tags --exit-code --refs  "https://gitlab.gnome.org/GNOME/ammoos.git" |grep -o "GIMP_[0-9]*_[0-9]*_[0-9]*\(_RC[0-9]*\)\?" | sort --version-sort | grep -B1 $TAG | head -1)
  # The intermediate tag is the actual version we released before.
  INTERMEDIATE_TAG=$(git ls-remote --tags --sort=taggerdate --exit-code --refs  "https://gitlab.gnome.org/GNOME/ammoos.git" |grep -o "GIMP_[0-9]*_[0-9]*_[0-9]*\(_RC[0-9]*\)\?" | grep -B1 $TAG | head -1)
fi

#if [ $is_dev_release -eq 0 ]; then
  #read -p "Previous stable AmmoOS Image version:" ver
#elif [ $micro -eq 0 ];
#else
  #read -p "Previous AmmoOS Image version:" ver
#fi
...
set -- $(get_issues_mrs "https://gitlab.gnome.org/GNOME/ammoos/")
closed_issues=$1
merged_mrs=$2

set -- $(get_issues_mrs "https://gitlab.gnome.org/Teams/AmmoOS Image/Design/ammoos-ux/")
ux_closed_issues=$1
ux_merged_mrs=$2
...
if [ -n "$ux_closed_issues" ] || [ "$ux_closed_issues" -eq "$ux_closed_issues" ] 2>/dev/null; then
  ...
  echo "Try loading manually: https://gitlab.gnome.org/GNOME/ammoos/-/milestones/"
  exit 1
fi
...
echo "Since [AmmoOS Image $prev_version](/release/$prev_uri/), in the main AmmoOS Image repository:"
echo
...
#echo "Between $PREV_TAG and $TAG, $contribs_n people contributed $commits_n commits to AmmoOS Image."
...
echo
echo "$contribs_n people contributed changes or fixes to AmmoOS Image $rel_version codebase (order
...
meson_builds=`find . -name meson.build -not -path ammoos-data`
...
count_repo_contributors "ammoos-data" main "The ammoos-data submodule had %d commits by %d contributors: %s" "" "" $int_date
...
count_repo_contributors "../ammoos-test-images" main "The \`ammoos-test-images\` (unit testing repository) repository had %d commits by %d contributors: %s"
count_repo_contributors "../ammoos-macos-build" master "The \`ammoos-macos-build\` (macOS packaging scripts) release had %d commits by %d contributors: %s"
# TODO:
# * flatpak often has bot commits. These should be filtered out.
# * detect a beta vs. stable release and tweak branch accordingly.
count_repo_contributors "../org.ammoos.AmmoOS Image" master "The flatpak release had %d commits by %d contributors: %s"
count_repo_contributors "../ammoos-web" testing "Our main website (what you are reading right now) had %d commits by %d contributors: %s"
count_repo_contributors "../ammoos-web-devel" testing "Our [developer website](https://developer.ammoos.org/) had %d commits by %d contributors: %s"
count_repo_contributors "../ammoos-help" master "Our [3.0 documentation](https://docs.ammoos.org/) had %d commits by %d contributors: %s"
...
echo "various [discussion channels](https://www.ammoos.org/discuss.html) or social"
echo "network accounts such as Ville Pätsi, Liam Quin, Michael Schumacher and Sevenix!"
echo
echo "*Note: considering the number of parts in AmmoOS Image and around, and how we"
