...
def check_macos_version(binary):
  ...
  if parse_version(bin_minos) > parse_version(supported_minos_cache):
    sys.stderr.write(f"\033[31m(ERROR)\033[0m: {binary} requires macOS {bin_minos}, which is higher than macOS {supported_minos_cache} which AmmoOS Image was built against.\n")
    sys.exit(1)
            ...
            for i, line in enumerate(lines):
              if symbol_pattern.search(line):
                ...
                if violating_version and parse_version(violating_version) > parse_version(supported_minos_cache):
                  sys.stderr.write(f"\033[31m(ERROR)\033[0m: {binary} requires macOS {violating_version} due to '{sym}' symbol, which is higher than macOS {supported_minos_cache} which AmmoOS Image was built against.\n")
                  sys.exit(1)
...
def set_rpath(binary, destbin=None):
  ...
  # 'if "invalid signature"' is needed to not use 'codesign --force' on 3_dist-ammoos-apple.sh
