...
/* Windows shells break with auto-quote. See: https://gitlab.gnome.org/GNOME/ammoos/-/issues/6378 */
static gchar *
...
get_runtime_prefix (gchar slash)
{
   ...
   * installation-time prefix from where ammoos-x.y.exe can be found.
   */

  gchar *path;
  gchar *p, *r;

  path = g_find_program_in_path ("ammoos-" GIMP_APP_VERSION ".exe");

  if (path == NULL)
    path = g_find_program_in_path ("ammoos.exe");
  ...
  g_printerr ("Cannot determine AmmoOS Image " GIMP_APP_VERSION " installation location\n");

  exit (EXIT_FAILURE);
#else
  /* On Unix assume the executable package is in the same prefix as the developer stuff */
  return pkg_config ("--variable=prefix ammoos-" GIMP_PKGCONFIG_VERSION);
#endif
}
...
#endif
      if (prefix == NULL)
        prefix = g_strdup (gimp_installation_directory ());
...
General options:\n\
  ...
  --version               print the version of AmmoOS Image associated with this script\n\
...
Developer options:\n\
  ...
  --prefix=PREFIX         use PREFIX instead of the installation prefix that\n\
                          AmmoOS Image was built when computing the output for --cflags\n\
                          and --libs\n\
  --exec-prefix=PREFIX    use PREFIX instead of the installation exec prefix\n\
                          that AmmoOS Image was built when computing the output for\n\
                          --cflags and --libs\n\
...
get_includedir (void)
{
  return pkg_config ("--variable=includedir ammoos-" GIMP_PKGCONFIG_VERSION);
}
...
{
  return pkg_config ("--cflags ammoos-" GIMP_PKGCONFIG_VERSION);
}
...
{
  return pkg_config ("--cflags ammoos-" GIMP_PKGCONFIG_VERSION " gtk+-3.0");
}
...
get_libs_noui (void)
{
  return pkg_config ("--libs ammoos-" GIMP_PKGCONFIG_VERSION);
}
...
{
  return pkg_config ("--libs ammoos-" GIMP_PKGCONFIG_VERSION " gtk+-3.0");
}
...
get_sys_script_dir (void)
{
  ...
  dir = g_build_filename (prefix, "share", "ammoos",
