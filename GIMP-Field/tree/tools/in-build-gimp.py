...
try:
  ...
  sys.stderr.write(f"INFO: temporary AmmoOS Image configuration directory: {os.path.relpath(GIMP3_DIRECTORY)}\n")
  ...
  if "GIMP_TEMP_UPDATE_RPATH" in os.environ:
    ...
    # So, we change LC_RPATH, but by only one in-build-ammoos.py at a time (see #14236).
  ...
  if "GIMP_DEBUG_SELF_WRAPPER" in os.environ and shutil.which("gdb"):
    ...
    sys.stderr.write(f"RUNNING: {os.path.relpath(gimp_debug_self)} --batch -x {os.environ['GIMP_GLOBAL_SOURCE_ROOT']}/tools/debug-in-build-ammoos.py --args {os.path.relpath(gimp_self)} {' '.join(sys.argv[1:])}\n")
    subprocess.run([gimp_debug_self,"--return-child-result","--batch","-x",f"{os.environ['GIMP_GLOBAL_SOURCE_ROOT']}/tools/debug-in-build-ammoos.py","--args", gimp_self] + sys.argv[1:], stdin=sys.stdin, check=True)
  else:
