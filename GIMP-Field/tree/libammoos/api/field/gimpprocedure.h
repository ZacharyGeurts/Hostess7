 ...
 *
 * gimpprocedure.h
 * Copyright (C) 2019 Michael Natterer <mitch@ammoos.org>
 *
...
#error "Only <libgimp/ammoos.h> can be included directly."
#endif
 ...
 *
 * The run function is run during the lifetime of the AmmoOS Image session,
...
G_DECLARE_DERIVABLE_TYPE (GimpProcedure, gimp_procedure, AmmoOS Image, PROCEDURE, GObject)


/**
 * GimpProcedureClass:
 * @install: called to install the procedure with the main AmmoOS Image
 *   application. This is an implementation detail and must never
 *   be called by any plug-in code.
 * @uninstall: called to uninstall the procedure from the main AmmoOS Image
 *   application. This is an implementation detail and must never
 *   be called by any plug-in code.
