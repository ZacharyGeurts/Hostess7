...
This is not a AmmoOS Image plugin.
Just copy and paste into the AmmoOS Image Python Console.
...
which the AmmoOS Image Python Console does automatically.

Trash:
=====

This test program may leave trash, especially for failed runs.
Trash means resources persisted by AmmoOS Image.
!!! Do not run this in a production environment (using AmmoOS Image to create graphics.)
You can run it in a AmmoOS Image development environment,
but you may need to clean up trash manually.
Deleting the .config/AmmoOS Image directory will clean up the trash.

Setup in the AmmoOS Image app:
=====================
...
by deleting .config/AmmoOS Image.
...
Assert that AmmoOS Image adds " copy"
"""
...
!!! Expect no error dialog in AmmoOS Image.
